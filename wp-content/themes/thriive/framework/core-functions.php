	...
	if(is_user_logged_in())
	{
		...
		if($isThriiveEvent)
		{
			$msg = "
				...
				<em style='color: #615c5c;'>
					Note:This is an automatically generated email, please do not reply. Any questions, feel free to contact us at <a href='mailto:accountmanager2@thriive.in'>accountmanager1@thriive.in</a> for help.
				<em>
			";
		...
		else
		{
			$msg = "
				...
				Team Thriive<br><br>
				<em style='color: #615c5c;'>
					Note:This is an automatically generated email, please do not reply. Any questions, feel free to contact us at <a href='mailto:accountmanager2@thriive.in'>accountmanager2@thriive.in</a> for help.
				<em>
			";
...
function deleteGallery($input)
{
	if(is_user_logged_in()) 
	{
		...
		$current_user = wp_get_current_user();
		$post_id = $current_user->post_id;
		$isDelete = delete_row('gallery', $row_index, $post_id);
...
function addLogoutLinkInNavigation($items, $args) 
{
	if($args->theme_location == 'menu-1')
	{
	    if (is_user_logged_in()) 
	    {
			$items .= '<li class="menu_logout"><a href="' . wp_logout_url('/soul-html/') . '">Logout</a></li>';
		}
	}
	if($args->theme_location == 'menu-2')
	{
	    if (is_user_logged_in()) 
	    {
			//$items .= '<li class="main_menuli  menu-item"><a href="' . site_url().'/issues-feedback-page">Issues / Feedback</a></li>';
...
function submitFeedback($input){
	if(isset($input['feedback']) && isset($input['user_id']) && isset($input['feed_ref']))
	{
		...
		Team Thriive
		";
		sendEmail('accountmanager2@thriive.in', 'Reviews are in!', $msg);
...
function contactUs($input)
{
	...
	if(isset($input['txtFirstName']) && isset($input['txtLastName']) && isset($input['txtEmail']) && isset($input['txtMobile']) && isset($input['txtMessage']))
	{
		...
		$emailMessage = "
			...
			Team Thriive
		";
		sendEmail("accountmanager2@thriive.in", "Contact us", $emailMessage);
...
*/
				...
				//sendEmail('prathamesh@thriive.in',$current_user->user_login.' stage 1 completed', $current_user->user_login.' has submitted personal details information.');
				
				sendEmail('accountmanager2@thriive.in',$current_user->user_login.' stage 1 completed', $current_user->user_login.' has submitted personal details information.');
...
function update_seeker($input)
{
	...
	if( isset($input['firstname']) && isset($input['mobile']) )
	{
		...
		$mobile = preg_replace('/(?<=\d)\s+(?=\d)/', '', $_POST['mobile']);
	...
	if($_POST['activity'] == 'deleting_account')
	{
		...
		$msg = "
			...
			<em style='color: #615c5c;'>
				Note:This is an automatically generated email, please do not reply. Any questions, feel free to contact us at <a href='mailto:accountmanager1@thriive.in'>accountmanager1@thriive.in</a> for help.
			<em>";
		sendEmail("accountmanager2@thriive.in", "$username has deleted their account.", $msg);
		echo "1";
	}
	else if($_POST['activity'] == 'unsubscribe')
	{
		...
		$msg = "
			...
			<em style='color: #615c5c;'>
				Note:This is an automatically generated email, please do not reply. Any questions, feel free to contact us at <a href='mailto:accountmanager1@thriive.in'>accountmanager1@thriive.in</a> for help.
			<em>";
		sendEmail("accountmanager2@thriive.in", "$username has submitted account deleted reason.", $msg);
		echo "1";
	}	
	exit;
}
	...
	if(isset($input['total_therapies']) && isset($input['package']) && isset($input['communication'])){
			if(is_user_logged_in()){
				...
				if(isset($input['is_review_b4_publish'])){
					...
					//sendEmail('prathamesh@thriive.in',$current_user->user_login.' stage 2 completed', $current_user->user_email.' has filled all required fields and submitted package details information.');
					sendEmail('accountmanager2@thriive.in',$current_user->user_login.' stage 2 completed', $current_user->user_email.' has filled all required fields and submitted package details information.');
				} else {
					//sendEmail('prathamesh@thriive.in',$current_user->user_login.' stage 2 completed', $current_user->user_email.' has submitted package details information.');
					sendEmail('accountmanager2@thriive.in',$current_user->user_login.' stage 2 completed', $current_user->user_login.' has submitted package details information.');
...
function saveSocialNetworkDetails($input){
		if(is_user_logged_in()){	
				...
				//sendEmail('prathamesh@thriive.in',$current_user->user_login.' stage 3 completed', $current_user->user_login.' has submitted socail network details information.');
				sendEmail('accountmanager2@thriive.in',$current_user->user_login.' stage 3 completed', $current_user->user_login.' has submitted socail network details information.');
...
*/
				...
				$total_images=0;
				//echo "<pre>";print_r(get_field("select_package",wp_get_current_user()->post_id));
				$select_package = get_field("select_package",wp_get_current_user()->post_id);
				//print_r($select_package);exit();
				if($select_package[0]->ID == 129){
					$max_images = 5;
					
				}elseif($select_package[0]->ID == 130){
					$max_images = 12;

				} else {
					$max_images = 50;
				}	
				//echo $max_images;exit(); 	
				foreach ($files['name'] as $key => $value) {
					if($key < ($max_images - 1)){
						if ($files['name'][$key]) {
							$file = array(
							...
							$_FILES = array('gallery_'.$key.'_images' => $file);
							//print_r($_FILES);
							$attachment_id = media_handle_upload( 'gallery_'.$key.'_images', wp_get_current_user()->post_id );
							//echo $attachment_id;
							$gallery_row = array('gallery' => $attachment_id);
							$resultData = add_row('gallery', $gallery_row, wp_get_current_user()->post_id);
							//$res = update_field('certificates_'.$key.'_certificate',$attachment_id,wp_get_current_user()->post_id);
						}
					} else {
						return generateJSON('error','Maximum Image '.$key+1,'');
				...
				//sendEmail('prathamesh@thriive.in',$current_user->user_email.' stage 4 completed', $current_user->user_email.' has submitted the form for verification.');
...
function callresendOTP($postdata){
	...
	$msg = $generatedPin . ' is the one time password (OTP) for your online login on Thriive.in';  
...
function generateBitlyURL($url){
	...
	$user_login = 'prathamesh@thriive.in';
...
function upgradePackage()
{
	...
	$msg = "Dear Account Manager,<br><br>
		...
		Team Thriive
		";
	sendEmail('accountmanager2@thriive.in', "Upgrade Thriive package", $msg);
	    ...
	    else if($old_status == 'pending-payments' && $new_status == 'publish') 
    	{
	    	...
	    	if($packageCharges > 0)
		    {
				...
				$msg = "Dear Account Manager,<br><br>
					...
					Love & light,<br>
					Team Thriive";
				sendEmail("accountmanager2@thriive.in", "Money is here!", $msg);
...
function currencyToWords(float $number) {
    ...
    $words = array(0 => '', 1 => 'One', 2 => 'Two',
        ...
        70 => 'Seventy', 80 => 'Eighty', 82 => 'Eighty Two', 90 => 'Ninety');
    ...
    $paise = ($decimal) ? "and " . $words[$decimal].' Paise' : 'and Zero Paise';
...
function assign_masked_number_to_user(){
	...
	if($callcount < 3){
		...
		if($check_caller_entry){
			...
			//sendEmail('prathamesh@thriive.in','Assigned masked number',$msg);
			//sendEmail('manishj@thriive.in','Assigned masked number',$msg);
...
function register_geolocation_menu_page() {
    ...
    add_menu_page('Business Module Page', 'Business Module', 'add_users', 'businessmodulepage', '_businessmodule_menu_page', null, 6);
}
...
function getTCallCountByStatus($mobile, $status){
	...
	$results = $wpdb->get_var("SELECT Count(*) FROM knowlarity_after_call WHERE call_status = '".$status."' AND (agent_number LIKE '%".$mobile."%' OR customer_number LIKE '%".$mobile."%')");
	return $results;
}
	...
	if ( username_exists( $email ) && email_exists($email)) {
		...
		if(!empty($fullname) && !empty($email) && !empty($pwd) && !empty($mobileNumber)) {
			...
			if (!$user_id and email_exists($email) == false) 
			{
				...
				if ( strpos(rtrim($fullname), '@') != false ) {
					echo generateJSON('error', 'Email Id not allowed in firstname');exit();
				}
				/* if (!preg_match('/^[\p{L} ]+$/u', $fullname)) {
					echo generateJSON('error', 'Name must only contain letters!');exit();
				} */
				if(!preg_match('/^\d{10}$/',$mobileNumber)){
					echo generateJSON('error', 'Mobile Number not valid');exit();
				}
				$user_id = wp_insert_user($seekerDetails);
				...
				$mobileNumber = get_user_meta($user_id,'mobile',true);
				$mobileNumber = preg_replace('/(?<=\d)\s+(?=\d)/', '', $mobileNumber);
		
				file_get_contents("https://media.smsgupshup.com/GatewayAPI/rest?method=OPT_IN&format=json&userid=2000209699&password=!7Tb5Q@t&phone_number=".$mobileNumber."&v=1.1&auth_scheme=plain&channel=WHATSAPP");
...
function seeker_signup_sd(){
    ...
    if(!empty($fullname) && !empty($email) && !empty($pwd) && !empty($mobileNumber)) {
		...
		if (!$user_id and email_exists($email) == false && empty($isMobileExist)) 
		{
			...
			//sendEmail('prathamesh@thriive.in','New signup on shreans daga live event pages',$emsg);
	...
	if ( username_exists( $email ) && email_exists($email)) {
		...
		if(!empty($email) && !empty($mobileNumber)) {
			...
			if (!$user_id and email_exists($email) == false)
			{
				...
				$result = sendSMS($countrycode . $mobileNumber, $msg,$tempid);
				sendEmail($email, 'Your OTP for registering on Thriive.in', $msg);
				//sendEmail('prathamesh@thriive.in', 'Your OTP for registering on Thriive.in', $msg);
...
function oc_call_seeker_signup(){
	...
	if(!empty($email) && !empty($mobileNumber)  && !empty($firstname) && empty($isMobileExist)) {
		...
		if (email_exists($email) == false ) {
			...
			if ( strpos(rtrim($firstname), '@') != false ) {
				echo generateJSON('error', 'Email Id not allowed in firstname');exit();
			}
			...
			if ( is_wp_error( $user_id  ) ) {
				echo generateJSON('error', $user_id->get_error_message());exit();
			}
			/* if ( strpos($username, '@') != false ) {
				echo generateJSON('error', 'Email Id not allowed in username');exit();
			} */
			...
			if($mobileNumber == '9561437211'){
				sendEmail('vinay@thriive.in', '+91 9561437211  registering on Thriive.in', "+91 9561437211  registering on Thriive.in");
				sendSMS('9820080865', $msg,$tempid);
				sendEmail('prathamesh@thriive.in', '+91 9561437211  registering on Thriive.in', "+91 9561437211  registering on Thriive.in");
			}	
			//sendEmail($email, 'Your OTP for registering on Thriive.in', $msg);
			echo generateJSON('success','OTP sent on '.$mobileNumber, $user_id.'-'.$countrycode.'-'.$mobileNumber);
			wp_die();
		...
		if (!empty($isMobileExist)) {
			echo generateJSON('error', 'Mobile Number Exist');
		} else {
			echo generateJSON('error','','');
		}	
...
function verify_user(){
	...
	if(!empty($otp)){
		if($dbotp==$otp){
			...
			echo generateJSON('success','You have verify your mobile number successfully.',$user_id);
			auto_login_new_user($user_id);
			wp_die();
		} else {
...
function oc_verify_user(){
	...
	if(!empty($otp)){
		if($dbotp==$otp){
			...
			$url = "https://media.smsgupshup.com/GatewayAPI/rest?userid=2000209699&password=!7Tb5Q@t&send_to=".$mobileNumber."&v=1.1&format=json&msg_type=TEXT&method=SENDMESSAGE&msg=Thank+you+for+registering+with+us.+For+any+assistance+or+issues+please+do+reply+here.+Team+Thriive.&isTemplate=true&header=Welcome+to+Thriive";
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
			$response = curl_exec($ch);
			...
			echo generateJSON('success','You have verified your mobile number successfully.',$user_id);
			auto_login_new_user($user_id);
			wp_die();
		} else {
			echo generateJSON('error','Entered OTP doesn’t match.'.$user_id,$user_id);
			wp_die();
		}
	} else {
		echo generateJSON('error','OTP is required.',$user_id);
...
add_action( 'rest_api_init', function () {
  register_rest_route( 'mo/v1', '/therapistbyslug/therapyid/(?P<term_id>\d+)/page/(?P<page>\d+)/count/(?P<per_page>\d+)', array(
    'methods' => 'GET',
    'callback' => 'getTherapybyslug',
  ) );
} ); 

function getTherapybyslug( $data ) {
	global $wpdb;
	$finalArr = array();
	
	$term = get_term($data['term_id']);
	/* print_r($term);
	echo 'term'.$term->name;exit(); */
	if($data['per_page'] <= 1000){
		if(have_rows('therapist_data', 'option')):
			while(have_rows('therapist_data', 'option')): the_row();
				if($term->slug == get_sub_field('taxonomy')->slug){
...
//'tarot-card-reading'
		...
		foreach($tempArr as $t){
			$post = get_post($t);
			$data_array['post_author'] = $post->post_author;
			$data_array['post_id'] = $t;
			$experience_order = array();
			$tdtchg = array();
			$term = get_term_by('slug',$term->slug, 'therapy');
			...
			$data_array['therapy_Exp'] = $therapy_Exp;
			$data_array['avg_rating'] = get_post_meta($post->ID,'avg_rating',true);
			$data_array['title'] = $post->post_title;
			$data_array['status'] = getTherapistStatus($post->ID);
			$bank_details = get_field('bank_details',$post->ID);
		
			if($bank_details){
				$data_array['account_holder_name'] = $bank_details['account_holder_name'];
				$data_array['bank_name'] = $bank_details['bank_name'];
				$data_array['account_number'] = $bank_details['account_number'];
				$data_array['ifsc_code'] = $bank_details['ifsc_code'];
				array_push($finalArr,$data_array); 
			}
			$querystr2 = "SELECT * FROM wp_postmeta WHERE post_id = '".$post->ID."' AND meta_key LIKE '%availability%' ORDER BY `wp_postmeta`.`meta_id` ASC ";
			$result2 = $wpdb->get_results( $querystr2, OBJECT ); 
			for($r=0;$r < count($result2);$r++){ 	
				$availdays = get_post_meta($post->ID,'availability_0_all_days',true);
				if($availdays == 1){
					$data_array['weekday'] = 'All Days';
					$availtime = get_post_meta($post->ID,'availability_0_time',true);
					for($p=0;$p < $availtime;$p++){ 
						$startvar = 'availability_'.$r.'_time_'.$p.'_start_time';
						$data_array['start_time'] = get_post_meta($post->ID,$startvar,true);			
						$endvar = 'availability_'.$r.'_time_'.$p.'_end_time';
						$data_array['end_time'] = get_post_meta($post->ID,$startvar,true);	
						array_push($finalArr,$data_array);
					} 
				} else {
					$weekday = get_post_meta($post->ID,'availability_'.$r.'_day',true);
					$data_array['weekday'] = $weekday;
					$availtime = get_post_meta($post->ID,'availability_'.$r.'_time',true);
					for($p=0;$p < $availtime;$p++){ 
						$startvar = 'availability_'.$r.'_time_'.$p.'_start_time';
						$data_array['start_time'] = get_post_meta($post->ID,$startvar,true);			
						$endvar = 'availability_'.$r.'_time_'.$p.'_end_time';
			...
			$userPost = get_post($post->ID);
			$therapy_terms = get_terms(array('taxonomy' => 'therapy','hide_empty' => false));
			
			$costcounter = get_post_meta($post->ID,'business_therapy_cost_model',true);
			$data_array1 = array();
			for ($x = 0; $x < $costcounter; $x++) {
				$varName = 'business_therapy_cost_model_'.$x.'_therapy_name';
				$data_array['therapy_name']  = $term->name;
				$termid = get_post_meta($post->ID,$varName,true);
				if($termid == $data['term_id']){
					$counter = get_post_meta($post->ID,'business_therapy_cost_model_'.$x.'_therapy_cost',true);
					array_push($finalArr,$data_array);
					for($y = 0;$y < $counter;$y++){
						$data_array1['varDuration'] = get_post_meta($post->ID,'business_therapy_cost_model_'.$x.'_therapy_cost_'.$y.'_select_duration',true);
						$data_array1['varigst'] = get_post_meta($post->ID,'business_therapy_cost_model_'.$x.'_therapy_cost_'.$y.'_price_including_gst',true);
						$data_array1['vargst'] = get_post_meta($post->ID,'business_therapy_cost_model_'.$x.'_therapy_cost_'.$y.'_gst_amount',true);
						$data_array1['varEarning'] = get_post_meta($post->ID,'business_therapy_cost_model_'.$x.'_therapy_cost_'.$y.'_therapist_earning',true);
						array_push($finalArr,$data_array1);
					}
		...
		$data_array['msg'] = "Maximum Limit is only 1000";
		array_push($finalArr,$data_array);  	
	}
	return $finalArr;
}	
	...
	if($data['per_page'] <= 500){
		...
		foreach ($posts as $post) {
			...
			foreach ($keyarr as $key => $value) {
				...
				$data_array['free_session_count'] = $post->free_session_count;
				$data_array['paid_session_count'] = $post->paid_session_count;
				$data_array['interested_in_business_model'] = $post->interested_in_business_model;
				$data_array['communication_channel'] = implode(',', $post->communication_channel);
				$data_array['communication_channel_appointmet'] = implode(',', $post->communication_channel_appointmet);
...
add_action('rest_api_init', function () {
  register_rest_route( 'mo/v1', '/regdate/(?P<regdate>\d+)', array(
    'methods' => 'GET',
    'callback' => 'getAllUsersbydate',
  ));
});
function getAllUsersbydate($data) {
    $finalArr = array();
    global $wpdb;
	
	$regdate = date("Y-m-d",strtotime($data['regdate']));
	
	$users =  $wpdb->get_results("SELECT u.* FROM `wp_users` as u,`wp_usermeta` as w  WHERE DATE(`user_registered`) > '".$regdate."' AND  meta_key = 'wp_capabilities' AND w.meta_value LIKE '%subscriber%' AND w.user_id = u.ID");
	...
	if(!empty($users)){
		foreach($users as $u){
			...
			$usrdet = get_user_meta($u->ID);
			//echo "<pre>";print_r($usrdet);exit();
			$data_array['name'] = $usrdet['first_name'][0] ." ".$usrdet['last_name'][0];
			$data_array['description'] = $usrdet['description'][0];
			$data_array['gender'] = $usrdet['gender'][0] ;
			$data_array['mobile_number'] = $usrdet['mobile'][0] ;
			$data_array['nationality'] = $usrdet['nationality'][0];
			$data_array['countryCode'] = $usrdet['countryCode'][0];
			$data_array['profile_picture'] = $usrdet['thechamp_avatar'][0];
			$data_array['lead_source'] = $usrdet['lead_form_source'][0];
			//print_r($usrdet);exit();
			array_push($finalArr, $data_array);
		}
		return $finalArr;
...
function showavailability(){
	...
	if(in_array('therapist', $members[0]->roles)) {
			...
			$html .= "<div class='form-group' id='alldays1'>
					<div class='row'>
						<div class='col-md-12  col-xs-12'>
							...
							<div class=''>
								<div class='col-md-3 col-xs-3'>
									<input type='text' placeholder='Start Time' name='availability_start_time[alldays][]' id='start0$weekday' class='form-control ' readonly />


								</div>
								<div class='col-md-3 col-xs-3'>
									<input type='text' placeholder='End Time' name='availability_end_time[alldays][]'  id='end0$weekday' class='form-control' readonly />
								</div>
							</div>	
							<div class='col-md-3 col-xs-3'>
		...
		//if(in_array('0',$existData)){
			$html .= "<div class='text-center'><input class='login_btn' type='submit' name='submit_business' value='Save'/></div>";
		//}
	} else {
...
function addBusinessModelByTherapist(){ 
	global $wpdb;
/* 	$current_user = wp_get_current_user(); 
	...
	if(in_array('therapist', $members[0]->roles)) {
		$postID = get_user_meta($members[0]->data->ID, 'post_id', true);
		$html .= "<input type='hidden' name='post_id' value='".$postID."' />";
		$communication_channel = get_field('communication_channel',$postID);
		$communication_channel_appointment = get_field('communication_channel_appointment',$postID);
		$interested_in_business_model = get_field('interested_in_business_model',$postID);
		$interested_in_free_session = get_field('interested_in_free_session',$postID);
		...
		$html .= '
				<div class="border">
					<div class="form-group">
						<div class="row">
							<div class="col-md-12 col-xs-12">
								<label for="Business_model" class="business_model businesstitle" >Business Model</label>
								<p>
								We are delighted to announce the launch of Thrive 2.0, not just a new website but a totally new business model that creates a great market platform and business opportunity for all our verified therapists. <a onclick="interested()" class="bmknmore">Know  More</a>
								</p>
							...
							<div class="col-md-12 col-xs-12">
								...
								<div class="col-md-11 col-xs-11"> 
									<span  id="interested" style="display:none;" class="ytbminteresteddesc">
										<ol>
											<li>All the experts will need to pay a 30% commission to Thriive basis their consultation charges per session.</li>
											<li>You receive an SMS or email alert, with a masked number for contacting the user.</li>
											<li>Now you have to call the user within 2 minutes of receiving the alert and start your session. </li>
										</ol>
									</span>
								</div>
							</div>	
						...
						<div class="row">
							...
							<div class="col-md-12 col-xs-12">
								...
								<div class="col-md-8 col-xs-8">
									<label for="interested_in_business_model" class="interested">I am Interested</label>
									
								</div>
									
								<div class="col-md-4 col-xs-4">
									<label class="switch">
										<input type="checkbox" class="interested" name= "interested_in_business_model" ';
										if($interested_in_business_model == 1){
											$html .= 'checked';
										}
					...
					<div class="col-md-12 col-xs-12">
						...
						<div class="col-md-8 col-xs-8">
							<label for="interested_in_free_session" class="interested">Offer up-to 5 free 20 min sessions a month     
							</label>
							<a class="bmknmore" onclick="popup();">Know  More</a><br/>
							  <span  id="offer" class="ytbmfreedesc" style="display:none;">We offer free trials so that our users can get a feel of the connection themselves, once they are comfortable 95% sign up for a longer sessions, and more than 50% come back for extra sessions. </span>
							
						</div>
						<div class="col-md-4 col-xs-4">
							<label class="switch interested">
								<input type="checkbox" name= "interested_in_free_session" ';
									if($interested_in_free_session == 1){
										$html .= 'checked';
									} 
			...
			';	
					...
					<div class='form-group'>
							...
							<label for='business-therapy' class='col-12'>Business Therapy Cost Model</label>
						
					</div> "; */
				
				//$checked  = array();
				
				$html .="<div class='border'><div class='form-group'>
							<div class='col-md-12 col-xs-12' >
								<label for='therapy' class='yttitle'>Your Therapies</label>
							</div>		
						</div>";
				$checked   = $price = array();	$g=0;$k=0;$l=0;			
				for ($x = 0; $x < $userPost->therapy; $x++) {
					...
					$therapy_name = ($userPost->$varName)[0];
					$html .= "<input type='hidden' id='total_therapies' name='total_therapies' value='$userPost->therapy'><input type='hidden' name='post_id' value='$postID' />";
					
					foreach($therapy_terms as $therapy) {
						if($therapy->parent != 0){ 
							if($therapy_name==$therapy->term_id){ 
								$termrow = $wpdb->get_row("SELECT meta_key  FROM $wpdb->postmeta WHERE meta_value = '". $therapy->term_id ."' ");
								$term_name = $termrow->name;
								//echo 'term'.($userPost->$varName)[0];
								//$y=0;
							/* 	echo $varName;echo "<br/>";
								 echo ($userPost->$varName);echo "<br/>"; */
								$html .= "<input type='hidden' id='therapy_id' name='therapy_id[]' value='".$therapy_name."'>
							
								
								<div class='form-group'>
									<div class='col-md-12 col-xs-12' >
										<label for='therapy' class='ytherapy' >$therapy->name</label>
									</div>
									<div class='col-md-12 col-xs-12'>
										<div class='col-md-3 col-xs-3'>
											<label for='duration' class='ytduration'>Select Duration</label>
										</div>
										<div class='col-md-3 col-xs-3'>
											<label for='price' class='ytprice'>Price (+GST)</label>
										</div>
										<div class='col-md-3 col-xs-3'>
											<label for='price' class='ytgstprice'>GST Amount</label>
										</div>
										<div class='col-md-3 col-xs-3'>
											<label for='price' class='ytearning'>Your Earnings (70%)</label>
										</div>
									</div>
								</div> "; 
							...
							for($y = 0;$y < 5;$y++){
									...
									$varcost = 'business_therapy_cost_model_'.$l.'_therapy_name';
									//echo "SELECT meta_value  FROM $wpdb->postmeta WHERE meta_value = '".$userPost->$varcost."' ";
									/* $termrow = $wpdb->get_row("SELECT meta_value  FROM $wpdb->postmeta WHERE meta_key = '".$userPost->$varcost."' "); */
									//$term_name = $termrow[0];echo $term_name;exit();
									if($userPost->$varcost == $therapy_name){
										$varDuration = 'business_therapy_cost_model_'.$l.'_therapy_cost_'.$y.'_select_duration';
										//echo ($userPost->$varDuration);echo "<br/>";
										$varigst = 'business_therapy_cost_model_'.$l.'_therapy_cost_'.$y.'_price_including_gst';
										
										$vargst = 'business_therapy_cost_model_'.$l.'_therapy_cost_'.$y.'_gst_amount';
										$varEarning ='business_therapy_cost_model_'.$l.'_therapy_cost_'.$y.'_therapist_earning';
								
										
									}
									
								//	echo $userPost->$varigst;echo "<br/>";
								//	echo ($userPost->$varDuration);echo "<br/>";
									if(($userPost->$varDuration) == '5 mins' && (($userPost->$varcost) == $therapy_name)){ $checked[$x][] = '5 mins';	$price[$x][] = $userPost->$varigst;} /* else {
										$checked[$x][] = "";
									}   */
									if(($userPost->$varDuration)  == '10 mins' && (($userPost->$varcost) == $therapy_name)){ $checked[$x][] = '10 mins';	$price[$x][] = $userPost->$varigst;}/*  else {
										$checked[$x][] = "";
									}    */
									if(($userPost->$varDuration)  == '20 mins' && (($userPost->$varcost) == $therapy_name)){ $checked[$x][] = '20 mins';	$price[$x][] = $userPost->$varigst;}/*  else {
										$checked[$x][] = "";
									}   */
									if(($userPost->$varDuration ) == '40 mins' && (($userPost->$varcost) == $therapy_name)){ $checked[$x][] = '40 mins';	$price[$x][] = $userPost->$varigst;} /*  else {
										$checked[$x][] = "";
									}   */
									if(($userPost->$varDuration) == '60 mins' && (($userPost->$varcost) == $therapy_name)){ $checked[$x][] = '60 mins';	$price[$x][] = $userPost->$varigst;}/* else {
										$checked[$x][] = "";
									}  */
					...
					$html .= "<input type='hidden' name='mins[$x][5]' value='5 mins'/>";
					$html .= "
							<div class='form-group'>
								<div class='row'>
									<div class='col-md-12 col-xs-12'>
										<div class='col-md-3 col-xs-3'>
											<label for='mins5$x' class='minstitle'>
											<input type='checkbox' name='cmins[$x][".($userPost->$varName)[0]."][5]' onclick='enablecost(5,$x);' id='mins5$x' "; 
											if(in_array('5 mins',$checked[$x])){ $html .= "checked";}
											$html .= "/><span class='fake-input'></span><span class='fake-label'>5 Mins</span>
											</label>
										</div>
										<div class='col-md-3 col-xs-3'>
											<input type='text' name = 'price[".($userPost->$varName)[0]."][5]' id='price5$x' class='form-control cost5$x' onkeyup = 'calcamt(5,\"$x\");' value='";if(in_array('5 mins',$checked[$x])){ $key = array_search('5 mins',$checked[$x]); $html .= $amt = $price[$x][$key];	 } else { $html .=$amt = 99; }
											
										$html .= "'"; 
										if(!in_array('5 mins',$checked[$x])){ $html .= "readonly"; }
										$gstamt = (0.15 * $amt);
										$html .= "/>
										</div>
										<div class='col-md-3 col-xs-3'>
											<input type='text' name = 'gst[".($userPost->$varName)[0]."][5]'  class='form-control ytgst' id='gst5$x' value='";/* if(in_array('5 mins',$checked[$x])){ */ $html .= ceil($gstamt) ; // } 
										$gstamt = $amt - (0.15 * $amt);
										$html .= "' readonly/>
										</div>
										<div class='col-md-3 col-xs-3'>
											<input type='text' name = 'earn[".($userPost->$varName)[0]."][5]'  class='form-control ytearn' id= 'earn5$x' value='";/* if(in_array('5 mins',$checked[$x])){ */ $html .=  $amt = floor(0.7 * $gstamt) ;//$html .= $amt - (0.3 * $amt);
											$z = $z+1; //} 
										$html .= "' readonly/>
										</div>
									</div>	
							...
							$html .= "<input type='hidden' name='mins[$x][10]' value='10 mins'/>";
							$html .= "<div class='form-group'>	
							<div class='row'>
								<div class='col-md-12 col-xs-12'>
									<div class='col-md-3 col-xs-3'>
										<label for='mins10$x' class='minstitle'>
										<input type='checkbox'  name='cmins[$x][".($userPost->$varName)[0]."][10]' onclick='enablecost(10,$x);'  id='mins10$x' ";  if(in_array('10 mins',$checked[$x])){ $html .= "checked";}
										$html .= "/><span class='fake-input'></span><span class='fake-label'>10 Mins</span>
										</label>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text' name = 'price[".($userPost->$varName)[0]."][10]' id='price10$x' onkeyup = 'calcamt(10,\"$x\");' class='form-control cost10$x' value='";if(in_array('10 mins',$checked[$x])){  $key = array_search('10 mins',$checked[$x]);$html .= $amt = $price[$x][$key]; }  else { $html .= $amt = 199; }
									$html .= "'"; 
									if(!in_array('10 mins',$checked[$x])){ $html .= "readonly"; }
									$gstamt = (0.15 * $amt);
									$html .= "/>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text' name = 'gst[".($userPost->$varName)[0]."][10]'  class='form-control ytgst' id='gst10$x'  value='";/* if(in_array('10 mins',$checked[$x])){  */
									$html .= ceil($gstamt) ; //} 
									$gstamt = $amt - (0.15 * $amt);
									$html .= "' readonly/>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text'  class='form-control ytearn' name ='earn[".($userPost->$varName)[0]."][10]'  id='earn10$x'  value='";/* if(in_array('10 mins',$checked[$x])){ */ $html .= $amt = floor(0.7 * $gstamt)  ;//$html .= $amt - (0.3 * $amt);
										$z = $z+1; //} 
									$html .= "' readonly/>
									</div>
								</div>	
							</div> </div> ";
								$html .= "<input type='hidden' name='mins[$x][20]' value='20 mins'/>";
							$html .= "<div class='form-group'>
								<div class='row'>
								<div class='col-md-12 col-xs-12'>
									<div class='col-md-3 col-xs-3'>
										<label for='mins20$x' class='minstitle'>
										<input type='checkbox'  name='cmins[$x][".($userPost->$varName)[0]."][20]' onclick='enablecost(20,$x);'  id='mins20$x' "; 
										if(in_array('20 mins',$checked[$x])){ $html .= "checked";}
										$html .= "/><span class='fake-input'></span><span class='fake-label'>20 Mins</span>
										</label>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text' name = 'price[".($userPost->$varName)[0]."][20]' id='price20$x' class='form-control cost20$x' onkeyup = 'calcamt(20,\"$x\");' value='";if(in_array('20 mins',$checked[$x])){ $key = array_search('20 mins',$checked[$x]);$html .= $amt = $price[$x][$key]; }  else { $html .= $amt = 299; }
									$html .= "'"; 
									if(!in_array('20 mins',$checked[$x])){ $html .= "readonly"; }
									$gstamt = (0.15 * $amt);
									$html .= "/>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text' name = 'gst[".($userPost->$varName)[0]."][20]'  class='form-control ytgst' id='gst20$x' value='";/* if(in_array('20 mins',$checked[$x])){  */ $html .= ceil($gstamt) ; //} 
									$gstamt = $amt - (0.15 * $amt);	
									$html .= "' readonly/>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text'  class='form-control ytearn' name = 'earn[".($userPost->$varName)[0]."][20]'  id='earn20$x'  value='";/* if(in_array('20 mins',$checked[$x])){  */$html .= $amt = floor(0.7 * $gstamt)  ;$z = $z+1;  //}
									$html .= "' readonly/>
									</div>
								</div>	
							</div> 
						...
						";
							$html .= "<input type='hidden' name='mins[$x][40]' value='40 mins'/>";
							$html .= "<div class='form-group'>
								<div class='row'>
								<div class='col-md-12  col-xs-12'>
									<div class='col-md-3 col-xs-3'>
										<label for='mins40$x' class='minstitle'>
										<input type='checkbox'  name='cmins[$x][".($userPost->$varName)[0]."][40]' onclick='enablecost(40,$x);'  id='mins40$x' "; 
										if(in_array('40 mins',$checked[$x])){ $html .= "checked";}
										$html .= "/><span class='fake-input'></span><span class='fake-label'>40 Mins</span>
										</label>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text' name = 'price[".($userPost->$varName)[0]."][40]' id='price40$x' class='form-control cost40$x' onkeyup = 'calcamt(40,\"$x\");' value='";if(in_array('40 mins',$checked[$x])){ $key = array_search('40 mins',$checked[$x]); $html .= $amt = $price[$x][$key]; }  else { $html .=  $amt = 499; }
									$html .= "'"; 
									if(!in_array('40 mins',$checked[$x])){ $html .= "readonly"; }
									$gstamt = (0.15 * $amt);
									$html .= "/>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text' name = 'gst[".($userPost->$varName)[0]."][40]'  class='form-control ytgst' id='gst40$x' value='";/* if(in_array('40 mins',$checked[$x])){ */ $html .= ceil($gstamt) ; // } 
										
									$gstamt = $amt - (0.15 * $amt);	
									$html .= "' readonly/>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text'  class='form-control' name = 'earn[".($userPost->$varName)[0]."][40]'  id='earn40$x' value='";/* if(in_array('40 mins',$checked[$x])){  */  $html .= $amt = floor(0.7 * $gstamt) ; $z = $z+1; //} 
									$html .= "' readonly/>
									</div>
								</div>	
							</div> 
							...
							$html .= "<input type='hidden' name='mins[$x][60]' value='60 mins'/>";
							$html .= "<div class='form-group'>
							<div class='row'>
								<div class='col-md-12  col-xs-12'>
									<div class='col-md-3 col-xs-3'>
										<label for='mins60$x' class='minstitle'>
										<input type='checkbox'  name='cmins[$x][".($userPost->$varName)[0]."][60]' onclick='enablecost(60,$x);'  id='mins60$x' "; 
										if(in_array('60 mins',$checked[$x])){ $html .= "checked";}
										$html .= "/><span class='fake-input'></span><span class='fake-label'>60 Mins</span>
										</label>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text' name = 'price[".($userPost->$varName)[0]."][60]' id='price60$x' class='form-control cost60$x' onkeyup = 'calcamt(60,\"$x\");' value='";if(in_array('60 mins',$checked[$x])){ $key = array_search('60 mins',$checked[$x]); $html .=  $amt = $price[$x][$key];	 }  else { $html .= $amt = 999; }
									
									$html .= "'"; 
									if(!in_array('60 mins',$checked[$x])){ $html .= "readonly"; }
									$gstamt =(0.15 * $amt);
									$html .= "/>
									
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text' name = 'gst[".($userPost->$varName)[0]."][60]' class='form-control' id='gst60$x' value='";/* if(in_array('60 mins',$checked[$x])){ */  $html .= ceil($gstamt) ; //} 
									$gstamt = $amt - (0.15 * $price[$x][$key]);		
									$html .= "' readonly/>
									</div>
									<div class='col-md-3 col-xs-3'>
										<input type='text'  class='form-control ytearn' name = 'earn[".($userPost->$varName)[0]."][60]'  id='earn60$x' value='";/* if(in_array('60 mins',$checked[$x])){ */   $html .= $amt = floor(0.7 * $gstamt) ;$z = $z+1;  //}
									$html .= "' readonly/>
									</div>
								</div>	 
							</div>
...
//	exit();
			...
			<div class='form-group'>
				<div class='row'>
					<div class='col-md-12  col-xs-12'>
						...
						<label for='bank_details' class='bank_details'>Bank Details</label>
		...
		$html .= "<div class='form-group'><div class='row'><div class='col-md-12  col-xs-12'><div class='col-md-6 col-xs-6'><label for='acc_holder_name' class='acc_holder_name'>Account Holder Name</label></div><div class='col-md-6 col-xs-6'><input type='text' class='form-control' id='acc_holder_name' name='acc_holder_name' value='$ahn' data-parsley-required-message='Account Holder Name is required.' /><div id='ahn_error' style='color:red'></div></div></div></div></div>";
		
		$html .= "<div class='form-group'><div class='row'><div class='col-md-12  col-xs-12'><div class='col-md-6 col-xs-6'><label for='bank_name' class='bank_name'>Bank Name</label></div><div class='col-md-6 col-xs-6'><input type='text' class='form-control' id='bank_name' name='bank_name' value='$bn' data-parsley-required-message='Bank Name is required.' /><div id='bn_error' style='color:red'></div></div></div></div></div>";
		
		$html .= "<div class='form-group'><div class='row'><div class='col-md-12  col-xs-12'><div class='col-md-6 col-xs-6'><label for='acc_number' class='acc_number'>Account Number</label></div><div class='col-md-6 col-xs-6'><input type='text' class='form-control' id='acc_number' name='acc_number' value='$an' data-parsley-required-message='Account Number is required.' /><div id='ahn_error' style='color:red'></div></div></div></div></div>";
		
		$html .= "<div class='form-group'><div class='row'><div class='col-md-12  col-xs-12'><div class='col-md-6 col-xs-6'><label for='ifsc_code' class='ifsc_code'>IFSC Code</label></div><div class='col-md-6 col-xs-6'><input type='text' class='form-control' id='ifsc_code' name='ifsc_code' value='$ifsc' data-parsley-required-message='IFSC Code is required.' /><div id='ifsc_error' style='color:red'></div></div></div></div></div>";
		
		$html .='</div>
		<div class="border">
			<div class="form-group">
				<label for="communication-channel" class="col-12 communication-channel ">Communication Channel</label>
				
				<div class="row">
					<label for="communication-channel" class="col-12 ytinstantservice ">Instant Service</label>
					<p class="intdesc">Select the services are you will be immediately available for</p>
					<div class="checkbox col-md-4 col-xs-4">
						<label for="call" class="call">
						<input type="checkbox" name="communication-channel[]" value="Call" id="call" data-parsley-multiple="communication-channel" data-parsley-errors-container="#message-holder" required ';
						if(in_array("Call" , $communication_channel)){ 
						$html .= "checked"; }
						$html .= '><span class="fake-input"></span>
							<span class="fake-label">Call</span></label>
					</div>
					
					<div class="checkbox col-md-4 col-xs-4">
						<label for="chat" class="chat">
						<input type="checkbox" name="communication-channel[]" value="Chat" id="chat" data-parsley-multiple="communication-channel" data-parsley-errors-container="#message-holder" required ';
						if(in_array("Chat" , $communication_channel)){ $html .= "checked"; }
						$html .= '>
						<span class="fake-input"></span>
							<span class="fake-label">Chat</span></label>
					</div>
					
					<div class="checkbox col-md-4 col-xs-4">
						<label for="video-call" class="video-call">
						<input type="checkbox" value="Video Call" name="communication-channel[]" id="video-call" data-parsley-multiple="communication-channel" data-parsley-errors-container="#message-holder" required '; 
						if(in_array("Video Call" , $communication_channel)){ $html .= "checked"; }
						$html .= '>
						<span class="fake-input"></span>
						<span class="fake-label">Video Call</span></label>
					</div>
		...
		$html .= '
			<div class="form-group">
				...
				<div class="row">
					<label for="communication-channel-appointment" class="col-12 communication-channel-appointment title">Appointment Booking</label>
					<p class="appdesc">Select the services for which you can give appointments</p>
					<div class="checkbox col-md-4 col-xs-4">
						<label for="acall" class="acall">
						<input type="checkbox" name="communication-channel-appointment[]" value="Call" id="acall"  ';
						if(in_array("Call" , $communication_channel_appointment)){ 
						$html .= "checked"; }
						$html .= '><span class="fake-input"></span>
							<span class="fake-label">Call</span></label>
					</div>
					...
					<div class="checkbox col-md-4 col-xs-4">
						<label for="achat" class="achat">
						<input type="checkbox" name="communication-channel-appointment[]" value="Chat" id="achat" ';
						if(in_array("Chat" , $communication_channel_appointment)){ $html .= "checked"; }
						$html .= '>
						<span class="fake-input"></span>
							<span class="fake-label">Chat</span></label>
					</div>
					...
					<div class="checkbox col-md-4 col-xs-4">
						<label for="avideo-call" class="avideo-call">
						<input type="checkbox" value="Video Call" name="communication-channel-appointment[]" id="avideo-call"  '; 
						if(in_array("Video Call" , $communication_channel_appointment)){ $html .= "checked"; }
						$html .= '>
						<span class="fake-input"></span>
						<span class="fake-label">Video Call</span></label>
					</div>
			...
			<div class="form-group">
				<div class="row">
					<div class="col-md-12  col-xs-12">
						...
						<div class="checkbox col-md-12 col-xs-12" style="text-align:left;">
							<ul style="display:none;">
								<li>All the experts will need to pay a 30% commission to Thriive basis their consultation charges per session. 
								</li>
								<li> All the experts are required to give five complementary sessions each month. Each session should be at least 15-20 minutes long. In lieu Thriive will promote experts on all its platforms. 
								</li>
								<li>To maintain the sanctity of this partnership, experts are forbidden from privately consulting the clients they have acquired through thriive. Experts found to do so will be penalised likewise. 
								</li>
								<li>Experts cannot, through their words or actions, can show Thrive in any negative light.
								</li>
								<li>Thrive would highly appreciate it if experts can promote Thrive on their personal social media and within their social network; and invite their peers to join thrive. 
								</li>
							</ul>
							<div class="checkbox col-md-6 col-xs-6">
								<label for="acceptterm" class="acceptterm">
								<input type="checkbox"  id="acceptterm" required >
								<span class="fake-input"></span>
								<span class="yttnc"><a href="https://www.thriive.in/wp-content/themes/thriive/assets/pdf/general-terms-of-use.pdf" target="_blank">Accept Terms and Condition</a> </span></label>
							</div>
						</div>
					</div>
				</div>
...
function addIssuesByTherapist(){
	...
	if(in_array('therapist', $members[0]->roles)) {
		...
		for ($x = 0; $x < $userPost->therapy; $x++) { 
			...
			if(!empty($userPost->$varIssue)) {
				...
				foreach ($ailments as $k => $v) {
					...
					if($k == 0){
						$html .= "<div class='row therapy_issues' style='padding: 0 5%;'><div class='col-md-2 col-xs-2 col-xs-2'><input type='checkbox' name='$htmlname' id='input_$v->slug' data-parsley-mincheck='1' data-parsley-maxcheck='5' data-parsley-mincheck-message='Please select minimum 1 issues' data-parsley-maxcheck-message='Only 5 issues can be selected' data-parsley-errors-container='#issue_error_$xVal' required value='$v->term_id' onClick='selectIssueTag(\"$name\",\"$v->slug\",\"$xVal\");' style='opacity: initial; position: relative; height: 14px;'></div><div class='col-md-10 col-xs-10 label_issues' style='padding-left: 0px'><label for='$v->slug' style='padding-left: 10px;'>$v->name</label></div></div>";
					} else {
						$html .= "<div class='row therapy_issues' style='padding: 0 5%;'><div class='col-md-2 col-xs-2 col-xs-2'><input type='checkbox' name='$htmlname' id='input_$v->slug' value='$v->term_id' onClick='selectIssueTag(\"$name\",\"$v->slug\",\"$xVal\");' style='opacity: initial; position: relative; height: 14px;'></div><div class='col-md-10 col-xs-10 label_issues' style='padding-left: 0px'><label for='$v->slug' style='padding-left: 10px;'>$v->name</label></div></div>";			
					}
				}
		...
		$html .= "<div class='form-group'>
		<div class='col-md-12  col-xs-12'>
			<label for='bank_details'>Timing</label>
		</div>
		</div>
		</div><div class='form-group'>
		<div class='col-md-12  col-xs-12'>
		<div class='col-md-2 col-xs-2'><label for='availability'>Availability</label></div><div class='col-md-7 col-xs-7'><div class='col-md-4'><label for='time'>Start Time</label></div><div class='col-md-4'>End Time</div></div>
		</div>
		</div> ";
		...
		if($alldays == 0){
			$html .= "<div class='form-group' id='alldays1'>
					<div class='row'>
					<div class='col-md-12  col-xs-12'>
					<div class='checkbox col-md-2 col-xs-2'>
					<label for='alldays'>
					<input type='checkbox'  onclick='enabledays();' id='alldays' name='availability_all_days['alldays'][]' value='1' />
					<span class='fake-input'></span>
					<span class='fake-label'>All Days</span>
					...
					<div class='col-md-7 col-xs-7'>
						<div class='col-md-4'>
							<input type='time' placeholder='Start Time' name='availability_start_time[alldays][]' id='start0$weekday' class='form-control' readonly />


						</div>
						<div class='col-md-4'>
							<input type='time' placeholder='End Time' name='availability_end_time[alldays][]'  id='end0$weekday' class='form-control ' readonly />
						</div>
					</div>	
					<div class='col-md-3 col-xs-3'>
						<button type='button' class='form-control  btn$weekday'  onClick='addmondaytime(\"$weekday\");' disabled >Add </button>
					</div>
								...
								<div class='form-group' id='$weekday$k'>
									<div class='row'>
										<div class='col-md-12  col-xs-12'>
											...
											<div class='col-md-7 col-xs-7'>
												<div class='col-md-4'>
													<input type='time' id='start$p$weekday' placeholder='Start Time'  name='availability_start_time[$weekday][]' class='form-control alldays ' value='".$start_time."' required/>
												</div>
												<div class='col-md-4'>
													<input type='time' id='end$p$weekday' placeholder='End Time' name='availability_end_time[$weekday][]' class='form-control alldays ' value='".$end_time."' required/>
												</div>
											</div>";
								if($m == 0){
									$html .= "<div class='col-md-3 col-xs-3'>
									<button type='button' class='form-control btn$weekday'  onClick='addmondaytime(\"$weekday\");'  >Add </button>
									</div>";$m++;
								}  else {
			...
			if($monday == 0){
				...
				<div class='row'>
					...
					<div class='col-md-7 col-xs-7'>
						<div class='col-md-4'>
							<input type='time' placeholder='Start Time' id='start$weekday0' name='availability_start_time[$weekday][]' class='form-control $weekday week' value=''  readonly/>


						</div>
						<div class='col-md-4'>
							<input type='time' placeholder='End Time' id='end$weekday0' name='availability_end_time[$weekday][]' class='form-control $weekday week' value=''  readonly/>
							</div>
						</div>
						<div class='col-md-3 col-xs-3'>
							<button type='button' class='form-control btnweek btn$weekday'  onClick='addmondaytime(\"$weekday\");' disabled >Add </button>
						</div>
					</div>
				</div>
				</div>";
					...
					if($result2[$g]->meta_key == 'availability_'.$r.'_time'){ 
						...
						if($result5[0]->meta_value =='Monday'){
							...
							for($p=0;$p < $counter;$p++){ 
								...
								<div class='col-md-7 col-xs-7'>
								<div class='col-md-4'>
								<input type='time' placeholder='Start Time'  name='availability_start_time[$weekday][]' id='start$p$weekday' class='form-control $weekday week' value='".$start_time."' required/>
								</div>
								<div class='col-md-4'>
								<input type='time'  placeholder='End Time' id='end$p$weekday' name='availability_end_time[$weekday][]' class='form-control $weekday week' value='".$end_time."' required/>
								</div>
								</div>";
								if($m == 0){
									$html .= "<div class='col-md-3 col-xs-3'>
									<button type='button' class='form-control btnweek btn$weekday'  onClick='addmondaytime(\"$weekday\");'  >Add </button>
									</div>";$m++;
								}  else {
									$html .="<div class='col-md-2 col-xs-2'>
									<button type='button' class='form-control btnweek btn$weekday' onClick='removerow(\"$weekday\",\"$k\");' >Remove</button>
			...
			if($tuesday == 0){
				...
				<div class='row'>
					...
					<div class='col-md-7 col-xs-7'>
						<div class='col-md-4'>
							<input type='time' placeholder='Start Time' id='start0$weekday' name='availability_start_time[$weekday][]' class='form-control $weekday week' value=''  readonly/>


						</div>
						<div class='col-md-4'>
							<input type='time'  placeholder='End Time' id='end0$weekday' name='availability_end_time[$weekday][]' class='form-control $weekday week' value=''  readonly/>
							</div>
						</div>
						<div class='col-md-3 col-xs-3'>
							<button type='button' class='form-control btnweek btn$weekday'  onClick='addmondaytime(\"$weekday\");' disabled >Add </button>
					...
					if($result2[$g]->meta_key == 'availability_'.$r.'_time'){ 
						...
						if($result5[0]->meta_value =='Friday'){
							...
							for($p=0;$p < $counter;$p++){ 
								...
								<div class='col-md-7 col-xs-7'>
								<div class='col-md-4'>
								<input type='text' placeholder='Start Time'  name='availability_start_time[$weekday][]' id='start$weekday$p' class='form-control $weekday week' value='".$start_time."' required/>
								</div>
								<div class='col-md-4'>
								<input type='text'  placeholder='End Time' id='end$weekday$p' name='availability_end_time[$weekday][]' class='form-control $weekday week' value='".$end_time."' required/>
								</div>
								</div>";
								if($m == 0){
									$html .= "<div class='col-md-3 col-xs-3'>
			...
			if($saturday == 0){
				...
				<div class='row'>
					...
					<div class='col-md-7 col-xs-7'>
						<div class='col-md-4'>
							<input type='text' placeholder='Start Time' id='start0$weekday' name='availability_start_time[$weekday][]' class='form-control $weekday week' value='' readonly />


						</div>
						<div class='col-md-4'>
							<input type='text'  placeholder='End Time' id='end0$weekday' name='availability_end_time[$weekday][]' class='form-control $weekday week' value='' readonly />
							</div>
						</div>
						<div class='col-md-3 col-xs-3'>
							<button type='button' class='form-control btnweek btn$weekday'  onClick='addmondaytime(\"$weekday\");' disabled >Add </button>
					...
					if($result2[$g]->meta_key == 'availability_'.$r.'_time'){ 
						...
						if($result5[0]->meta_value =='Saturday'){
							...
							for($p=0;$p < $counter;$p++){ 
								...
								<div class='col-md-7 col-xs-7'>
								<div class='col-md-4'>
								<input type='text' placeholder='Start Time'  id='start$p$weekday' name='availability_start_time[$weekday][]' class='form-control $weekday week' value='".$start_time."' required/>
								</div>
								<div class='col-md-4'>
								<input type='text'  placeholder='End Time' id='end$p$weekday' name='availability_end_time[$weekday][]' class='form-control $weekday week' value='".$end_time."' required/>
								</div>
								</div>";
								if($m == 0){
									$html .= "<div class='col-md-3 col-xs-3'>
					...
					if($result2[$g]->meta_key == 'availability_'.$r.'_time'){ 
						...
						if($result5[0]->meta_value =='Sunday'){
							...
							for($p=0;$p < $counter;$p++){ 
								...
								<div class='col-md-7 col-xs-7'>
								<div class='col-md-4'>
								<input type='text' placeholder='Start Time'  name='availability_start_time[$weekday][]' id='start$p$weekday' class='form-control $weekday week' value='".$start_time."' required/>
								</div>
								<div class='col-md-4'>
								<input type='text'  placeholder='End Time' id='end$p$weekday' name='availability_end_time[$weekday][]' class='form-control $weekday week' value='".$end_time."' required/>
								</div>
			...
			$html .= '<div class="form-group">
			  			<div class="row">
							<label for="interested_in_business_model" class="col-12">Interested in Business Model</label>
							<label class="switch">
								<input type="checkbox" name= "interested_in_business_model" ';
								if($interested_in_business_model == 1){
									$html .= 'checked';
								}
			...
			$html .= '<div class="form-group">
			  			<div class="row">
							<label for="interested_in_free_session" class="col-12">Interested in Free Session</label>
							<label class="switch">
								<input type="checkbox" name= "interested_in_free_session" ';
								if($interested_in_free_session == 1){
									$html .= 'checked';
			...
			$html .='<div class="form-group">
			  			<div class="row">
					  		<label for="communication-channel" class="col-12">Communication Channel</label>
					  		<div class="checkbox-wrapper col-6">
						  		<input type="checkbox" name="communication-channel[]" value="Call" id="call" data-parsley-multiple="communication-channel" data-parsley-errors-container="#message-holder" required ';
						  		if(in_array("Call" , $communication_channel)){ 
								$html .= "checked"; }
								$html .= '><label for="call">Call</label>
					  		</div>
					  		...
					  		<div class="checkbox-wrapper col-6">
						  		<input type="checkbox" value="Appointment Booking" name="communication-channel[]" id="appointment-booking" data-parsley-multiple="communication-channel" data-parsley-errors-container="#message-holder" required ';
								if(in_array("Appointment Booking" , $communication_channel)){ $html .= "checked"; } 
								$html .= '>
					...
					$html .= "<div class='form-group'>
						<div class='col-md-12  col-xs-12'>
							<div class='col-md-3 col-xs-3'><label for='duration'>Select Duration</label></div><div class='col-md-3 col-xs-3'><label for='price'>Price Cost</label></div><div class='col-md-3 col-xs-3'><label for='price'>GST Amount</label></div><div class='col-md-3 col-xs-3'><label for='price'>Your Earnings</label></div>
						</div>
					</div> ";
				//$checked  = array();
				...
				for ($x = 0; $x < $userPost->therapy; $x++) {
					...
					$varName = 'therapy_'.$x.'_therapy_name';
					$varCost = 'business_therapy_cost_model_'.$x.'_therapy_cost';
					//echo ($userPost->$varName)[0];echo "<br/>";
					//echo $userPost->$varCost;echo "<br/>";
					...
					foreach($therapy_terms as $therapy) {
						...
						if($therapy->parent != 0){
							...
							if((($userPost->$varName)[0])==($therapy->term_id)){
								//$y=0;
							/* 	echo $varName;echo "<br/>";
								 echo ($userPost->$varName);echo "<br/>"; */
								$html .="<div class='form-group' ><div class='row'><div class='col-md-12  col-xs-12'>$therapy->name</div></div>";
							for($y = 0;$y < 5;$y++){
									
									$varDuration = 'business_therapy_cost_model_'.$x.'_therapy_cost_'.$y.'_select_duration';
									//echo ($userPost->$varDuration);echo "<br/>";
									$varigst = 'business_therapy_cost_model_'.$x.'_therapy_cost_'.$y.'_price_including_gst';
									
									$vargst = 'business_therapy_cost_model_'.$x.'_therapy_cost_'.$y.'_gst_amount';
									$varEarning ='business_therapy_cost_model_'.$x.'_therapy_cost_'.$y.'_therapist_earning';
								
									//echo $userPost->$varDuration;echo "<br/>";
								//	echo ($userPost->$varDuration);echo "<br/>";
									if(($userPost->$varDuration) == '5 mins'){ $checked[$x][] = '5 mins';	$price[$x][] = $userPost->$varigst;} /* else {
										$checked[$x][] = "";
									}   */
									if(($userPost->$varDuration) == '10 mins'){ $checked[$x][] = '10 mins';	$price[$x][] = $userPost->$varigst;}/*  else {
										$checked[$x][] = "";
									}    */
									if(($userPost->$varDuration) == '20 mins'){ $checked[$x][] = '20 mins';	$price[$x][] = $userPost->$varigst;}/*  else {
										$checked[$x][] = "";
									}   */
									if(($userPost->$varDuration) == '40 mins'){ $checked[$x][] = '40 mins';	$price[$x][] = $userPost->$varigst;} /*  else {
										$checked[$x][] = "";
									}   */
									if(($userPost->$varDuration) == '60 mins'){ $checked[$x][] = '60 mins';	$price[$x][] = $userPost->$varigst;}/* else {
										$checked[$x][] = "";
									}  */
									
								}	//print_r($checked);print_r($price);exit();
					//	print_r($price);print_r($checked);
						/*print_r($checked10);	
						print_r($checked60); */
							//	echo ($userPost->$varDuration);
								$html .= "<input type='text' name='mins[$x][]' value='5 mins'/>";
									$html .="<div class='form-group' ><div class='row'><div class='col-md-12  col-xs-12'>";
										...
										<div class='col-md-12  col-xs-12'>
											<div class='col-md-3 col-xs-3'>
												<label for='mins5$x'>
												<input type='checkbox' name='mins[$x][]' onclick='enablecost(5,$x);' id='mins5$x' value='5 mins'"; 
												if(in_array('5 mins',$checked[$x])){ $html .= "checked";}
												$html .= "/><span class='fake-input'></span><span class='fake-label'>5 Mins</span>
												</label>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'price[$x][5]' id='price5$x' class='form-control cost5$x' onkeyup = 'calcamt(5,\"$x\");' value='";if(in_array('5 mins',$checked[$x])){ $key = array_search('5 mins',$checked[$x]); $html .= $amt = $price[$x][$key];	 } 
											...
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'gst[$x][5]'  class='form-control ' id='gst5$x' value='";if(in_array('5 mins',$checked[$x])){ $html .= $gstamt = (0.18 * $price[$x][$key]); } 
											$html .= "' readonly/>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'earn[$x][5]'  class='form-control' id= 'earn5$x' value='";if(in_array('5 mins',$checked[$x])){ $amt = $amt - $gstamt ;$html .= $amt - (0.3 * $amt);$z = $z+1; } 
											$html .= "' readonly/>
											</div>
										</div>	
									</div></div> ";
									$html .= "<input type='text' name='mins[$x][]' value='10 mins'/>";
									$html .= "<div class='form-group'>	<div class='row'>
										<div class='col-md-12  col-xs-12'>
											<div class='col-md-3 col-xs-3'>
												<label for='mins10$x'>
												<input type='checkbox' onclick='enablecost(10,$x);'  id='mins10$x' ";  if(in_array('10 mins',$checked[$x])){ $html .= "checked";}
												$html .= "/><span class='fake-input'></span><span class='fake-label'>1044 Mins</span>
												</label>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'price[$x][10]' id='price10$x' onkeyup = 'calcamt(10,\"$x\");' class='form-control cost10$x' value='";if(in_array('10 mins',$checked[$x])){  $key = array_search('10 mins',$checked[$x]);$html .= $amt = $price[$x][$key]; } 
											$html .= "'"; 
											if(!in_array('10 mins',$checked[$x])){ $html .= "readonly"; }
											$html .= "/>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'gst[$x][10]'  class='form-control' id='gst10$x'  value='";if(in_array('10 mins',$checked[$x])){ $html .= $gstamt = (0.18 * $price[$x][$key]); } 
											$html .= "' readonly/>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text'  class='form-control ' name ='earn[$x][10]'  id='earn10$x'  value='";if(in_array('10 mins',$checked[$x])){  $amt = $amt - $gstamt ;$html .= $amt - (0.3 * $amt);$z = $z+1; } 
											$html .= "' readonly/>
											</div>
										</div>	
									</div> </div> ";
									$html .= "<input type='text' name='mins[$x][]' value='20 mins'/>";
									$html .= "<div class='form-group'><div class='row'>
										<div class='col-md-12  col-xs-12'>
											<div class='col-md-3 col-xs-3'>
												<label for='mins20$x'>
												<input type='checkbox' onclick='enablecost(20,$x);'  id='mins20$x' "; 
												if(in_array('20 mins',$checked[$x])){ $html .= "checked";}
											...
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'price[$x][20]' id='price20$x' class='form-control cost20$x' onkeyup = 'calcamt(20,\"$x\");' value='";if(in_array('20 mins',$checked[$x])){ $key = array_search('20 mins',$checked[$x]);$html .= $amt = $price[$x][$key]; } 
											$html .= "'"; 
											if(!in_array('20 mins',$checked[$x])){ $html .= "readonly"; }
											$html .= "/>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'gst[$x][20]'  class='form-control ' id='gst20$x' value='";if(in_array('20 mins',$checked[$x])){  $html .= $gstamt = (0.18 * $price[$x][$key]); } 
											$html .= "' readonly/>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text'  class='form-control ' name = 'earn[$x][20]'  id='earn20$x'  value='";if(in_array('20 mins',$checked[$x])){ $amt = $amt - $gstamt ;$html .= $amt - (0.3 * $amt);$z = $z+1;  }
											$html .= "' readonly/>
											</div>
										</div>	
									</div> 	</div>";
									$html .= "<input type='text' name='mins[$x][]' value='40 mins'/>";
									$html .= "<div class='form-group'><div class='row'>
										<div class='col-md-12  col-xs-12'>
											<div class='col-md-3 col-xs-3'>
												<label for='mins40$x'>
												<input type='checkbox' onclick='enablecost(40,$x);'  id='mins40$x' "; 
												if(in_array('40 mins',$checked[$x])){ $html .= "checked";}
											...
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'price[$x][40]' id='price40$x' class='form-control cost40$x' onkeyup = 'calcamt(40,\"$x\");' value='";if(in_array('40 mins',$checked[$x])){ $key = array_search('40 mins',$checked[$x]); $html .= $amt = $price[$x][$key]; } 
											$html .= "'"; 
											if(!in_array('40 mins',$checked[$x])){ $html .= "readonly"; }
											$html .= "/>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'gst[$x][40]'  class='form-control' id='gst40$x' value='";if(in_array('40 mins',$checked[$x])){ $html .= $gstamt =  (0.18 * $price[$x][$key]); } 
											$html .= "' readonly/>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text'  class='form-control' name = 'earn[$x][40]'  id='earn40$x' value='";if(in_array('40 mins',$checked[$x])){  $amt = $amt - $gstamt ;$html .= $amt - (0.3 * $amt);$z = $z+1; } 
											$html .= "' readonly/>
											</div>
										</div>	
									</div> </div>";
									...
									$html .= "<div class='form-group'><div class='row'>
										<div class='col-md-12  col-xs-12'>
											<div class='col-md-3 col-xs-3'>
												<label for='mins60$x'>
												<input type='checkbox' onclick='enablecost(60,$x);'  id='mins60$x' "; 
												if(in_array('60 mins',$checked[$x])){ $html .= "checked";}
											...
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'price[$x][60]' id='price60$x' class='form-control cost60$x' onkeyup = 'calcamt(60,\"$x\");' value='";if(in_array('60 mins',$checked[$x])){ $key = array_search('60 mins',$checked[$x]); $html .=  $amt = $price[$x][$key];	 } 
											...
											<div class='col-md-3 col-xs-3'>
												<input type='text' name = 'gst[$x][60]' class='form-control' id='gst60$x' value='";if(in_array('60 mins',$checked[$x])){  $html .= $gstamt = (0.18 * $price[$x][$key]); } 
											$html .= "' readonly/>
											</div>
											<div class='col-md-3 col-xs-3'>
												<input type='text'  class='form-control' name = 'earn[$x][60]'  id='earn60$x' value='";if(in_array('60 mins',$checked[$x])){  $amt = $amt - $gstamt ;$html .= $amt - (0.3 * $amt);$z = $z+1;  }
											$html .= "' readonly/>
											</div>
										</div>	 
									</div>
...
//	echo "<pre>";print_r($price);
	...
	if(isset($input['total_therapies'])){
		...
		sendEmail('accountmanager2@thriive.in','Issues and therapies updated',get_the_title($post_id).' has saved issues from add issue form');
...
function oc_assign_masked_number_to_user($therapist_email,$seeker_email){
	...
	if($check_caller_entry){
		...
		$msg = "Caller Email: ".$seeker->user_email."<br/>Caller Number: ".$c_masked_number."<br/><br/>Receiver Email: ".$therapist->user_email."<br/>Receiver Number: ".$r_masked_number;
		//sendEmail('prathamesh@thriive.in','Assigned masked number',$msg);
		//sendEmail('kazi@thriive.in','Assigned masked number',$msg);

		$result = array("status"=>"success","masked_number"=>$c_masked_number);
		return $result;
		//wp_send_json_success($result);
}
function get_pending_onlinesession($uid,$therapist_id,$package){
	global $wpdb;
	$data = array();
	//$userdetails = $wpdb->get_row("SELECT (SUM(remaining_session)) as pending_session,SUM(no_of_sessions) as total_sessions FROM online_consultation WHERE uid = '". $uid ."' AND payment_status = 'success' ORDER BY id DESC");
	echo "SELECT tid,remaining_session,no_of_sessions,therapy_name FROM online_consultation WHERE uid = '". $uid ."' AND tid = '". $therapist_id ."' AND no_of_sessions = '". $package ."'  AND payment_status = 'success' ORDER BY id DESC";
	$userdetails = $wpdb->get_results("SELECT tid,remaining_session,no_of_sessions,therapy_name FROM online_consultation WHERE uid = '". $uid ."' AND tid = '". $therapist_id ."' AND no_of_sessions = '". $package ."'  AND payment_status = 'success' ORDER BY id DESC");
	foreach($userdetails as $ud){
		$data[$ud->tid][$ud->therapy_name]['remaining_session'] += $ud->remaining_session;
		$data[$ud->tid][$ud->therapy_name]['no_of_sessions'] += $ud->no_of_sessions;
		$usrdet = get_user_meta($ud->tid,0);
		$data[$ud->tid][$ud->therapy_name]['therapist_name'] = $usrdet['first_name'][0] ." ".$usrdet['last_name'][0];
	}
	return $data;
	
}
function get_therapistpending_onlinesession($uid,$therapist_id){
	global $wpdb;
	$data = array();
	$userdetails = $wpdb->get_results("SELECT tid,remaining_session,no_of_sessions,therapy_name FROM online_consultation WHERE  tid = '". $therapist_id ."'  AND no_of_sessions > 1 AND remaining_session > 0 AND payment_status = 'success' ORDER BY id DESC");
	foreach($userdetails as $ud){
		$data[$ud->tid][$ud->therapy_name]['remaining_session'] += $ud->remaining_session;
		$data[$ud->tid][$ud->therapy_name]['no_of_sessions'] += $ud->no_of_sessions;
		$usrdet = get_user_meta($ud->tid,0);
...
function saveAppointmentData($uid,$tid,$first_name,$productinfo,$amount,$status,$txnid,$sessions_booked,$email){
	global $wpdb; 

	if($status == 'success'){
		if($productinfo == 'sex-therapy' || $productinfo == 'love-therapy'){
			if($sessions_booked != 1){
				$sessions_booked = $sessions_booked - 1; 
			}		
			$data = array( 
				'uid' => $uid,
				'tid' => $tid,
				'sessions_booked' => $sessions_booked,
				'sessions_pending' => $sessions_booked,
			...
			$wpdb->insert("appointment_session_details",$data);
		}
		$tempid = "1007714192897078909";
		$uidmobile = get_user_meta($uid,'mobile',true);
		$uidmobile	= preg_replace('/(?<=\d)\s+(?=\d)/', '', $uidmobile);
		
		$sextherapylink = site_url()."/appointment";
		$sextherapy_url = generateBitlyURL($sextherapylink)['data']['url'];
		$tmsg = "Congratulations! Your payment of Rs ".$amount." for ".$sessions_booked." therapy sessions is successful. Please schedule your appointment with your prefered therapist. You can select the same or a different therapist for your future sessions. ".$sextherapylink.". Love & Light - Thriive Wellness";
		sendSMS($uidmobile,$tmsg,$tempid);
		sendSMS('8850630321',$tmsg,$tempid);
		$subject = "Your payment is successful";
		$msg ="Hello ".$first_name.", Congratulations! Your payment of Rs ".$amount." for ".$sessions_booked." therapy sessions is successful. Please schedule your appointment with your prefered therapist. You can select the same or a different therapist for your future sessions. https://www.thriive.in/appointment<br/><br/>All the best!<br/>Love & Light,<br/>Team Thriive";
		sendEmail($email, $subject, $msg);	
	}			
}	
function saveOCAppointmentData($dt){
	$_POST = $dt;
	...
	$email = $_POST['email'];
	$tid = $_POST['tid'];
	$tfirst_name = $_POST['tname'];
	$temail = $_POST['temail'];
	$appointmentdate = $_POST['appointment_date'];
	$payment_txnid = $_POST['payment_txnid'];
	$amount = $_POST['amount'];
	...
	if($status == 'success'){	
		$split_datetime = explode(",",$appointmentdate);
		$timestamp = strtotime($split_datetime[1]) + 3600;
		...
		$video_data['book_date'] = date("Y-m-d",strtotime($split_datetime[0]));
		$video_data['book_time'] = date("H:i:s",strtotime($split_datetime[1]));
		$video_data['end_time'] =  $endtime == '00:00:00'?'23:59:59':$endtime;
		$video_data['channel'] = rand().time();
		//print_r($video_data);exit();
		
		create_room($video_data['channel']);
		$wpdb->insert("oc_video_call",$video_data);
		if($category_name == 'sex-therapy' || $category_name == 'love-therapy'){
			$wpdb->query("UPDATE appointment_session_details SET sessions_pending = sessions_pending - 1 WHERE payment_txnid = '".$payment_txnid."'"); 
		 } else {
			$adata = array( 
				'uid' => $uid,
				'tid' => $tid,
				'sessions_booked' => 1,
				'sessions_pending' => 0,
				'therapy_name' => $category_name,
				'amount' => $amount,
				'payment_status' => $status,
				'payment_txnid' => $payment_txnid,
				'created_date' => date("Y-m-d H:i:s")
			);
			$wpdb->insert("appointment_session_details",$adata);
		}	
		...
		$clink = get_site_url()."/appointment";
		$cgenerated_url = generateBitlyURL($clink)['data']['url']; 
		
		$tsubject = "You have an Appointment booking for ".date("dS M Y",strtotime($split_datetime[0])).", ".date("h:i a",strtotime($split_datetime[1]));
		$tmsg = "Hi,<br/>".$uname." has booked a Appointment session with you. Their payment for  ".date("dS M Y",strtotime($split_datetime[0]))." at ".date("h:i a",strtotime($split_datetime[1]))." session is successful.<br/>Please start the session 5 minutes before the scheduled time, to avoid any delay.<br/><br/>Please Note:<br/>There is no auto complete for Appointment session. It is your responsibility to end the session within the stipulated time.<br/><br/>You can encourage your client to extend the session, a link for which will be shared with the client after this call ends.<br/><br/>Note: This is an internet based audio/ video call, so ensure you have a strong internet connection. Also, ensure that you sit in a well-lit area, with little to no background noise. Once the session is over, please encourage your client to extend the session by buying another session from the Thriive website.<br/>Love & Light,<br/>Team Thriive.";  
		
		sendEmail($temail, $tsubject, $tmsg);
		
		$subject = "Your Appointment with ".$tfirst_name." is confirmed.";
		$link = site_url();
		$website_url = generateBitlyURL($link)['data']['url']; 		   
		
		$msg = "Hello,<br/> Thank you for booking an appointment with ".$tfirst_name.". Your appointment is confirmed for ".date("dS M Y",strtotime($split_datetime[0])).", ".date("h:i a",strtotime($split_datetime[1])).". You can select either to chat or call the Therapist using the Call button on your dashboard. For any help, please reach out to our Customer Service team at support@thriive.in.Note: Once your session is over, you can continue the session with your therapist, or select a new therapist by booking another session from the Thriive website https://www.thriive.in <br/><br/>All the best!<br/>Love & Light,<br/>Team Thriive";
		sendEmail($email, $subject, $msg);
		$tempid = '1007934526215067827';
		$tmsg = "Hello, ".$uname." has booked a session for ".date("dS M Y",strtotime($split_datetime[0]))." on ".date("h:i a",strtotime($split_datetime[1])).". Kindly start the session 5 minutes in advance to avoid any delay. Use ".$clink." to begin the session. For any changes or concerns, please write to our support team. Love & Light, Team Thriive";	
		$tidmobile = get_user_meta($tid,'mobile',true);
		$tidmobile = preg_replace('/(?<=\d)\s+(?=\d)/', '', $tidmobile);
		sendSMS($tidmobile,$tmsg,$tempid);
		
		$tempid = '1007011184981469157';
		$tmsg = "Hello ".$uname." Thank you for booking an appointment with Thriive. Your appointment is confirmed for ".date("dS M Y",strtotime($split_datetime[0])).", ".date("h:i a",strtotime($split_datetime[1])).". Your therapist will start the session as per schedule, so please use the link below or go to Dashboard to access the Appointment Page at least 5 mins in advance. ".$clink." Love & Light,Team Thriive";
		$uidmobile = get_user_meta($uid,'mobile',true);
		$uidmobile	= preg_replace('/(?<=\d)\s+(?=\d)/', '', $uidmobile);
		sendSMS($uidmobile,$tmsg,$tempid);
		sendSMS('9136313785',$tmsg,$tempid);
		sendSMS('9821717700',$tmsg,$tempid);
		sendSMS('9769521131',$tmsg,$tempid);
	}	
} 
function saveOCData($uid,$firstname,$email,$tid,$tfirst_name,$temail,$action,$package,$productinfo,$amount,$status,$txnid,$sendemail,$umobile,$tmobile,$freemins,$coupon,$book_date_time,$tpid,$mihpayid=''){
	...
	if($freemins != ""){
		...
		if($sessionarr[4] != 0){
			...
			$data['session_duration'] =  10;
			
			$sessionarr[4] = 10;
		}	
		$data['category_name'] = $sessionarr[7];
		$data['remaining_duration'] = $sessionarr[4];
...
//	$busy_date = $data['busy_date'];
	...
	if($isexist == 0){
		...
		?>
			...
			/* $(document).ready(function(){
				...
				clevertap.event.push("Payment Success", { 
					...
					"Therapist": "<?php echo $tfirst_name; ?>",
					"Therapy": "<?php echo $productinfo; ?>",
					"Plan Title": "<?php echo $package; ?>",
					"Plan Cost": <?php echo $amount; ?>,
					"Coupon Applied": "<?php echo $is_applied; ?>",
					"Coupon Code": "<?php echo $coupon; ?>",
					"Plan Type": "Paid",
				...
				$.ajax({ 
					...
					type: 'POST',
					dataType : 'html',
					data: {'action': 'clevertap','txnid': '<?php echo $txnid; ?>'},
		...
		<?php 	
		$wpdb->query("UPDATE user_session_taken SET session_taken = session_taken+1 WHERE  uid = '".$uid."'");
		$results = $wpdb->get_results("SELECT * FROM online_consultation  WHERE payment_status = 'success'  AND clevertap = 0 AND DATE(payment_date) = '".date("Y-m-d")."' GROUP BY payuid");
		foreach($results as $rs){
			$package = $rs->package;
			//$txnid = $rs->payment_txnid;
			$payuid = $rs->payuid;
			$temail = $rs->temail;
			$tfirst_name = $rs->tname;
			$status = $rs->payment_status;
			$productinfo = $rs->therapy_name;
			$firstname = $rs->uname;
			$email = $rs->uemail;
			$action = $rs->action;
			$amount = $rs->amount;
			$coupon = $rs->coupon_code;
			if($coupon != '' && $coupon != '-' && $coupon != 'NA'){
				$is_applied = "Yes";
			} else {
				$is_applied = "No";
			}
			if($rs->clevertap == 0 && date("Y-m-d") == date("Y-m-d",strtotime($rs->payment_date))){ 
			...
			<script>
				...
				clevertap.event.push("Payment Success", { 
					...
					"payuid": "<?php echo $payuid; ?>",
					"txnid": "<?php echo $rs->payment_txnid;?>"
				});
				  
			</script>
		<?php	$wpdb->query("UPDATE online_consultation SET clevertap='1' WHERE payment_txnid = '".$rs->payment_txnid ."'");
		...
		$wpdb->insert("call_chat_history",array('oc_id'=>$lastid,'tid' => $tid,'uid' => $uid));
		...
		if($status == 'success'){	
			...
			$tidmobile = preg_replace('/(?<=\d)\s+(?=\d)/', '', $tidmobile);
			if($uid != '78072'){
				$url = "https://media.smsgupshup.com/GatewayAPI/rest?userid=2000209699&password=!7Tb5Q@t&send_to=".$uidmobile."&v=1.1&format=json&msg_type=TEXT&method=SENDMESSAGE&msg=Thank%20You%20for%20your%20payment%20on%20Thriive.%20Enjoy%20your%20session%20and%20in%20case%20of%20any%20issue%20please%20do%20get%20in%20touch.%20Payment%20ID%3A%20".$mihpayid."%20Love%20%26%20Light.%20Team%20Thriive%20!!&isTemplate=true&header=Payment%20Success%20!!";
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, $url);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
				$response = curl_exec($ch);
		...
		if($action == 'call'  && $status == 'success' && $productinfo != 'Appointment' ){
			$call_redirect = 1; 
			...
			$tmsg = "Hi,<br/>".$uname." has booked an Instant Call session with you. Their payment for ".$sessionarr[2]." minutes session is successful.<br/>You will receive a call from +91 8048891009, please answer this call to start your session. For a successful session, ensure that you have a strong network connection on your device.<br/><br/> Note: We now have an auto disconnect feature, wherein the call will disconnect once the purchased session time is over. Please encourage your client to extend the session by purchasing another session from the Thriive website. ";
			$tmsg .= "<br/><br/>Love and Light,<br/>Team Thriive";
		    sendEmail($temail, $tsubject, $tmsg);
			
		} 
		if(($action == 'chat' || $action == 'book_laterchat') && $status == 'success'){ 
			...
			$tsubject = "New Session from ".$uname." -".$lastid;
			$tmsg = "Hi,<br/><br/>".$uname." has booked an Instant Chat session with you. Their payment for ".$sessionarr[2]." minutes session is successful.<br/><br/>Please start the session within the next 2 minutes. For a successful session, ensure that you have a strong internet connection on your device.<br/><br/>Note: We now have an auto disconnect feature, wherein the chat will disconnect once the purchased session time is over. Please encourage your client to extend the session by purchasing another session from the Thriive website.<br/><br/>Love & Light,<br/>Team Thriive";
		    sendEmail($temail, $tsubject, $tmsg);
			$wpdb->query("UPDATE online_consultation SET remaining_session = 0 WHERE id='".$lastid."'");
		}elseif($action == 'chat' && $status == 'success') {
			$wpdb->query("UPDATE online_consultation SET cron_update=2,waiting=0,remaining_session=1,uid_accept=-2,tid_accept=-2,func_name='chat thankyou ' WHERE id='".$lastid."' AND payment_status = 'success'");	
		...
		if($sendemail == "yes" && $status == 'success'){
			...
			if($action == 'call' && ($productinfo == 'all-page' || $productinfo == 'cosmic-astrology' || $productinfo == 'numerology' || $productinfo == 'tarot-card-reading' || $productinfo == 'angel-reading' )){
				...
				$tempid = '1007536510685468677';
				sendSMS($umobile,$umsg,$tempid);
				$subject = "Your payment is successful";
			  
				$msg = "Hey,<br/><br/>You have successfully booked an Instant Call session with ".$tfirst_name." for ".$package." minutes. You will receive a call from the number +91 8048891009. For a seamless session, make sure you have a strong network connection on your device.<br/><br/>Note: We now have an auto disconnect feature, wherein the call will disconnect once the purchased session time is over. You can continue the session with your therapist by booking another session from the Thriive website.<br/><br/>All the best!<br/><br/>Love & Light,<br/>Team Thriive";
			...
			if($action == 'chat'){
				$umsg = "Your payment is successful for  ".$package.". To connect with ".$tfirst_name." click on start chat to connect with an expert on thriive.in";
				$tempid = '1007671932057109060';       
				$password = "password";
				$encrypted_tid = openssl_encrypt($tid,"AES-128-ECB",$password);
				$encrypted_uid = openssl_encrypt($uid,"AES-128-ECB",$password);
			    ...
			    $msg = "Hey,<br/><br/>You have successfully booked an Instant Chat session with ".$tfirst_name." for ".$package." minutes. The therapist will start the session within the next 5 minutes, so make sure you have a strong internet connection on your device.<br/><br/>Note: We now have an auto disconnect feature, wherein the chat will disconnect once the purchased session time is over. You can continue the session with your therapist by booking another session from the Thriive website.<br/><br/>Love & Light, <br/>Team Thriive.";
				sendEmailWithInvoice($email, $subject, $msg, $uid, 'subscriber', 'oc_invoice.php');
				
			}
			$alertmsg = $uname." connected via ".$action." to ".$tfirst_name." for ".$sessionarr[2]." and for therapy ".$productinfo." on thriive"; 
			if(($action == 'book_latercall' || $action == 'book_laterchat' || $action == 'videocall')){
				...
				$msg = "Hey,<br/>You have successfully booked a Premium audio / video session with ".$tfirst_name." for ".date("dS M Y",strtotime($split_datetime[0]))." at ".date("h:i a",strtotime($split_datetime[1])).". The therapist will call you at the scheduled time, so make sure you have a strong internet connection on your device.<br/><br/>Note: Once your session is over, you can continue the session with your therapist by booking another session from the Thriive website.
				<br/><br/>All the best!<br/>Love & Light,<br/>Team Thriive";

				sendEmailWithInvoice($email, $subject, $msg, $uid, 'subscriber', 'oc_invoice.php');
				
				$alertmsg = $uname." connected via video call  to ".$tfirst_name." on ".date("dS M Y",strtotime($split_datetime[0]))." at ".date("h:i a",strtotime($split_datetime[1]))." on thriive";
			}
			
			sendEmail('accountmanager2@thriive.in','Alert! User made the payment and wants to connect with therapist',$alertmsg);
...
function clevertap(){
	global $wpdb;
	$txnid = $_POST['txnid'];
	$wpdb->query("UPDATE online_consultation SET clevertap='1' WHERE payment_txnid = '".$txnid ."'");
	exit();
}	
...
$response = curl_exec($curl);
curl_close($curl);
$created_room_id = json_decode($response,true);
$created_room_id = $created_room_id['data']['id'];
$room_id_new = '294-'.$room_id;
$query = "UPDATE oc_video_call SET page_id=$created_room_id WHERE channel=$room_id_new";
$wpdb->query($query);
$cemail = 'akash@thriive.in';
$csub = 'video';
$cbody = $response.$query;
...
function getTherapistConsultStatus($tid,$created_date=''){ 
	global $wpdb;
	if($created_date == ''){ 
	...
	$consultrow = $wpdb->get_row("SELECT * FROM therapist_availability_status WHERE tid = '". $tid ."' ");
	
	$consultrow1 =0;
	
	return $consultrow;
}
function getTherapistConsultStatus1($tid,$id){ 
	global $wpdb;
	if($created_date == ''){ 
		$currentdate = date("Y-m-d");
	} else {
	...
	$consultrow = $wpdb->get_results("SELECT * FROM online_consultation WHERE tid = '". $tid ."' AND id= '".$id."' ");
	$consultrow1 =0;
	if($consultrow){
		
		foreach($consultrow as $cr){
			$currentdate = $cr->created_date; 
			$session_duration = $cr->session_duration + 5;
			$currenttime = $cr->busy_date;
			
			$consultrow1 = $wpdb->get_var("SELECT COUNT(*) FROM online_consultation WHERE tid = '". $tid ."' AND (busy_date BETWEEN '". date('Y-m-d H:i:s') ."' AND '".$currenttime."') AND payment_status = 'success' AND action='call' AND connected=1 ORDER BY busy_date DESC");
...
function getTherapistChatStatus($tid,$created_date=''){ 
	global $wpdb;
	//$currentdate = date("Y-m-d");
	if($created_date == ''){
		$currentdate = date("Y-m-d");
	...
	$crow = 0;
	$consultrow = $wpdb->get_row("SELECT * FROM online_consultation WHERE tid = '". $tid ."' AND DATE(created_date) = '".$currentdate."' AND action='chat' AND remaining_session > 0");	
	if($consultrow > 0){ 
		$chatrow = $wpdb->get_var("SELECT COUNT(*) FROM chat_block_users WHERE (to_user_id = '".$tid ."' OR from_user_id = '". $tid ."')  AND (complete_status = 1 || incomplete_status = 1)");
		$session_duration = $consultrow->session_duration + 5;
		if($chatrow){	
			$currenttime = date('H:i:s', strtotime($consultrow->busy_date ."+". $session_duration . " mins"));
			$chatrow = $wpdb->get_var("SELECT COUNT(*) FROM online_consultation WHERE tid = '". $tid ."' AND DATE(created_date)  = '".$currentdate."' AND TIME(busy_date) BETWEEN '". date('H:i:s') ."' AND '".$currenttime."' AND payment_status = 'success' AND action='chat' AND remaining_session > 0");
			if($chatrow){	
				$crow = 1;
			}	
		} 
...
function changeavailability(){
	...
	if($current_user->ID != 0){
		...
		$row = $wpdb->get_row("SELECT * FROM therapist_availability_status WHERE tid = '".$current_user->ID ."'");
		$wpdb->query("UPDATE therapist_availability_status SET availability_status = '".$availability."',prev_session_resp=0,auto_deactive=0 WHERE tid = '".$current_user->ID ."'");
		$date = date("Y-m-d H:i:s");
		$ts_id = $row->ts_id;
		$data = array('tid' => $current_user->ID,'availability_status'=> $availability,'created_date'	=> $date);
		$wpdb->insert("therapist_availability_details",$data);
		
		$firstname = get_user_meta($current_user->ID,'first_name',true);
		$lasttname = get_user_meta($current_user->ID,'last_name',true);
		//sendEmail('prathamesh@thriive.in','Availability Changed', $availability.' '.$firstname.' '.$lastname.' '.$current_user->ID);	
	}	
	exit();
}
		...
		for($k=0;$k < $availability;$k++) {
			...
			if($all_days){
				for($j=0;$j < $timecnt;$j++) { 
					...
					$startmin = date('Y-m-d H:i',strtotime($start_time));
					$endhr = date('H',strtotime($end_time));
					$endmin = date('Y-m-d H:i',strtotime($end_time));
					if($starthr < 13){
						if (strtotime($current_time) >= strtotime($startmin)){
							if(strtotime($current_time) <= strtotime($endmin)){
								//if($current_min >= $startmin || $current_min <= $endmin){
									$status = 'Available';	
								//}
							}	
					...
					if($starthr >= 13){
						if (strtotime($current_time) >= strtotime($startmin)){
							if($endhr < 13){
								$current_time = date('Y-m-d H:i');
								$endmin = date('Y-m-d H:i', strtotime('+1 day', strtotime($current_time)));
								if(strtotime($current_time) <= strtotime($endmin)){
									//if($current_min >= $startmin || $current_min >= $endmin){	
										$status = 'Available';
									//}
...
add_action('admin_menu', 'availability_dashboard_admin_menu');

function availability_dashboard_admin_menu() {
    add_submenu_page('index.php', 'Therapist Availability Dashboard ', 'Therapist Availability', 'manage_options', 'availabilitydashboard', 'getAvailability', 1);
...
function getAvailability(){ 
	...
	$sval = $_GET['s'];
	$start = ($paged-1)*$posts_per_page;
	$sql = "SELECT f.*,p.post_title FROM `therapist_availability_details` as f,`wp_posts` as p  WHERE availability_status IN (0,1) AND p.post_author = f.tid AND post_type = 'therapist' ";
	if($sval != "" ){
		$sql .= " AND (post_title LIKE '%".$sval."%')";
	}
	$sql .= " ORDER BY did  DESC ";
	$success_count = count($wpdb->get_results($sql));  
	$oc_count = $success_count ;
	...
	$sql .= " ORDER BY did  DESC LIMIT $start,$posts_per_page";  
	$result = $wpdb->get_results($sql,OBJECT);//getConnectedData
	//<a href='admin.php?page=onlineconsultationdashboard&action=exp_oc_session_data&sd=".$sdval."&ed=".$edval."&s=".$sval."&m=".$mval."&_wpnonce=$nonce' class='page-title-action'>Export to CSV</a>
	$html = "<div class='wrap'><h1 class='wp-heading-inline'>".esc_html( get_admin_page_title() )."</h1><hr class='wp-header-end'>";
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td>Therapist Name</td><td>Availability</td><td>Auto DeActivated</td></tr></thead>";
	$html .= "<form id='fsd-filter' action='".esc_html(admin_url('index.php'))."' method='get'><p class='search-box'><label class='screen-reader-text' for='fsd-search-input'>Search Posts:</label><input type='hidden' value='availabilitydashboard' name='page' /><input type='search' id='fsd-search-input' name='s' value='".$inputval."'><input type='submit' id='search-submit' class='button' value='Search'></p><div class='tablenav top'><div class='alignleft actions'></div></p>";
	$paytxarr = array();
	$autodeactive  = array("0"=>"No","1"=>"Yes");
	//print_r($result);
	foreach($result as $r){
		
		//$therapist = $wpdb->get_row("SELECT * FROM wp_posts WHERE post_author = '".$r->tid ."' AND post_type = 'therapist'");
        $first_name = $r->post_title;
		$html .= "<tr>";
		$html .= "<td>".$first_name."</td>";
		$row = $wpdb->get_row("SELECT * FROM therapist_availability_details WHERE availability_status = 4 AND did > '". $r->did ."' AND tid = '".$r->tid ."' LIMIT 1");
		if($row){
			$availability_range = date("dS M Y h:i a",strtotime($r->created_date)).'--'.date("dS M Y h:i a",strtotime($row->created_date));
		} else {	
			$availability_range = date("dS M Y h:i a",strtotime($r->created_date)).'--';
		} 
			$html .= "<td>". $availability_range ."</td>";
			$html .= "<td>". $autodeactive[$row->auto_deactive] ."</td>";
			$html .= "</tr>";
	}
	
	$html .= "</table><div style='text-align: right;display: inline-flex;'>";
	if($paged > 1){
		$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=availabilitydashboard&paged=1&s=$sval'>First</a></div>";
	}
	if($paged <= 1){
		//$html .= "<div class='disabled' style='margin: 5px 10px;'><a href='#'>Prev</a></div>";
    } else {
    	$pageno = $paged-1;
    	$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=availabilitydashboard&paged=$pageno&s=$sval'>Prev</a></div>";
    }
    if($paged >= $total_pages){
		//$html .= "<div class='disabled' style='margin: 5px 10px;'><a href='#'>Next</a></div>";
    } else {
    	$pageno = $paged+1;
    	$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=availabilitydashboard&paged=$pageno&s=$sval'>Next</a></div>";
    }
	if($paged < $total_pages){
		$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=availabilitydashboard&paged=$total_pages&s=$sval'>Last</a></div></div>";
	}
		
    echo $html;
}

add_action('admin_menu', 'feedback_dashboard_admin_menu');

function feedback_dashboard_admin_menu() {
    add_submenu_page('index.php', 'Feedback Dashboard Page', 'Feedback', 'manage_options', 'feedbackdashboard', 'getFeedback', 1);
...
function getFeedback(){ 
	...
	$mval = $_GET['m'];
	$start = ($paged-1)*$posts_per_page;
	$sql = "SELECT f.* FROM `call_feedback` as f,`online_consultation` as o WHERE 1 ";
	if($sval != "" ){
		$sql .= " AND (uname LIKE '%".$sval."%' OR uemail LIKE '%".$sval."%'  OR therapy_name LIKE '%".$sval."%'  OR payuid LIKE '%".$sval."%')";
	}  
	$sql .= " AND f.ocid = o.id ORDER BY id  DESC";
	$success_count = count($wpdb->get_results($sql));  
	$oc_count = $success_count ;
	...
	$sql = "SELECT f.*,o.tname,o.uname,o.uemail,o.payuid,o.payuid,o.call_id,o.uid,o.tid,o.id,o.action,o.payment_status,o.created_date as payment_date,o.therapy_name FROM `call_feedback` as f,`online_consultation` as o WHERE 1 ";
	if($mval != ""){
		$sql .= " AND payment_status = '". $mval."'";
	}  else {
	//	$sql .= " AND payment_status = 'success'";
	...
	//if($mval == "" && $sdval == "" && $edval == "" && $sval == "" ) {
		$sql .= " AND f.ocid = o.id ORDER BY cid  DESC";
	/* } else {
		$sql .= " ORDER BY id  ";
	} */
	...
	$html = "<div class='wrap'><h1 class='wp-heading-inline'>".esc_html( get_admin_page_title() )."</h1><hr class='wp-header-end'>";
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td>Payment ID</td><td>Therapy</td><td>User Email</td><td>User Mobile</td><td> Therapist</td><td>Therapist Mobile</td><td>Feedback</td><td>Session Type</td><td>Payment Date</td><td>Created Date</td></tr></thead>";
	$html .= "<form id='fsd-filter' action='".esc_html(admin_url('index.php'))."' method='get'><p class='search-box'><label class='screen-reader-text' for='fsd-search-input'>Search Posts:</label><input type='hidden' value='feedbackdashboard' name='page' /><input type='search' id='fsd-search-input' name='s' value='".$inputval."'><input type='submit' id='search-submit' class='button' value='Search'></p><div class='tablenav top'><div class='alignleft actions'></div></p>";
	$paytxarr = array();
	//print_r($result);
	foreach($result as $r){
			...
			$tidmobile = get_user_meta($r->tid,'mobile',true);
			$html .= "<td>".$tidmobile."</td>";
			$html .= "<td><a href='#TB_inline?width=3&height=150&inlineId=".$r->cid ."'  class='thickbox' >";
			if($r->session_type != 4){
				$html .= "Issues</a></td>";
			} else {
				$html .= "Feedback</a></td>";
			}	
			if($r->therapy_name != 'Appointment'){
				$html .= "<td>".$r->action ."</td>";
				
			} else {
				$html .= "<td>".$r->therapy_name .' '. $r->action ."</td>";
			}
			
			$payrow = $wpdb->get_row("SELECT created_date as payment_date FROM online_consultation WHERE payuid = '".$r->payuid ."' ");
			$html .= "<td>".date("dS M Y",strtotime($payrow->payment_date)) ."</td>";
			$html .= "<td>".date("dS M Y H:i",strtotime('+5 hour +30 minutes +1 seconds',strtotime($r->created_date))) ."</td>";
			
			
			$html .= "</tr>";
			$html1 = '<div id="'.$r->cid .'" style="display:none;">';
	
			$therapistrows = $wpdb->get_row("SELECT * FROM  call_feedback WHERE ocid = '". $r->id ."' ");
			if($therapistrows){
				$duration = "";
				if($r->action == 'call'){
					$callrow = $wpdb->get_row("SELECT * FROM knowlarity_after_call WHERE call_uuid LIKE '%".$r->call_id ."%' ");	
					
					if(!empty($callrow)){
						
						
						$cduration = explode(":",$callrow->call_duration);
						
					}
					$call_timer = $callrow->call_timer;
					if($cduration[1] != '00'){
						$callpercentage = ($cduration[1]/$call_timer) * 100 . ' %';
						$duration = $callrow->call_duration;
					} else {
						$callpercentage = "--"; 
					}		
					...
					$callpercentage = "--"; 
					if($r->chat_start_time !=''){
						
						$start_chat_time = date("H:i:s",strtotime($r->chat_start_time));
						if($r->chat_end_time != ''){
							$end_chat_time =  date("H:i:s",strtotime($r->chat_end_time));
						}	else {
							$end_chat_time = "--";
						...
						//$html .= "<td>".$start_chat_time." |".$end_chat_time."</td>";
						//$html .= "<td>".$duration."</td>";
					
					}
				}	
			$session_type = array("1"=>'Call',"2"=>'Chat',"3"=>'Appointment',"4"=>'Others');
			$html1 .= '<table border="1px;" width="100%;"><tr><td>Session Type</td><td>'.$session_type[$therapistrows->session_type].'</td></tr><tr><td>Payment Status</td><td>'.$r->payment_status .'</td></tr><tr><td>Opinion</td><td>'.$therapistrows->call_opinion .'</td></tr><tr><td>Feedback</td><td>'.$therapistrows->others .'</td></tr><tr><td>Error Code</td><td>'.$therapistrows->call_drop .'</td></tr><tr><td>Duration</td><td>'.$duration .'</td></tr><tr><td>Call Percentage</td><td>'.$callpercentage .'</td></tr></table>';
		}
		echo $html1 .= '</div>';
	...
	$html .= "</table><div style='text-align: right;display: inline-flex;'>";
	if($paged > 1){
		$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=feedbackdashboard&paged=1&sd=$sdval&ed=$edval&s=$sval&m=$mval'>First</a></div>";
	}
	if($paged <= 1){
		//$html .= "<div class='disabled' style='margin: 5px 10px;'><a href='#'>Prev</a></div>";
    } else {
    	$pageno = $paged-1;
    	$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=feedbackdashboard&paged=$pageno&sd=$sdval&ed=$edval&s=$sval&m=$mval'>Prev</a></div>";
    }
    if($paged >= $total_pages){
		//$html .= "<div class='disabled' style='margin: 5px 10px;'><a href='#'>Next</a></div>";
    } else {
    	$pageno = $paged+1;
    	$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=feedbackdashboard&paged=$pageno&sd=$sdval&ed=$edval&s=$sval&m=$mval'>Next</a></div>";
    }
	if($paged < $total_pages){
		$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=feedbackdashboard&paged=$total_pages&sd=$sdval&ed=$edval&s=$sval&m=$mval'>Last</a></div></div>";
	}
...
function online_consultation_dashboard_admin_menu() {
    add_submenu_page('index.php', 'Online Consultation Dashboard', 'Online Consultation', 'manage_options', 'onlineconsultationdashboard', 'getOnlineConsultation', 1);
    ...
    add_submenu_page('index.php', 'Inprogress Online Consultation', 'Inprogress Online Consultation', 'manage_options', 'inprogressonlineconsultation', 'getInprogressConsultation', 1);
...
function getOnlineConsultation(){ 
	...
	//$failed_count = $wpdb->get_var("SELECT count(*) FROM `online_consultation` WHERE payment_status = 'failed' GROUP BY payment_txnid");
	$sql = "SELECT DISTINCT(payment_txnid) FROM `online_consultation` o";
	if($nuval != "" ){
		$sql .= ",wp_users u ";
	}	
	$sql .= " WHERE payment_status = 'failed' ";
	if($sval != "" ){
		$sql .= " AND (uname LIKE '%".$sval."%' OR uemail LIKE '%".$sval."%' OR package LIKE '%".$sval."%' OR therapy_name LIKE '%".$sval."%' OR amount LIKE '%".$sval."%' OR payment_txnid LIKE '%".$sval."%' OR payuid LIKE '%".$sval."%')";
	}  
	if($sdval != "" && $edval != ""){
		$sql .= " AND DATE(created_date) BETWEEN '".$sdval."' AND '".$edval."'";
	...
	if($nuval != ""){
		if($sdval == "" && $edval == ""){
			$sdval = $edval = date("Y-m-d");
		}	
		$sql .= " AND DATE(user_registered) BETWEEN '".$sdval."' AND '".$edval."' AND u.id = o.uid ";
	} 
	$sql .= " ORDER BY id  DESC";
	$failed_count = count($wpdb->get_results($sql)); 
	$sql = "SELECT DISTINCT(payment_txnid) FROM `online_consultation` o";
	if($nuval != "" ){
		$sql .= ",wp_users u ";
	}
	$sql .= " WHERE payment_status = 'success' ";
	if($sval != "" ){
		$sql .= " AND (uname LIKE '%".$sval."%' OR uemail LIKE '%".$sval."%'  OR tname LIKE '%".$sval."%' OR package LIKE '%".$sval."%' OR therapy_name LIKE '%".$sval."%' OR amount LIKE '%".$sval."%' OR payment_txnid LIKE '%".$sval."%'  OR payuid LIKE '%".$sval."%')";
	}  
	if($nuval == ""){
		if($sdval != "" && $edval != ""){
			$sql .= " AND DATE(created_date) BETWEEN '".$sdval."' AND '".$edval."'";
		...
		$sql .= " AND DATE(created_date) BETWEEN '".date("Y-m-d")."' AND '".date("Y-m-d")."'";
	}  */	 
	if($cpval != ""){
		$sql .= " AND coupon_code = '". $cpval."'";
	}
	if($catval != ""){
		$sql .= " AND category_name = '". $catval."'";
	}	
	if($nuval != ""){
		if($sdval == "" && $edval == ""){
			$sdval = $edval = date("Y-m-d");
	...
	$oc_count = $success_count ;
	$totalcnt =  $success_count + $failed_count; 
	$sql = "SELECT DISTINCT(payment_txnid) FROM `online_consultation` o";
	if($nuval != "" ){
		$sql .= ",wp_users u ";
	}
	...
	if($sdval == "" && $edval == ""){
		$sdval1 = $edval1 = date("Y-m-d");
	} else {
		$sdval1 = $sdval;
		$edval1 = $edval;
	}	
	$silversql = " AND DATE(payment_date) BETWEEN '".$sdval1."' AND '".$edval1."' AND (category_name = 'silver-readers' OR category_name = '') AND therapy_name != 'Appointment'";
	$goldsql = " AND DATE(payment_date) BETWEEN '".$sdval1."' AND '".$edval1."' AND category_name = 'gold-readers' ";
		
	
	$sql1 = " ORDER BY o.id  DESC";
	//echo 'silver'.$sql.$goldsql.$sql1;
	$silvercnt = count($wpdb->get_results($sql.$silversql.$sql1));  
	$goldcnt = count($wpdb->get_results($sql.$goldsql.$sql1)); 
	if($nuval != ""){
		if($sdval == "" && $edval == ""){
			$sdval = $edval = date("Y-m-d");
		}	
	...
	$bstatusval = ($mval == 'success')?"selected":"";
	$nobstatusval = ($mval == 'failed')?"selected":"";
	$newuserval = ($nuval == '1')?"checked":"";
	$coupval50 = ($cpval == 'THR50')?"selected":"";
	$coupval01 = ($cpval == 'THR20')?"selected":"";
	$coupval20 = ($cpval == 'THRIIVE20')?"selected":"";
	$coupvalLTA = ($cpval == 'LTA20')?"selected":"";
	$coupval10 = ($cpval == 'THR10')?"selected":"";
	$cval = ($catval == 'completed')?"selected":"";
	$ncval = ($catval == 'not-completed')?"selected":"";
	if($sdval != ""){
		$today_date = $sdval;
	} else {
	...
	if($edval != ""){
		...
		$etoday_date = date("Y-m-d");
	}	
	$newusercnt = $wpdb->get_var("SELECT count(*) FROM wp_users WHERE  DATE(user_registered) BETWEEN '".$today_date."' AND  '".$etoday_date."'");
	$newusersesscnt = $wpdb->get_results("SELECT DISTINCT(payment_txnid) FROM wp_users u,online_consultation o WHERE  DATE(user_registered) BETWEEN '".$today_date."' AND  '".$etoday_date."'  AND u.id = o.uid AND o.payment_status = 'success' ");
	
	$sql = "SELECT DISTINCT(payment_txnid) as payment_txnid FROM `online_consultation` o";
	if($nuval != "" ){
		$sql .= ",wp_users u ";
	}
	$sql .= " WHERE 1 ";
	...
	if($catval != ""){
		if($catval == "not-completed"){
			$sql .= " AND ((remaining_duration > 0 AND action = 'call') OR (action = 'chat' AND remaining_session = 1) ) ";	
		} else {	
			$sql .= " AND ((remaining_duration = 0 AND action = 'call' AND busy_date < '".date("Y-m-d H:i:s")."') OR (chat_end_time != '' AND action = 'chat' AND remaining_session = 0) ) ";
		}
	}	
	if($nuval != ""){
		$sql .= " AND DATE(user_registered) BETWEEN '".$sdval."' AND '".$edval."' AND u.id = o.uid ";
...
//
	$html = "<style>.blink_me {
			...
			@keyframes blinker {  
			  50% { opacity: 0; }
			}</style><div class='wrap'><h1 class='wp-heading-inline'>".esc_html( get_admin_page_title() )."</h1><hr class='wp-header-end'>";
	$html .= "<ul class='subsubsub'><li class='all'> Consultation Count: <span class='count'>(".$totalcnt.")</span></li> | <li class='all'> Success Count: <span class='count'>(".$success_count."/".$totalcnt.")</span></li> | <li class='all'> Silver / Gold Count: <span class='count'>(<span id='silvercnt'>".$silvercnt."</span>/<span id='goldcnt'>".$goldcnt."</span>)</span></li> | <li class='all'>New User Count: <span class='count'>(".$newusercnt.")</span></li>  | <li class='all'>New User Session Taken: <span class='count'>(".count($newusersesscnt).")</span></li> </ul>";
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td width='7%' >User Name & Mobile</td><td>User Email</td><td>Action</td><td>Therapist Response</td><td>User Response</td><td>Remaining Session </td><td width='7%'>Final Therapist & Mobile</td><td>Total Therapist Change Count</td><td width='5%'>Therapy Name</td><td width='5%'>Amount</td><td width='5%'>Coupon Code</td><td>Payu ID</td><td>QA Filled</td><td>Session Date</td><td>Pay Date</td><td>Call / Chat  Time </td><td>Duration </td></tr></thead>";
	$html .= "<form id='fsd-filter' action='".esc_html(admin_url('index.php'))."' method='get'><div class='tablenav top'><div class='alignleft actions'><label for='filter-by-status' class='screen-reader-text'>Filter by Status</label><select name='m' id='filter-by-status'><option value=''>All status</option><option value='success' ".$bstatusval.">Success</option><option value='failed' ".$nobstatusval.">Failed</option></select><select name='k' id='filter-by-category'><option value=''>Category</option><option value='completed' ".$cval.">Completed</option><option value='not-completed' ".$ncval.">Not Complete</option></select><select name='cp' id='filter-by-status'><option value=''>Coupon</option><option value='THR50' ".$coupval50.">THR50</option><option value='THRIIVE20' ".$coupval20.">THRIIVE20</option><option value='THR20' ".$coupval01.">THR20</option><option value='LTA20' ".$coupvalLTA.">LTA20</option><option value='THR10' ".$coupval10.">THR10</option></select><label class='all'> New User</label><input type='checkbox' value='1' name='nu' ".$newuserval."/></div><div class='alignleft actions'></div><p><input type='date' name='sd' value='".$sdate."'/><input type='date' name='ed' value='".$edate."'/><input type='hidden' value='onlineconsultationdashboard' name='page' /><input type='search' id='fsd-search-input' name='s' value='".$inputval."'><input type='submit' id='search-submit' class='button' value='Search'><a href='admin.php?page=onlineconsultationdashboard&action=exp_oc_session_data&sd=".$sdval."&ed=".$edval."&s=".$sval."&m=".$mval."&_wpnonce=$nonce' class='page-title-action'>Export to CSV</a></p>";
	$paytxarr = array();
//	<a href='admin.php?page=onlineconsultationdashboard&action=exp_oc_session_data&sd=".$sdval."&ed=".$edval."&s=".$sval."&m=".$mval."&cp=".$cpval."&k=".$catval."&_wpnonce=$nonce' class='page-title-action'>Export to CSV</a>
	//<input type='submit' name='filter_action' id='fsd-query-submit' class='button' value='Filter'><p class='search-box'><label class='screen-reader-text' for='fsd-search-input'>Search Posts:</label><input type='hidden' value='onlineconsultationdashboard' name='page' /><input type='search' id='fsd-search-input' name='s' value='".$inputval."'><input type='submit' id='search-submit' class='button' value='Search'></p><input type='submit' name='date_filter_action' id='fsd-date-submit' class='button' value='Date Filter' />
	//print_r($result);
	//$silvercnt = $goldcnt = 0;
	
	foreach($result as $r){
		
		//if(!in_array($r->payment_txnid,$paytxarr)){
			//$paytxarr[] = $r->payment_txnid;
			$second_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE payment_txnid = '".$r->payment_txnid ."'   ORDER BY id DESC  LIMIT 1");	
			$qacnt = $wpdb->get_var("SELECT COUNT(*) FROM QA_feedback WHERE payment_txnid = '".$second_therapist->payment_txnid ."'");
			...
			$totalcount = 0;
			$chkuser = $wpdb->get_var("SELECT COUNT(DISTINCT(payment_txnid)) FROM online_consultation_archive WHERE uid = '".$r->uid ."' AND created_date < '".$r->created_date ."' AND payment_status = 'success' ");
			if($chkuser > 0){
				$usertype = "Old User";
				$newflag = 1;
				$totalcount = $chkuser;
			}	
			/* $chkuser = $wpdb->get_var("SELECT COUNT(DISTINCT(payuid)) FROM online_consultation WHERE uid = '".$r->uid ."' AND payment_date < '".$r->payment_date ."'  AND payment_status = 'success' ");
			if($newflag == 0){
				$usertype = "Old User";
				$totalcount = $chkuser;	
				$newflag = 1;
			 ...
			 */
			$chkuser = $wpdb->get_row("SELECT * FROM user_session_taken WHERE uid = '".$second_therapist->uid ."' ");	
			if($chkuser->session_taken == 1){
				$usertype = "New User";
				$totalcount = 1;
				
			} else {
				$usertype = "Old User";
				$totalcount = $chkuser->session_taken;
			}		
		
			$umobile = get_user_meta($second_therapist->uid, 'mobile', true);
			$tmobile = get_user_meta($second_therapist->tid, 'mobile', true);
			$color = "";
			/* if($extra_session > 0){
				$color = 'background-color:#F9CEC5;';
				
			}else */ 
			if($second_therapist->cron_update == 1){
				$color = 'background-color:#9FF6E9;';
				
			}
			if($second_therapist->def_warn == 4 && $second_therapist->action == 'call'){
				$color = 'background-color:#bef69f;';
				
			}
			if($second_therapist->therapy_name != 'Appointment'){
				if($second_therapist->category_name == '' || $second_therapist->category_name == 'silver-readers'){
					$category_name = 'Silver Readers';
					$cat_name_slug = "silver-readers";
				} else {
					$category_name = 'Gold Readers';
					$cat_name_slug = "gold-readers";
				}
			} else {
				if($second_therapist->category_name == 'past-life-regression'){
					$category_name = 'PLR';
					$cat_name_slug = "plr-appointment-page";
				}
				if($second_therapist->category_name == 'eft-emotional-freedom-technique'){
					$category_name = 'EFT';
					$cat_name_slug = "eft-appointment";
				}
				if($second_therapist->category_name == 'inner-child-healing'){
					$category_name = 'Inner Child Healing';
					$cat_name_slug = "inner-child-healing";
				}
				if($second_therapist->category_name == 'hypno-therapy'){
					$category_name = 'Hypnotherapy';
					$cat_name_slug = "hypnotherapy-appointment-page";
				}	
				if($second_therapist->category_name == 'sex-therapy'){
					$category_name = 'Sex Therapy';
					$cat_name_slug = "sex-therapy";
				}
				if($second_therapist->category_name == 'love-therapy'){
					$category_name = 'Love Therapy';
					$cat_name_slug = "love-therapy";
				}

			}		
			if(have_rows('therapist_data', 'option')):
				while(have_rows('therapist_data', 'option')): the_row();
					if($cat_name_slug == get_sub_field('taxonomy')->slug){
						$ids = get_sub_field('display_ids');  
					}
				endwhile;
			endif;
			/* while ( have_rows('coupon_prices','option') ) : the_row();
				$temparr = array();
				if(strtoupper(get_sub_field('user_coupon_code')) == strtoupper($second_therapist->coupon_code) && get_sub_field('custom_therapy') == $second_therapist->category_name){
					foreach(get_sub_field('package_details') as $call){ 
						if($call['discount_price'] != $second_therapist->amount){
			...
			endwhile; */	
			$idarr = explode(",",$ids);
			$mypost = get_user_meta($second_therapist->tid,'post_id',true);
			if(!in_array($mypost,$idarr)){
				$color = 'background-color:yellow;';
			}
			//class='blink_me'
			$html .= "<tr style= '".$color."' class='".$second_therapist->payment_txnid ."'>";
			$html .= "<td>".$second_therapist->uname."<br/>(".$umobile.")<br/>(<b>".$usertype."</b>)(<b>".$totalcount."</b>)</td>";
			$html .= "<td>".$second_therapist->uemail."</td>";
			//$html .= "<td>".$umobile."</td>";
			$html .= "<td>".$second_therapist->action."</td>";
			if($second_therapist->action == 'chat' || $second_therapist->action == 'appointment_chat' || $second_therapist->action == 'videocall'){
				if($second_therapist->connected != -1){
					if($second_therapist->tid_accept == 1 || $second_therapist->tid_accept == 6){
						$html .= "<td>Accepted</td>";
					}elseif($second_therapist->tid_accept == 0 || $second_therapist->tid_accept == 2){
						$html .= "<td>No Response</td>";
					}elseif($second_therapist->tid_accept == -5) {
						$html .= "<td>Time Expired</td>";
					}elseif($second_therapist->tid_accept < 0) {
						$html .= "<td>Rejected</td>";
					}
				} else {
					$html .= "<td>Inactivity By Therapist</td>";
				}		
				if($second_therapist->uid_accept == 1){
					$html .= "<td>Accepted</td>";
				}elseif($second_therapist->uid_accept == 0){
					$html .= "<td>No Response</td>";
				}elseif($second_therapist->uid_accept == -2 || $second_therapist->uid_accept == -3) {
					$html .= "<td>No Response By Therapist</td>";
				}elseif($second_therapist->uid_accept == -5) {
					$html .= "<td>Network Issue</td>";
				}
				
			} else if($second_therapist->action == 'call') {
				$seconds = (strtotime($second_therapist->busy_date) - strtotime($second_therapist->created_date))/60/60/24;
				if($second_therapist->call_id != ''){
					$callrow = $wpdb->get_row("SELECT * FROM knowlarity_after_call WHERE call_uuid LIKE '%".$second_therapist->call_id ."%' ");	
				
					if($callrow->agent_status == 'Connected'){	
						$html .= "<td>Connected</td>";
					} else {
						if(count($callrow) == 0){
							$html .= "<td>Inprocess</td>";		
						} else {
							$html .= "<td>Not Connected</td>";
						}
					}
					if($callrow->call_transfer_status == 'Connected'){
						$html .= "<td>Connected</td>"; 
					} else {
						if(count($callrow) == 0){
							$html .= "<td>Inprocess</td>";		
			...
			} else if($second_therapist->therapy_name == 'Appointment') {
				$html .= "<td>--</td>";
				$html .= "<td>--</td>";	
				
			}	
			$appointment_session_count = $wpdb->get_row("SELECT * FROM appointment_session_details WHERE payment_txnid='". $second_therapist->payment_txnid ."'  LIMIT 1");
			if($second_therapist->therapy_name != 'Appointment'){
				if($second_therapist->payment_txnid != ''){
					$html .= "<td>".$second_therapist->remaining_session ."</td>";
				} else {
					$html .= "<td>NA</td>";
				}
			} else {
				$html .= "<td>".$appointment_session_count->sessions_pending ."</td>";
			}		
			if($second_therapist->payment_txnid != ""){
				$first_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE payment_txnid = '".$second_therapist->payment_txnid ."'  LIMIT 1"); 
			} else {
				$first_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE id='". $second_therapist->id ."'  LIMIT 1"); 
			}
			
			
			//$html .= "<td>".$r->tname ."</td>";
			if($second_therapist->payment_txnid != ""){
				$second_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE payuid = '".$second_therapist->payuid ."'   ORDER BY id DESC  LIMIT 1");
				$tidmobile = get_user_meta($second_therapist->tid,'mobile',true);
				$html .= "<td>". $second_therapist->tname ."<br/>(".$tidmobile.")</td>";
					
			}
			//if($second_therapist->therapy_name != 'Appointment'){
				$changetpcnt = $wpdb->get_var("SELECT count(*) FROM online_consultation WHERE  payuid = '".$second_therapist->payuid ."'");  
					
				if(($changetpcnt-1) >= 1){
					$html .= "<td><a href='#TB_inline?width=3&inlineId=".$second_therapist->id ."'  class='thickbox' >".($changetpcnt-1) ."</a></td>"; 
				} else {	
					$html .= "<td>".($changetpcnt-1) ."</td>"; 
			...
			$therapy_name = explode("-",$second_therapist->therapy_name);
			$package = explode(" ",$second_therapist->package);
			$therapy_name = $category_name.'<br/>('.ucwords($therapy_name[0]).')'.'<br/>('.ucwords($package[0]).' Mins.)';
			/* $therapy_name .= ucwords(str_replace('-', ' ', $second_therapist->therapy_name)); */
			$html .= "<td>".$therapy_name."</td>";
			
			$html .= "<td>".$second_therapist->amount."</td>";
			/* if($second_therapist->coupon_code != '' && $second_therapist->coupon_code != '-' && $second_therapist->coupon_code != 'NA'){
				$html .= "(Original : ".$amount." )";
			} */
			$html .= "<td>".$second_therapist->coupon_code ."</td>";
			$html .= "<td>".$second_therapist->payuid ."</td>";
			$html .= "<td>".$qafilled ."</td>";
			if($second_therapist->therapy_name != 'Appointment'){
				$html .= "<td>". date("dS M Y",strtotime($second_therapist->created_date)) ."</td>";
			} else {
				$ocvideoquery = $wpdb->get_row("SELECT * FROM oc_video_call WHERE oc_id = '".$second_therapist->id ."'   ORDER BY id DESC  LIMIT 1");
				$html .= "<td>". date("dS M Y",strtotime($ocvideoquery->book_date)) .' '.date("h:i a",strtotime($ocvideoquery->book_time))."</td>";
				
			}		
			if($second_therapist->payment_date == ''){
				$second_therapist->payment_date = date("dS M Y");
				
			}	
			$html .= "<td>". date("dS M Y",strtotime($second_therapist->payment_date)) ."</td>";
			if($second_therapist->action == 'call' || $second_therapist->action == 'chat' || $second_therapist->action == 'videocall'){
				if($second_therapist->action == 'call'){
					if($second_therapist->call_id != ""){
						if(!empty($callrow)){
							 
							
							$secs = strtotime($callrow->call_duration)-strtotime("00:00:00");
							$call_end_time = date("H:i",strtotime($callrow->call_time)+$secs);
							$html .= "<td>". date("H:i",strtotime($callrow->call_time)) ." |".$call_end_time."</td>";
							//$html .= "<td>". date("i:s",strtotime($callrow->call_duration)) ."</td>";
							$html .= "<td ><div class='".$second_therapist->payment_txnid ."d'>". (($second_therapist->session_duration) - ($second_therapist->remaining_duration)) ."</div></td>";
						} else {
							$html .= "<td>--</td>";
							$html .= "<td>--</td>";
						}
						...
						$html .= "<td>--</td>";
						$html .= "<td><div class='".$second_therapist->payment_txnid ."d'>--</div></td>";
						
					}
				}elseif($second_therapist->action == 'chat'){
					...
					$k=0;
					if($second_therapist->chat_start_time !=''){
						
						$start_chat_time = date("H:i:s",strtotime($second_therapist->chat_start_time));
						if($second_therapist->chat_end_time != ''){
							$end_chat_time =  date("H:i:s",strtotime($second_therapist->chat_end_time));
						}	else {
							$end_chat_time = "--";
						...
						$html .= "<td>".$start_chat_time." |".$end_chat_time."</td>";
						$html .= "<td class='".$second_therapist->payment_txnid ."d'>".$duration."</td>";
					} else {
						$html .= "<td>--</td>";
						$html .= "<td>--</td>";
					}
				}elseif($second_therapist->action == 'videocall'){	
					$videorow = $wpdb->get_row("SELECT utime,ttime,book_time FROM oc_video_call WHERE  oc_id = '".$second_therapist->id ."' AND (utime !=0 AND ttime !=0)");	
					if($videorow){	
						...
						//$html .= "<td>".date("H:i:s",strtotime('+'. $vtime .' mins',strtotime($videorow->book_time)))."</td>";
							$html .= "<td>". $videorow->book_time ." | ".date("H:i:s",strtotime('+'. $vtime .' mins',strtotime($videorow->book_time)))."</td>";
					} else {
						$html .= "<td>--</td>";
						$html .= "<td>--</td>";
					}	
			...
			$html .= "</tr>";
		//}	
		$html1 = '<div id="'.$second_therapist->id .'" style="display:none;">';
	
		$therapistrows = $wpdb->get_results("SELECT * FROM online_consultation WHERE payuid = '". $second_therapist->payuid ."' ");
		$manuallyconn = array("0"=>'No',"1"=>'Yes',"4"=>'Call Back');
		if($therapistrows){
			 
		$extra_session = 0;$total_duration = 0;$k=0;
			foreach($therapistrows  as $tr){
				$color = '';	
				$ocid = $tr->id;
				if($k == 0){
					$coupon_code = '';
					if($tr->coupon_code != ''){
						$coupon_code = '<br/>Coupon Code : '.$tr->coupon_code;
					}	
					$html1 .= '<h2>Session Duration : '. $tr->session_duration .' Mins. '.$coupon_code .'</h2><p><table border="1px">
					<tr >
						<th>Therapist Name</th>
						<th>Therapy Name</th>';
					if($tr->therapy_name != 'Appointment'){	
					$html1 .=	'<th>Duration</th>
						<th>Therapist Response</th>
						<th>Remaining Session</th>
						
						<th>Menu</th>
						<th>Action</th>';
					}	
					$html1 .= '<th>Session Date</th>';
					if($tr->therapy_name != 'Appointment'){
						$html1 .=	'<th>Manually Connected</th>
						<th>Comment</th>
						<th>Error Code</th>';
					}	
					$html1 .= '</tr>';
					$session_duration = $tr->session_duration;
					$k++;
				}
				if($tr->action == 'call'){
					if($tr->call_id != ''){
						$calldata = $wpdb->get_row("SELECT * FROM knowlarity_after_call WHERE call_uuid LIKE '%". $tr->call_id ."%'");
						...
						if(!empty($calldata)){
							...
							//if($cduration[1] != '00'){
								//$callpercentage = ($cduration[1]/$call_timer) * 100;
								$total_duration += $cduration[1].'.'.$cduration[2];
							//} else {
								//$callpercentage = 0;
								//$total_duration += $cduration[1].'.'.$cduration[2];
							//}
					...
					if(($cduration[1] >= (($session_duration) + 1))){
						$extra_session++;
					}	
					$menu = '--';
				}elseif($tr->action == 'chat'){
					$calldata = $wpdb->get_row("SELECT * FROM chat_call_conf WHERE oc_id = '". $tr->id ."'");
					if($tr->chat_end_time != ''){
						$call_duration = (strtotime($tr->chat_end_time) - strtotime($tr->chat_start_time))/60;
						$call_duration = round($call_duration,0); 
						$extra_session++;
				...
				if($extra_session > 1){
					$tmsg = $second_therapist->payment_txnid;
					$color = 'background-color:#F9CEC5;';
					
				}
				
				if($tr->cron_update == 2){
					$response = "Therapist Busy";
					
				} else {
					if($tr->connected != -1){
						if($tr->tid_accept == 1 || $tr->tid_accept == 6){
							$response = "Accepted";
						}elseif($tr->tid_accept == 0){
							$response = "No Response";
						}elseif($tr->tid_accept == -5){
							$response = "Time Expired";
						}elseif($tr->tid_accept < 0 || $tr->tid_accept == 2 ) {
							$response = "Rejected";
						}
					} else {
						$response = "Inactivity By Therapist";
					}		
					if($tr->action == 'call')  {
						if($tr->call_id != ''){
							$response = $calldata->call_status; 
						} else {
							$response = 'Not Connected';
						}	
					} 	
				}	
				if($tr->def_warn == 4 && $tr->action == 'call'){
					$color = 'background-color:#bef69f;';
				...
				if(gettype(json_decode($tr->support_modification)) == 'object'){
					$support_modification = json_decode($tr->support_modification);
					$support_by = $support_modification->administrator;
					$mantxt = 'Extra Session Manually Given by '.$support_by;
					$comment = $support_modification->comment;
				} else {
					//echo 'sdsd'.$ocid;echo "<br/>";
					$support_by1 = json_decode($tr->support_modification);
					if($tr->support_modification != ""){
						$mantxt = $manuallyconn[$tr->def_warn];
						$comment = $tr->support_modification;
					} else {
						$mantxt = $manuallyconn[$tr->def_warn];
						$comment = 'NONE ';
					} 
					//$mantxt = $manuallyconn[$tr->def_warn];
						//$comment = 'NONE '.$tr->support_modification;
				}
				if($tr->therapy_name == 'Appointment'){
					$ocvideoquery = $wpdb->get_row("SELECT * FROM oc_video_call WHERE oc_id = '".$tr->id ."'   ORDER BY id DESC  LIMIT 1");
					$book_date =  date("dS M Y",strtotime($ocvideoquery->book_date)) .' '.date("h:i a",strtotime($ocvideoquery->book_time));
					$therapy_name = ucfirst(str_replace('-', ' ', $tr->category_name));
				} else {
					$book_date = date("dS M Y H:i:s",strtotime($tr->created_date));
					$therapy_name = ucfirst(str_replace('-', ' ', $tr->therapy_name));
				}		
				
				$html1 .= '<tr style= '.$color.'>
					<td>'. $tr->tname .'</td>
					<td>'. $therapy_name .' '.$second_therapist->payuid.'</td>';
				if($tr->therapy_name != 'Appointment'){	
					$html1 .=	'<td>'. $call_duration .'</td>
					<td>'. $response .'</td><td>'. $tr->remaining_session .'</td><td>'. $menu .'</td>
					<td>'. $tr->action .'</td>';
				}	
				$html1 .=	'<td>'. $book_date .'</td>';
				if($tr->therapy_name != 'Appointment'){	
					$html1 .=	'<td>'. $mantxt .'</td>
					<td>'. $tr->comment.'</td>
					<td>'. $calldata->drop_id.'</td>';
				}	
				$html1 .= '</tr>';
			}	
			$html1 .= '</table></p>';
		}
		$html .= '<script type="text/javascript">console.log('.$total_duration .');$(".'.$tr->payment_txnid .'d").html('.$total_duration.');';
		if($total_duration > (($session_duration)+3) && $tr->therapy_name != 'Appointment'){
			$html .= '$(".'.$tr->payment_txnid .'").css("background-color","#F9CEC5");$(".'.$tr->payment_txnid .'").addClass("blink_me");';
		}
		//$(".'.$tr->payment_txnid .'").addClass("blink_me");
		$html .= '</script>';
		echo $html1 .= '</div>';
		
	}
	//document.getElementsByClassName(cbdcc835c56a8b4fdc02d).innerHTML  = "'.$total_duration .'";
	$html .= "</table><div style='text-align: right;display: inline-flex;'>";
	if($paged > 1){
		$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=onlineconsultationdashboard&paged=1&sd=$sdval&ed=$edval&s=$sval&m=$mval&cp=$cpval&k=$catval&nu=$nuval'>First</a></div>";
	}
	if($paged <= 1){
		//$html .= "<div class='disabled' style='margin: 5px 10px;'><a href='#'>Prev</a></div>";
    } else {
    	$pageno = $paged-1;
    	$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=onlineconsultationdashboard&paged=$pageno&sd=$sdval&ed=$edval&s=$sval&m=$mval&cp=$cpval&k=$catval&nu=$nuval'>Prev</a></div>";
    }
    if($paged >= $total_pages){
		//$html .= "<div class='disabled' style='margin: 5px 10px;'><a href='#'>Next</a></div>";
    } else {
    	$pageno = $paged+1;
    	$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=onlineconsultationdashboard&paged=$pageno&sd=$sdval&ed=$edval&s=$sval&m=$mval&cp=$cpval&k=$catval&nu=$nuval'>Next</a></div>";
    }
	if($paged < $total_pages){
		$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=onlineconsultationdashboard&paged=$total_pages&sd=$sdval&ed=$edval&s=$sval&m=$mval&cp=$cpval&k=$catval&nu=$nuval'>Last</a></div></div>";
	}
	
	echo $html;
}
function getInprogressConsultation(){ 
	global $wpdb;
	
	$sql = "SELECT * FROM `online_consultation` WHERE 1 ORDER BY id DESC LIMIT 50";
	...
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td>User Name</td><td>User Email</td><td>User Mobile</td><td>Action</td><td>Therapist Name</td><td>Therapist Email</td><td>Therapist Mobile</td><td>Package</td><td>Therapy Name</td><td>Amount</td><td>Call/Chat Status</td><td>Payment Status</td><td>Busy Date</td><td>Created Date</td></tr></thead>";
	...
	foreach($result as $r){
		$post_id = $wpdb->get_row("SELECT ID FROM wp_posts WHERE post_status = 'publish' AND post_author = '".$r->tid ."'");
	//	echo 'tid'.$r->tid;	echo "<br/>";
		$status = getTherapistStatus($post_id->ID);
		$consultrow = getTherapistConsultStatus1($r->tid,$r->id);
		$chatrow =  getTherapistChatStatus($r->tid);
		//if($status == "Available"){ 
			if($consultrow > 0){
				$cstatus = "On Call";
			} else {
		...
		if($consultrow > 0 || $chatrow > 0){ 
			...
			/* if($r->action == 'call'){
				...
				if($r->call_id != ""){
					$callrow = $wpdb->get_row("SELECT call_duration FROM knowlarity_after_call WHERE call_uuid = '".$r->call_id ."'");
					if(!empty($callrow)){
						$html .= "<td>Connected</td>";
			...
			if($r->action == 'videocall'){
				$videorow = $wpdb->get_var("SELECT COUNT(*) FROM oc_video_call WHERE  oc_id = '".$r->id ."'  AND (utime !=0 AND ttime !=0)");	
				if($videorow > 0){
					$html .= "<td>Connected</td>";	
				} else {
					$html .= "<td>Not Connected</td>";	
...
function csv_export_free_data() {
    ...
    $data_rows = array();
    $result = $wpdb->get_results("SELECT q.*  FROM `QA_feedback` q WHERE  1 ORDER BY q.id DESC",OBJECT);
    
		$html .= "<tr>";
    foreach ( $result as $r ) {
		$faqdata = json_decode($r->feedback);
		$call_duration = "";
		$callrow = $wpdb->get_row("SELECT o.payment_date,o.session_duration,o.category_name,o.action,o.call_id,remaining_duration FROM `online_consultation` o WHERE  o.payuid  = '". $faqdata->payuid ."' ORDER BY id DESC LIMIT 1",OBJECT);
		if($callrow->action == 'call'){
			$call_duration = ($callrow->session_duration - $callrow->remaining_duration);
		}
         $row = array(
        	$faqdata->name,
        	$faqdata->dob,
        	$faqdata->gender,
        	$callrow->category_name,
        	$callrow->action,
        	$callrow->session_duration,
        	$call_duration,
        	$faqdata->issue_catg2,
        	$faqdata->issue_catg3,
        	$faqdata->issue_catg4,
        	$faqdata->issue_catg5,
        	$faqdata->payuid,
        	date("Y-m-d",strtotime($callrow->payment_date))
        );
        $data_rows[] = $row;
    }
    $fh = @fopen( 'php://output', 'w' );
...
function csv_export_oc_data() {
	...
	$sql = "SELECT 	o.modified_date,o.id,o.uname,o.tname,o.uemail,o.temail,o.remaining_session,o.action,o.package,o.payment_txnid,o.payuid,o.chat_start_time,o.chat_end_time,o.payment_date,o.created_date,o.session_duration,o.therapy_name,o.amount,o.category_name,o.coupon_code FROM online_consultation o WHERE ";
	if($mval != ""){ 
		$sql .= "  o.payment_status = '". $mval."'";
	}   else {
		$sql .= "  o.payment_status = 'success'";
		
	} 
	if($sdval != "" && $edval != ""){
		$sql .= " AND DATE(o.created_date) BETWEEN '".$sdval."' AND '".$edval."'";
		
	}
	if($sval != "" ){
		$sql .= " AND (o.uname LIKE '%".$sval."%' OR o.uemail LIKE '%".$sval."%' OR o.package LIKE '%".$sval."%' OR o.therapy_name LIKE '%".$sval."%' OR o.amount LIKE '%".$sval."%' OR o.payuid LIKE '%".$sval."%')";
	}
	//$sql .= " AND o.id > b.id";	
	$sql .= " GROUP BY  payment_txnid   ORDER BY id DESC";
	...
	$paytxarr = array();
    foreach ( $result as $r ) {
		$totalcount = 0;$newflag = 0;
		//$pendingcnt = 	$wpdb->get_row("SELECT uid FROM online_consultation WHERE payuid = '".$r->payuid."' AND id > '".$r->id."' LIMIT 1");
		
		//if(!in_array($r->payment_txnid,$paytxarr)){
			/* $chkuser = $wpdb->get_row("SELECT session_taken FROM user_session_taken WHERE uid = '".$r->uid."'");
			if($chkuser->session_taken == 1){
				$usertype = "New User";
				$totalcount = 1;
			...
			/* if($r->category_name == ""){
				$r->category_name = 'silver-readers';
			}	
			 */
			$yellowcolor = '';
			if(date("m",strtotime($r->created_date)) != date("m",strtotime($r->payment_date))){
				$session ='Old Session';
			...
			$remaining = $r->remaining_session;	
			$modified_date = $r->modified_date;
			$pendingcnt = 0;
			$extra = 'No';	
			if($remaining == 0){
				$pendingcnt = 	$wpdb->get_row("SELECT modified_date,temail,tname,created_date,remaining_session FROM online_consultation WHERE payuid = '".$r->payuid."' ORDER BY id DESC LIMIT 1");
				if(strtotime(date("Y-m-d",strtotime($pendingcnt->created_date))) > strtotime(date("Y-m-d",strtotime($edval)))){
					$remaining = 1;
					$extra = 'Yes';
					
				} else {
					$remaining = $pendingcnt->remaining_session;
					$extra = 'No';
					
				}			
				$temail = $r->temail;
				$tname = $r->tname;	
				$modified_date = $pendingcnt->modified_date;
			}	
			$duration = 0;
			if($r->action == "call"){
		
				$duration = (($r->session_duration) - ($r->remaining_duration));
				//$html .= "<td>". date("i:s",strtotime($callrow->call_duration)) ."</td>";
			} 
			if($r->action == "chat"){
				$end_chat_time = $r->chat_end_time;
				$start_chat_time = $r->chat_start_time;
				if($r->chat_end_time != ''){
					$duration = (strtotime($end_chat_time) - strtotime($start_chat_time)) / 60;
			...
			if($remaining == 0){
				$temail = $pendingcnt->temail;
				$tname = $pendingcnt->tname; 
				/* if($duration > 0){
					$extra = 'Yes';
				} else {
					$extra = 'No';	
			...
			$orgamt = $r->amount;
			if($r->category_name == 'silver-readers'){
				if($r->package == '3 Mins Deep Consult' || $r->package == '3 Mins Quick Consult' || $r->package == '3 Mins Instant Consult'){
					$orgamt = 99;
				}
				if($r->package == '10 Mins Deep Consult' || $r->package == '10 Mins Quick Consult' || $r->package == '10 Mins Instant Consult'){
					$orgamt = 199;
				}
				if($r->package == '20 Mins Deep Consult' || $r->package == '20 Mins Quick Consult' || $r->package == '20 Mins Instant Consult'){
					$orgamt = 399;
				}	
				if($r->package == '30 Mins Quick Consult' || $r->package == '30 Mins Instant Consult' || $r->package == '30 Mins Deep Consult'){
					$orgamt = 599;
				}
			}	
			if($r->category_name == 'gold-readers'){
				if($r->package == '3 Mins Deep Consult' || $r->package == '3 Mins Quick Consult' || $r->package == '3 Mins Instant Consult'){
					$orgamt = 149;
				}
				if($r->package == '10 Mins Deep Consult' || $r->package == '10 Mins Quick Consult' || $r->package == '10 Mins Instant Consult'){
...
function fsd_sendemail_callback() {
	...
	foreach (explode(',', $ids) as $id) {
		...
		if($email){
			$url = site_url()."/tarot-free-session";
...
add_action( 'delete_my_operator_assigned_users', 'cron_delete_my_operator_assigned_users_34496053', 10, 0 );
...
function apply_coupon_code(){ 
	...
	/* if(strtoupper($code) == 'THRIIVE20'){
		$code = 'THR20';
	}	 */
	$db_code = explode(",",str_replace(' ','',get_field('coupon_code','option')));
	if(in_array(strtoupper($code),$db_code)){
		//if(strtoupper($code) != 'THRIIVE20' && strtoupper($code) != 'THR10'){
			$couponcnt50 = $wpdb->get_var("SELECT count(*) FROM coupon_code_consultation,online_consultation o  WHERE coupon_code = 'THR50' AND uid = '".$coupon_user."' AND payment_status = 'success' AND c.oc_id = o.id"); 
			//$couponcnt01 = $wpdb->get_var("SELECT count(*) FROM coupon_code_consultation WHERE coupon_code = 'THR001' AND uid = '".$coupon_user."'");
			$couponcnt20 = $wpdb->get_var("SELECT count(*) FROM coupon_code_consultation c,online_consultation o  WHERE c.coupon_code = 'LTA20'  AND c.uid = '".$coupon_user."' AND payment_status = 'success' AND c.oc_id = o.id");
			$usercnt = $wpdb->get_var("SELECT count(*) FROM online_consultation WHERE uid = '".$coupon_user."' AND payment_status = 'success'"); 
	
			if(($couponcnt50 == 0 && $usercnt == 0 && strtoupper($code) == "THR50")  || ($couponcnt20 == 0 && strtoupper($code) == "LTA20")){
				$data['resStatus'] = 'success';
				$data['success'] = 'Coupon code applied.';
				$data['code'] = base64_encode($_POST['code']);
			}elseif((strtoupper($code) == "THR50" || $couponcnt50 > 0)){
				$data['resStatus'] = 'error';
				$data['resMessage'] = 'Coupon code only for first time users.';
				$data['code'] = base64_encode($_POST['code']);
			}elseif((strtoupper($code) == "THR50" && $usercnt > 0)){
				$data['resStatus'] = 'error';
				$data['resMessage'] = 'Sorry this code is Not Available';
				$data['code'] = base64_encode($_POST['code']);
			} else {
				$data['resStatus'] = 'error';
				$data['resMessage'] = 'Coupon code already used.';
				$data['code'] = base64_encode($_POST['code']);
			}	
		/* } else {
			$data['resStatus'] = 'success';
		...
		//echo generateJSON('error','Invalid coupon code.');
		$data['resStatus'] = 'error';
		$data['resMessage'] = "Invalid coupon code.";
		$data['code'] = base64_encode($_POST['code']);
		//echo generateJSON('success','Coupon code applied.');
		echo json_encode($data);
	}
...
function my_acf_set_repeater( $value, $post_id, $field ){
    ...
    foreach($value as $v){
    	$enabledbtn = 0;
    	if($v['field_5f311d2014a89'] == 1){
			$enabledbtn += 1;
		}
		if($v['field_5f311d2f14a8a'] == 1){
			$enabledbtn += 1;
		}
		if($v['field_5f311d3d14a8b'] == 1){
			$enabledbtn += 1;
		}
		if($v['field_5f311d6314a8c'] == 1){
			$enabledbtn += 1;
		}
		if($enabledbtn == 1){
			$value[$i]['field_5f56461fc1cf5'] = 'col-md-6';
		}
		if($enabledbtn == 2){
			$value[$i]['field_5f56461fc1cf5'] = 'col-md-6';
		}
		if($enabledbtn == 3){
			$value[$i]['field_5f56461fc1cf5'] = 'col-md-4';
		}
		$i++;
    }
    return $value;
...
add_filter('acf/load_value/name=button_enabled', 'my_acf_set_repeater', 10, 3);

add_filter('acf/load_field/key=field_5f56461fc1cf5', 'acf_read_only');
...
function liveeventdetails($event_position,$user_id,$user_email,$user_mobile){
	...
	$i = 0; 
	while(have_rows('custom_shreans_events',32466)) : the_row();
		if($i == $event_position){
...
function otp_login(){
	...
	if(!empty($mobileNumber) && !empty($isMobileExist)) {
		...
		//$msg = $generatedPin . ' is the one time password (OTP) for your online registration on Thriive.in. This is usable once &amp valid for 15 min';
		$msg = $generatedPin . ' is the one time password (OTP) for your online login on Thriive.in';  
		$tempid="1007164686093486285";		
		$result = sendSMS($countrycode . $mobileNumber,$msg ,$tempid);
		$mobileNumber = get_user_meta($user_id,'mobile',true);
...
function saveFreeOCData($uid,$firstname,$email,$tid,$tfirst_name,$temail,$action,$productinfo,$amount,$freemins,$sendemail,$book_date_time,$tpid,$umobile,$tmobile){
	...
	if($isexist == 0){
		...
		if($action == 'call'){
			...
			$tmsg = "Your client ".$uname." has taken free session and is waiting for a session with you. Please call on the ".$result['masked_number']." to connect with client within the next 2 minutes on thriive.in";
			sendSMS($tmobile,$tmsg,$tempid);
