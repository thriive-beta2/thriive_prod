...
<div class="container">
	...
	<?php 
	$sessiondata = $wpdb->get_row("SELECT action,id,remaining_session,session_duration,category_name,call_id,waiting  FROM online_consultation WHERE uid = '". $current_user->ID ."' AND category_name='".$cat_name."'  AND payment_status='success' AND remaining_session > 0");
	...
	//if($sessiondata->remaining_session > 0){
		$remaining_flag = 1;
		//echo 'remaining_flag'.$remaining_flag;
		if($sessiondata->category_name == ''){
			$sessiondata->category_name = 'silver-readers';
		}
	    if($_GET['t'] == 'tarot-card-reading' && $sessiondata->remaining_session > 0){
		    if($sessiondata->category_name != $cat_name){ 
				$remaining_flag = 0; 
			}	 
			if($sessiondata->action == 'call'){ 
				
				/* if($sessiondata->call_id != ''){
					
					$knowcall = $wpdb->get_row("SELECT * FROM knowlarity_after_call WHERE call_uuid LIKE '%".$sessiondata->call_id ."%'");
					//print_r($sessiondata->call_id);
					if(count($knowcall) == 0 AND $sessiondata->waiting != 2){
						$remaining_flag = 0; 	
					} else {
						$remaining_flag = 1;
					}	
		...
		<script>
			$('#remainingdiv').append("You have an Existing Session of <?php echo $sessiondata->session_duration;?> min.<br/>Press Continue to check if Therapist is Available & Start your Session Now.<br/><br/> ");
			$('#remainingdiv').append('<div class="text-center"><button type="button" class="contbtn"><a href="<?php echo site_url();?>/session-thankyou-page?q=<?php echo $_GET["q"];?>&a=<?php echo $_GET["a"];?>&t=<?php echo $_GET["t"]; ?>&c=<?php echo $_GET["c"]; ?>"><span class="clicktxt">Continue</span></a></button><br/></div>');
		</script>
		...
		<div class="col-md-12 col-xs-12">
			...
			if( have_rows('package_details','option') ):
				...
				while ( have_rows('package_details','option') ) : the_row();
						...
						foreach(get_sub_field('call_packages') as $call){ 
							if($call['discount_price']){
								...
								/* if($no_of_sessions[0] > 1){
									...
									$posted['udf2'] = $posted['amount'].','.$call['no_of_sessions'].','.$call['description'].','.$amountarr[$k].','.$call['session_duration'].','.$action.',NA,'.$cat_name.','.$session_count;
								//}
					...
					//$sessiondata = $wpdb->get_row("SELECT COALESCE(sum(remaining_session), 0) as remaining_session  FROM online_consultation WHERE uid = '". $current_user->ID ."'  AND therapy_name = '".$_GET['t']."' AND payment_status='success' AND remaining_session > 0 limit 1");
					$m++;
					//if($remaining_flag == 0){ ?>
						   
						<div class="col-md-3 col-xs-3 packmobile <?php  if($call['is_popular']){ echo "mostpb";}?>">
							<button type='submit' onclick='whatsapp_payment_reminder("<?php echo $_GET['q'];?>","<?php echo $_GET['t']; ?>","<?php echo $_GET['c'];?>","<?php echo $current_user->ID;?>")'  data-therapist='$slug' data-plan_title='$plan_title' data-no_of_sessions='$no_of_sessions' data-plan_cost='$plan_cost' data-action='$action' data-therapy='$therapy' id='paid_payment_button' >
            ...
            <section class="widgetblock">
				...
				if( have_rows('coupon_prices','option') ):
                    ...
                    while ( have_rows('coupon_prices','option') ) : the_row();
                        ...
                        foreach(get_sub_field('therapy') as $t){
                            array_push($temparr,get_sub_field('custom_therapy'));
                            array_push($temparr,get_sub_field('user_coupon_code'));
                            array_push($temparr, $t->slug);
                        }
                        if($k == 0){ ?>
                            ...
                            <form method="post" style="display: none;" id="coupon_form" class="text-center">
								...
								<input type="hidden" id="cat_name" value="<?php echo $cat_name; ?>" />
                                <input type="text" id="coupon_code" oninput="this.value = this.value.toUpperCase()" autocomplete="off" class="input_class" />
								<input type="hidden" id="coupon_user" value="<?php echo $current_user->ID; ?>" />
