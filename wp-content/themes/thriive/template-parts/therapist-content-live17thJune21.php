      ...
      while ( have_rows('button_enabled','option') ) : the_row();
        ...
        if(in_array(get_query_var("therapy")[0], $temparr)){ ?> 
		  ...
		  if(get_query_var('payment_type')){
				...
				?>
					 <a class=" <?php if($status != "Busy" && $status != "On Call" && $cstatus != "On Chat"){ echo ' consult_online_link_oc btnCallnow'; } else { echo "busybtn";} ?>" id="consult_online_<?php echo get_the_id(); ?>" data-slug="<?php echo $post->post_name; ?>" data-waiting="<?php echo $sessiondata->waiting; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>"  style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo 'border: 1px solid #27265f ;background-color:#fff;color:#eb0029;';} else { echo "color: #fff;";}?>"><i class="fa fa-phone <?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "busybtn"; }?>" aria-hidden="true"  style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "color: red;";} else { echo "color: #fff;";}?>"></i>Call Now</a>
				?>
           ...
           <?php  
			...
			?>
					<a class=" <?php if($status != "Busy" && $status != "On Call" && $cstatus != "On Chat"){ echo ' consult_online_link_oc btnCallnow'; } else { echo "busybtn";} ?>" id="consult_online_<?php echo get_the_id(); ?>" data-slug="<?php echo $post->post_name; ?>" data-waiting="<?php echo $sessiondata->waiting; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" data-remaining="0" style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo 'border: 1px solid #27265f ;background-color:#fff;color:#eb0029;';} else { echo "color: #fff;";}?>"><i class="fa fa-phone <?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "busybtn"; }?>" aria-hidden="true"  style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo "color: red;";} else { echo "color: #fff;";}?>"></i>Call Now </a>
			<?php
				//if($inhouse == 1){
      ...
      <input type="hidden" id="chat_event" value="<?php echo $event; ?>">
        ...
        <?php }?>
            ...
            <?php }else if(get_sub_field('video_call_button') == 1){?>
              <a class="btnChatnow <?php if($status != "Busy" && $status != "On Call" && $cstatus != "On Chat") { echo " book_now_link_oc";} ?> " id="book_now_<?php echo get_the_id(); ?>" data-action="videocall" data-slug="<?php echo $post->post_name; ?>" data-from_status="<?php echo $from_status; ?>" data-therapy="<?php echo get_query_var("therapy")[0]; ?>" style="<?php if($status == "Busy" || $status == "On Call" || $cstatus == "On Chat") { echo 'background-color: #80808075 !important;color:#000'; } ?>"><i class="fa fa-calendar" aria-hidden="true"></i>Video Call</a>
            <?php }										  
          }
        }
      endwhile; 
