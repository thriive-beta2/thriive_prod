...
$(".start_chat_oc").unbind('click').click(function(e){
    ...
    if (from_status == 0) {
        //location.href = '/seeker-regsiter-landing-page';
        gtag('event', 'click', {
            event_category: 'button',
    ...
    window.location.replace(window.location.protocol + "//" + window.location.host + "/payment-details?q="+slug+"&a=chat&t="+therapy);
});
...
$(".video_call_now_link_oc").unbind('click').click(function(e){
	...
	window.location.replace(window.location.protocol + "//" + window.location.host + "/booking-date-and-time?q="+slug+"&a=video_call&t="+therapy); 
});
...
$(".book_now_link_oc").unbind('click').click(function(e){
	...
	window.location.replace(window.location.protocol + "//" + window.location.host + "/booking-date-and-time?q="+slug+"&a=book_later&t="+therapy); 
});
...
$(function () {
	...
	*/
	...
	// delete ajax_upload image (blueimp)
	$(".imagePreview1").on('click', '.close', function () {
		//alert('fgf');
		//console.log("Hello");
		var $this = $(this);
		var row_index = $this.attr('data-row_id');
		$this.closest(".imagePreview1").addClass('rfa_loader');
		...
		jQuery.ajax({
			...
			success: function (data) {
				//console.log(data);
				$this.closest(".imagePreview1").removeClass('rfa_loader').html(data);
				//$this.closest(".imagePreview");
			},
			complete: function (data) {},
			error: function (err) {
				$this.closest(".imagePreview1").removeClass('rfa_loader');
			}
		});
	});
	$('input[type=file]').change(function () {
		...
		var maximage = $(this).data('maximage');
		//alert('gfhg'+(parseInt(files.length)+parseInt(currentimage)));
		//alert('etretr'+(parseInt(maximage)-parseInt(currentimage)));
		/* if((parseInt(files.length)+parseInt(currentimage)) > 200){
			alert("Max Images Allowed "+maximage);
			return;
		}	
		 */
			...
			for (var i = 0; i < files.length; i++) {
				...
				reader.onloadend = function () { // set image data as background of div
					...
					if (files[0].size <= 20971520) {
						...
						if (files.length == 1) {
							$this.closest(".form-group").find(".imagePreview1").html("");
						}
						$this.closest(".form-group").find(".imagePreview1").append('<img src="' + this.result + '" />').css({
							"display": "block"
						});
...
$('#event_call_seeker_signup_btn').on('submit', function (e) {
	e.preventDefault();
	e.stopPropagation();
	if ($(this).parsley().validate()) {
	//if ($(this).parsley('isValid')) {
		...
		jQuery.ajax({
	       	url: ajax_object.ajax_url,
	       	type: 'POST',
	       	data: {'action': 'event_call_seeker_signup', 'mobile_number': mobileNumber, 'country_code': country_code, 'email': email, 'lead_source': lead_source},
	       	success: function (data) {
	       		try {
			        var jsonObj = JSON.parse(data);
			        if(jsonObj.success == 'error'){
				        $("#error_msg").text(jsonObj.resMessage);
				      	...
				      	gtag('event', 'click', {
							event_category: 'button',
							event_label: 'live event call signup started',
						});
						var str = jsonObj.resData;
						var res = str.split("-");
					      $("#error_msg").text("");
						  ...
						  $("#mobile_number").val(res[2]);
						  $("#country_code").val(res[1]);
						  $("#event_name").val("live event call signup completed");
						  $("#event_reg_hide").css("display", "none");
						  $("#event_reg_unhide").css("display", "block");
				       	} else {
					     	$("#error_msg").text(jsonObj.resMessage);  
				       	}
					}
...
$('#seeker_submit_sd').on('submit', function (e) {
	e.preventDefault();
	e.stopPropagation();
	...
	if ($(this).parsley('isValid')) {
		...
		var email = $('#email').val();
		var pwd = $('#pwd').val(); 
		var lead_form_id = $('#lead_form_id').val();
		jQuery.ajax({
	       	url: ajax_object.ajax_url,
	       	type: 'POST',
	       	data: {'action': 'seeker_signup_sd', 'mobile_number': mobileNumber, 'country_code': country_code, 'fullname': fullname, 'email': email, 'pwd': pwd, 'lead_form_id': lead_form_id},
	       	success: function (data) {
	       		try {
			        var jsonObj = JSON.parse(data);
			        if(jsonObj.success == 'error'){
			        ...
			        if(jsonObj.resStatus == 'error'){
				        $("#error_msg_otp").text(jsonObj.resMessage);
						  ...
						  if(payment == 1 && (action == 'video_call' || action == 'book_later')){
						  	window.location.replace(window.location.protocol + "//" + window.location.host + "/booking-date-and-time?q="+slug+"&a="+action+"&t="+therapy); 
...
$('#verify_mobile_event').on('click', function (e) {
	e.preventDefault();
	e.stopPropagation();
	if ($(this).parsley().validate()) {
		var event_name = $("#event_name").val();
		...
		jQuery.ajax({
	       	url: ajax_object.ajax_url,
	       	type: 'POST',
	       	data: {'action': 'event_verify_user', 'otp': otp, 'user_id': user_id, 'event_position': event_position},
	       	success: function (data) {
	       		try {
			        var jsonObj = JSON.parse(data);
			        if(jsonObj.resStatus == 'error'){
