 ...
 */
 ...
 //SMS gateway url
define("SMS_URL","http://sms6.rmlconnect.net:8080/bulksms/bulksms?"."username=THRIIVEGLOBAL&password=thriive0&type=0&dlr=1&source=THRIIV&entityid=1001373488736615240");

//pay u details

if($_SERVER['SERVER_NAME'] == 'localhost' || $_SERVER['SERVER_NAME'] == 'beta1.thriive.in' || $_SERVER['SERVER_NAME'] == 'thriive-staging.noesis.tech'){
    //For Dev & Staging
    define('MERCHANT_KEY','icapBa');
    define('SALT','Gx9R1iP6');
    ...
    define('SALT','03C5uxbtoeAaw7nDIOvugQATJRTie1xB');
...
define('OC_PAYU_RETURN_URL', site_url() . '/oc-thank-you');
define('FAILURE_PAYU_RETURN_URL', site_url().'/therapy-payment-page' );
    
$file_includes = array(
    'framework/actions.php',
    'framework/assets.php',
    'framework/core-functions.php',
	'framework/businessmodel-functions.php',
    //'framework/cron.php',
    'framework/init.php',
...
function fetch_user_chat_history(){
	...
	if($role == 'subscriber'){
    	$query = " SELECT * FROM chat_message_details  WHERE ((from_user_id = ".$from_user_id."  AND to_user_id = ".$to_user_id.") OR (from_user_id = ".$to_user_id." AND to_user_id = ".$from_user_id.")) AND (delete_status = 0) ORDER BY chat_message_id DESC ";
    } else {
    	$query = " SELECT * FROM chat_message_details  WHERE ((from_user_id = ".$from_user_id."  AND to_user_id = ".$to_user_id.") OR (from_user_id = ".$to_user_id." AND to_user_id = ".$from_user_id.")) AND (delete_status = 0) AND (block_status = 0) ORDER BY chat_message_id DESC ";
    }
		...
		$output = '<ul class="list-unstyled">';
 		foreach($result as $row){
			...
			if($row->from_user_id == $from_user_id){
				...
				$output .= '<div class="chat_content"><div class= "chat_text">'.$image_holder.'<div class="chat_time">'.format_date1($row->chat_time).'</div></div><div class="chat_details"><h4>'.$user_name.'</h4></div></div></li>';
			} else {
...
function custom_meta(){
	if(is_page(71106)){
		echo "<meta name='title' content='Covid 19 Counselor Online - Book Appointment & Consult'><meta name='description' content='Free COVID mental health counseling with Thriive is a FREE 20-minute session, with verified and trained mental health counselors'>";
	}

}
/*
add_action('wp_head', 'custom_meta');
add_action("wp_ajax_is_user_blocked","check_is_blocked");
add_action("wp_ajax_nopriv_is_user_blocked","check_is_blocked");
function check_is_blocked(){
	...
	global $wpdb;
	$output = "";
	$sql = "SELECT block_status FROM chat_block_users WHERE to_user_id = $to_user_id AND from_user_id = $from_user_id AND block_status = 1";
	$is_blocked = $wpdb->get_var($sql);
	if($is_blocked == '1'){
		$output .= "<p class='block_msg'>Can't send a message to blocked user $to_user_name</p>";
	}
	echo $output;
	wp_die();
}
...
function fetch_user_chat_history_last(){
	...
	if($role == 'subscriber'){
		...
		$result = $wpdb->get_results($query);
		//$msgquery = " SELECT * FROM chat_message_details  WHERE ((from_user_id = '".$from_user_id."'  AND to_user_id = '".$to_user_id."')  OR (from_user_id = '".$to_user_id."'  AND to_user_id = '".$from_user_id."')) and (delete_status = 0)";	
		$msgquery = " SELECT * FROM chat_message_details  WHERE ((from_user_id = '".$from_user_id."'  AND to_user_id = '".$to_user_id."')) and (delete_status = 0)";	
		$result1 = $wpdb->get_results($msgquery);
	} else {
		$query = " SELECT * FROM chat_message_details  WHERE ((from_user_id = '".$from_user_id."'  AND to_user_id = '".$to_user_id."')  OR (from_user_id = '".$to_user_id."'  AND to_user_id = '".$from_user_id."')) AND (terepist_status = 0) AND (block_status = 0) ORDER BY chat_time DESC limit 1 ";
		$result = $wpdb->get_results($query);
	}	
	//print_r($result);
	$output = '';
	...
	if(count($result) > 0){
		...
		$output .= '</ul>';
		if(count($result1)== 1){
			$welcomemsg = 'You will receive an sms once your expert is online and ready to chat.';
			$welcomemsg1 = 'Incase you did not receive a message shortly please call or message the customer team on +918369515594.';
			$output .= '<div class="chat_details"><h4 class="chat-infotopTxt">'.$welcomemsg.'</br></br>'.$welcomemsg1.'</br></h4></div>';
		}
		echo $output;
...
function check_box_open(){
	...
	if($role == 'subscriber'){
 		$query = "SELECT * FROM chat_message_details  WHERE to_user_id = '".$from_user_id."' AND user_status = 0 AND block_status = 0 ORDER BY chat_time DESC limit 1 ";
	} else {
		$query = "SELECT * FROM chat_message_details  WHERE to_user_id = '".$from_user_id."' AND terepist_status = 0 AND block_status = 0 ORDER BY chat_time DESC limit 1 ";
	}
	$result = $wpdb->get_results($query);
	if(count($result) > 0){
 		foreach($result as $row){
...
function insert_chat(){
	...
	global $wpdb;
	$is_blocked = $wpdb->get_var("SELECT block_status FROM chat_block_users WHERE ((to_user_id = $to_user_id AND from_user_id = $from_user_id) OR (to_user_id = $from_user_id AND from_user_id = $to_user_id)) AND block_status = 1");
	if($is_blocked == 1){
		$args['block_status'] = 1;
	}
	$wpdb->insert("chat_message_details", $args);
	if($is_blocked != 1){
		$current_user = wp_get_current_user();
		$role =  $current_user->roles[1]; 
		if($role == '')
		...
		$SMS_URL = 'http://sms6.rmlconnect.net:8080/bulksms/bulksms?username=THRIIVEGLOBAL&password=y371H2Hv&type=0&dlr=1&source=THRIIV&entityid=1001373488736615240&tempid=&';
		if($role == 'subscriber'){
	 		$sql_str = "SELECT * FROM chat_message_details  WHERE from_user_id = '".$from_user_id."' or to_user_id = '".$to_user_id."' AND block_status = 0 ORDER BY chat_time ASC limit 0,2";
			...
			$sql_str = "SELECT * FROM chat_message_details  WHERE from_user_id = '".$from_user_id."' or to_user_id = '".$to_user_id."' AND block_status = 0 ORDER BY chat_time DESC  limit 0,2";
			$result3 = $wpdb->get_results($sql_str);
			if(count($result3) > 0){
				$chat_time = date('Y-m-d H:i:s',strtotime($results3[1]->chat_time));
				$curr_time = date('Y-m-d H:i:s');
...
add_action('init', function(){
	return add_rewrite_rule(
		'daily-horoscope/(.*)/?$',
		'index.php?pagename=daily-horoscope&q=$matches[1]',
		'top'
	);
});
*/
...
add_action('init', function(){
	return add_rewrite_rule(
		'weekly-horoscope/(.*)/?$',
		'index.php?pagename=weekly-horoscope&q=$matches[1]',
		'top'
	);
});
...
add_action('init', function(){
	return add_rewrite_rule(
		'monthly-horoscope/(.*)/?$',
		'index.php?pagename=monthly-horoscope&q=$matches[1]',
		'top'
	);
});
...
add_filter('query_vars', function($v){return array_merge(array('q'),$v);});
...
add_action( "wp_ajax_block_user_by_therapist", "block_user_by_therapist" );
add_action( "wp_ajax_nopriv_block_user_by_therapist", "block_user_by_therapist" );
function block_user_by_therapist(){
	global $wpdb;
	$from_user = $_POST['from_user'];
	$to_user = $_POST['to_user'];
	$from_user_name = get_user_name($from_user);
	$to_user_name = get_user_name($to_user);
	$user_block_details = array(
		'status' => 'block',
		'date_time' => date("Y/m/d H:i:s"),
	);
	$user_data = array(
	    ...
	    'block_status' => 1, 
	    'block_details' => serialize($user_block_details),
	);
	$results = $wpdb->get_results( "SELECT * FROM chat_block_users WHERE to_user_id = $to_user AND from_user_id = $from_user", OBJECT );
	if($results){
		$tempArr = array();
		foreach($results as $result){
			$block_details = unserialize($result->block_details);
			$tempArr = $block_details;
			array_push($tempArr,$user_block_details);
			$wpdb->update('chat_block_users', array('block_details' => ""), array('id' => $result->id));
			$user_data['block_details'] = serialize($tempArr);
			$wpdb->update('chat_block_users', $user_data, array('id' => $result->id));
		} 
	} else {
		$user_data['block_details'] = serialize(array($user_block_details));
		$wpdb->insert('chat_block_users', $user_data);
	}
	sendEmail('manishj@thriive.in','User has been blocked',"Therapist $from_user_name blocked $to_user_name");
	wp_die(); // ajax call must die to avoid trailing 0 in your response
}

add_action( "wp_ajax_unblock_user_by_therapist", "unblock_user_by_therapist" );
add_action( "wp_ajax_nopriv_unblock_user_by_therapist", "unblock_user_by_therapist" );
function unblock_user_by_therapist(){
	global $wpdb;
	$from_user = $_POST['from_user'];
	$to_user = $_POST['to_user'];
	$from_user_name = get_user_name($from_user);
	...
	sendEmail('manishj@thriive.in','User has been unblocked',"Therapist $from_user_name unblocked $to_user_name");
	wp_die(); // ajax call must die to avoid trailing 0 in your response
}
...
function fetch_user_subscriber(){
	...
	global $wpdb;
    //$query = " SELECT * FROM chat_message_details WHERE to_user_id = '".$session_id."' ORDER BY chat_time DESC limit 10 ";
	$query = "SELECT DISTINCT from_user_id, MAX(chat_time) as chat_time,MAX(to_user_id) as to_user_id FROM chat_message_details WHERE to_user_id=$session_id GROUP BY from_user_id ORDER BY MAX(chat_time) DESC, from_user_id LIMIT 10";
	$result = $wpdb->get_results($query);
	$loop_count = count($result);
	for($i=0;$i<$loop_count;$i++){
	$comp_to =  $result[$i]->to_user_id;
	$comp_from =  $result[$i]->from_user_id;
	$query = "SELECT complete_status FROM chat_block_users WHERE from_user_id=$comp_to and to_user_id=$comp_from OR from_user_id=$comp_from and to_user_id=$comp_to";
...
*/
/*
add_action( 'init' , 'fetch_user_therapist' );
function fetch_user_therapist(){
	$current_user = wp_get_current_user();
	$session_id = $current_user->ID;
	global $wpdb;
    //$query = " SELECT * FROM chat_message_details WHERE from_user_id = '".$session_id."' group by to_user_id  ORDER BY chat_time DESC limit 10 ";
	//$result = $wpdb->get_results($query);	
	$query = "SELECT DISTINCT to_user_id, MAX(chat_time) as chat_time,MAX(from_user_id) as from_user_id FROM chat_message_details WHERE from_user_id=$session_id GROUP BY to_user_id ORDER BY MAX(chat_time) DESC, to_user_id LIMIT 10";
	$result = $wpdb->get_results($query);
...
function therapist_chat_history(){
	...
	foreach($result as $row){
		...
		$output .= '<tr><td>'.$name.'</td><td>'.$last_coversation.'</td><td class="btns_group" id="start_chat_button_'.$to_user_id.'"><button type="button" class="btn btn-info btn_link1 view_chat btn btn-primary btn-big btn-transparent connect_with_btn_listing" data-fromuserid = "'.$session_id.'" data-touserid="'.$from_user_id.'" data-tousername="'.$name.'"  data-role="subscriber">View Chat</button><a href = "'.$anchor.'" target = "_blank" class="btn btn-info anch_link1 btn btn-primary btn-big btn-transparent connect_with_btn_listing" javascript= "void()">Export</a></td></tr>';
	}
	$output .= '</table>';
	return($output);
}
...
function therapist_chat_history_mobile(){
    ...
    $query = " SELECT from_user_id FROM chat_message_details  WHERE (to_user_id   = '".$session_id."' and delete_status = 0) group by from_user_id ";
	$result = $wpdb->get_results($query);
	$output = '<section class="mobi_tableview"><div class="container"><div class="row"><div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12"><table class="table">';
	foreach($result as $row){
	$from_user_id = $row->from_user_id;
	$therepist_data = get_userdata( $from_user_id);
	$t_name = $therepist_data->display_name;
	...
	$anchor = site_url()."/therapist-chat-history/?to_user=".$user_id;
	$last_coversation = last_login($user_id);
 	$output .= '<thead class="thead-light"><tr><th colspan="2">'.$name.'</th></tr></thead><tbody><tr><td>Last Conversation (Date&Time)</td><td>'.$last_coversation.'</td></tr><tr><td>Action</td><td class="btns_group" id="start_chat_button_"><button type="button" class="btn btn-info btn_link1 view_chat btn btn-primary btn-big btn-transparent connect_with_btn_listing" data-fromuserid = "'.$session_id.'" data-touserid="'.$from_user_id.'" data-tousername="'.$name.'"  data-role="subscriber">View Chat</button><a href = "'.$anchor.'" target = "_blank" class="btn btn-info anch_link1 btn btn-primary btn-big btn-transparent connect_with_btn_listing" javascript= "void()">Export</a></td></tr></tbody>';
	}
...
function user_chat_history(){
	...
	foreach($result as $row){
		...
		$output .= '<tbody><tr><td>'.$name.'</td><td>'.$termname[1].'</td><td>'.$termname[0].'</td><td>'.$last_coversation.'</td><td class="btns_group" id="start_chat_button_'.$to_user_id.'"><button type="button" class="btn btn-info btn_link1 view_chat btn btn-primary btn-big btn-transparent connect_with_btn_listing" data-fromuserid = "'.$session_id.'" data-touserid="'.$to_user_id.'" data-tousername="'.$name.'"  data-role="subscriber">View Chat</button><a href = "'.$anchor.'" target = "_blank" class="btn btn-info btn_link1 anch_link1 btn btn-primary btn-big btn-transparent connect_with_btn_listing" javascript= "void()">Export</a><button type="button" id = "del1" class="btn btn-info btn_link1 connect_with_btn_listing" data-to_user = "'.$user_id.'" data-from_user = "'.$session_id.'" onclick="delete_msggrp(this)">Delete</button></td></tr></tbody>';
	}
	$output .= '</table>';
	return($output);
}
...
function user_chat_history_mobile(){
	...
	foreach($result as $row){
		...
		$anchor = site_url()."/user-chat-history/?to_user=".$user_id;
		$last_coversation = last_login($user_id,$session_id);
		$output .= '<tbody><tr><th widht="20%">Therapist</th><td>'.$name.'</td></tr><tr><th widht="20%">Cure</th><td>'.$termname[1].'</td></tr><tr><th widht="20%">Specaility</th><td>'.$termname[0].'</td></tr><tr><th widht="20%">Last Conversation (Date&Time)</th><td>'.$last_coversation.'</td></tr><tr><th width="20%">Action</th><td class="btns_group" id="start_chat_button_'.$to_user_id.'"><button type="button" class="btn btn-info btn_link1 view_chat btn btn-primary btn-big btn-transparent connect_with_btn_listing" data-fromuserid = "'.$session_id.'" data-touserid="'.$to_user_id.'" data-tousername="'.$name.'"  data-role="subscriber">View Chat</button><a href = "'.$anchor.'" target = "_blank" class="btn btn-info anch_link1 btn btn-primary btn-big btn-transparent connect_with_btn_listing" javascript= "void()">Export</a><button type="button" id = "del1" class="btn btn-info btn_link1 connect_with_btn_listing" data-to_user = "'.$user_id.'" data-from_user = "'.$session_id.'" onclick="delete_msggrp(this)">Delete</button></td></tr></tbody>';
	}
	$output .= '</table></div></div></div></section>';
	return($output);
}
...
function fetch_user_chat_history_user(){
	...
	if($_POST['therapist'] != '' && $_POST['user'] != '' ){
		...
		if($from_date == '' && $to_date == '')
			$query = " SELECT * FROM chat_message_details  WHERE ((from_user_id = '".$from_user_id."'  AND to_user_id = '".$to_user_id."')  OR (from_user_id = '".$to_user_id."'  AND to_user_id = '".$from_user_id."')) AND block_status = 0 ORDER BY chat_time DESC";
		else
	  		$query = " SELECT * FROM chat_message_details  WHERE ((from_user_id = '".$from_user_id."'  AND to_user_id = '".$to_user_id."')  OR (from_user_id = '".$to_user_id."'  AND to_user_id = '".$from_user_id."')) AND (chat_time >= '".$from_date." 00:00:00'  AND chat_time <= '".$to_date." 23:59:59') AND block_status = 0 ORDER BY chat_time DESC";
	} else if($_POST['therapist'] != ''){
		$from_user_id = $_POST['therapist'];
		if($from_date == '' && $to_date == '')
			$query = " SELECT * FROM chat_message_details  WHERE (from_user_id = '".$from_user_id."'  OR to_user_id = '".$from_user_id."') AND block_status = 0 ORDER BY chat_time DESC ";
		else
			$query = " SELECT * FROM chat_message_details  WHERE (from_user_id = '".$from_user_id."'  OR to_user_id = '".$from_user_id."') and (chat_time >= '".$from_date." 00:00:00'  AND chat_time <= '".$to_date." 23:59:59') AND block_status = 0 ORDER BY chat_time DESC ";
	} else if($_POST['user'] != ''){
		$from_user_id = $_POST['user'];
		if($from_date == '' && $to_date == '')
			$query = " SELECT * FROM chat_message_details  WHERE (from_user_id = '".$from_user_id."'  OR to_user_id = '".$from_user_id."') AND block_status = 0 ORDER BY chat_time DESC ";
		...
		if($from_date == '' && $to_date == '')
			$query = " SELECT * FROM chat_message_details WHERE block_status = 0 ORDER BY chat_time DESC ";
		else
			$query = " SELECT * FROM chat_message_details  WHERE (chat_time >= '".$from_date." 00:00:00' AND chat_time <= '".$to_date." 23:59:59') AND block_status = 0 ORDER BY chat_time DESC ";
	}   
	$result = $wpdb->get_results($query);
	//print_r($result);
	$output = '<table class="table table-bordered"><thead><tr><th>User ID</th><th>Message From</th><th>Message To</th><th>Message</th><th>Date Time</th></tr></thead><tbody>';
	foreach($result as $row){
		$user_name = ''; 
		...
		if($is_file == 'yes'){
 		...
 		$output .= '<tr><td>'.$row->from_user_id.'</td><td>'.$user_name_from.'<td>'.$user_name.'</td><td>'.$chat_message.'</td><td>'.format_date($row->chat_time).'</td></tr>';
 	}
	$output .= '</tbody></table>';
	echo $output;
	wp_die(); // ajax call must die to avoid trailing 0 in your response
...
function view_logs_therapist(){
	...
	$arr2 = array();
	$arr = array();
	$html = '<div class="container"><div class="row"><div class="col-lg-12 col-md 12 col-sm-12 col-12"><h4>Total Therapists</h4><table class="table table-bordered"><thead><tr><th>Name</th><th>Last Login</th><th>Chat with Customers</th></tr></thead><tbody>';
	if($from_date == '' && $to_date == '')
	...
	foreach($result as $row){
		...
		if((in_array("therapist",$roles_to,true ) )){
			...
			foreach($result1 as $row1){
				...
				$is_blocked = $wpdb->get_var("SELECT block_status FROM chat_block_users WHERE to_user_id = ".$row1->from_user_id." AND from_user_id = ".$row->to_user_id." AND block_status = 1");
				$user_status = $is_blocked == '1' ? "<span style='font-size:12px; color:red;' data-reload = '1' data-to_user_name = '".$name1."' data-to_user = '".$row1->from_user_id."' data-from_user = '".$row->to_user_id."' onclick = 'unblock_user(this)'>(Unblocked)</span>" : "<span style='font-size:12px; color:green;' data-reload = '1' data-to_user_name = '".$name1."' data-to_user = '".$row1->from_user_id."' data-from_user = '".$row->to_user_id."' onclick = 'block_user(this)'>(Block)</span>";
				$arr1[]= '<a href="#" data-to = "'.$row->to_user_id .'" data-from = "'.$row1->from_user_id.'" onclick="show_chat_therepist_user(this)">'.$name1.' '.$user_status.'</a>';
			}
...
function view_logs_user(){
	...
	$arr2 = array();
	$arr = array();
	$html = '<div class="container"><div class="row"><div class="col-lg-12 col-md 12 col-sm-12 col-12"><h4>Total Customer</h4><table class="table table-bordered"><thead><tr><th>Name</th><th>Last Login</th><th>Chat with Therapist</th></tr></thead><tbody>';
	if($from_date == '' && $to_date == '')
...
//                 <a href="https://www.thriive.in" target="_blank"><img src="https://www.thriive.in/wp-content/uploads/2020/11/logo_thriive_new-png-1.png" style="width: 25%;" alt=""> </a></td></tr><tr><td class="content_03" style="width: 100%;height:auto;padding: 30px 40px;background: #fff;vertical-align: top;"><p style="float: left;margin: 0 0 5px 0;font-size: 12px;line-height: 18px;">Hi <strong style="text-transform: capitalize;">'.$s_name.',</strong><br><br>You have started a Online chat with <strong style="text-transform: capitalize;">"'.$t_name.'"</strong> <br><br>'.$t_name.' will review and respond to your query within 6 hours.(Expect a little delay in responses for messages sent between 9:00 pm to 9:00 am) <br><br><a href="https://www.thriive.in/login" target="_blank">View Your Online Chat </a><br><br> Kindly call us at 022 66666035 if you have any query.<br><br><br><br><strong style="text-transform: capitalize;">Keep Thriiving :)</strong><br><strong style="text-transform: capitalize;">Team Thriive Art and Soul</strong></p></td> </tr><tr>
//                 <img src="" style="width: 100%;vertical-align: bottom;" alt=""></td> </tr>
//         <tr> <td class="footer_section" style=" width: 100%;height: 100%;margin:0px auto;padding: 10px 40px;background: #19194a;">
//                 <h4 style="font-size: 24px;line-height:30px;margin: 0;color: #ffb813;	text-transform: uppercase; text-align: center;"></h4><p style="color: #fff; text-align: center;">&copy; 2020 THRIIVE ART &amp; SOUL LLP. All Rights Reserved.</p></td></tr></table></body></html>';
// 	return($body);
// }
...
//     <table border="0" cellpadding="0" cellspacing="0" class="tab-holder" style="background-color: #fff;width: 600px;height: 100%;font-family:"Open Sans",sans-serif !important;margin:0px auto;padding:0;border:1px solid #e4e4e4;"><tr><td align="center" class="header_section" style="width: 100%; height: 100%;margin:0px auto;padding: 20px 30px;background: #fff;"><a href="https://www.thriive.in" target="_blank"><img src="https://www.thriive.in/wp-content/uploads/2020/11/logo_thriive_new-png-1.png" style="width: 25%;" alt=""></a></td></tr><tr>
...
//                     <br><br><a href="https://www.thriive.in/login" target="_blank">View Your Online Chat</a><br><br><br><br><strong style="text-transform: capitalize;">Keep Thriiving :)</strong><br>
//                     <strong style="text-transform: capitalize;">Team Thriive Art and Soul</strong>
//                 </p></td></tr><tr><td>
//                 <img src="" style="width: 100%;vertical-align: bottom;" alt="">
//             </td></tr><tr>
//             <td class="footer_section" style=" width: 100%;	height: 100%;margin:0px auto;padding: 10px 40px;background: #19194a;">
//                 <h4 style="font-size: 24px;line-height:30px;margin: 0;color: #ffb813;text-transform: uppercase; text-align: center;"></h4>
//                 <p style="color: #fff; text-align: center;">&copy; 2020 THRIIVE ART &amp; SOUL LLP. All Rights Reserved.</p>
//             </td>
//         </tr>
//     </table>
// </body>
...
//                     <img src="https://www.thriive.in/wp-content/uploads/2020/11/logo_thriive_new-png-1.png" style="width: 25%;" alt="">
...
//                     news! <strong style="text-transform: capitalize;">'.$s_name.'</strong> wants to consult with you online. Please reply by clicking the link given below.
//                     <br><br><a href="https://www.thriive.in/login" target="_blank">View Your Online Chat </a><br><br><br><br><strong style="text-transform: capitalize;">Keep Thriiving :)</strong><br> <strong style="text-transform: capitalize;">Team Thriive Art and Soul</strong>
//                 </p>
//             </td>
//         </tr>
//         <tr>
...
//         <tr><td align="center" class="header_section" style="width: 100%;height: 100%;margin:0px auto;padding: 20px 30px;background: #fff;"><a href="https://www.thriive.in" target="_blank"><img src="https://www.thriive.in/wp-content/uploads/2020/11/logo_thriive_new-png-1.png" style="width: 25%;" alt=""></a></td>
//         </tr><tr><td class="content_03" style="width: 100%;height:auto;padding: 30px 40px;background: #fff;vertical-align: top;">
//                 <p style="float: left;margin: 0 0 5px 0;font-size: 12px;line-height: 18px;">
//                     Hi <strong style="text-transform: capitalize;">'.$s_name.',</strong><br><br>Great news! Your question has been answered by <strong style="text-transform: capitalize;">'.$t_name.'</strong> <br><br> <a href="https://www.thriive.in/login" target="_blank">View Your Online Chat</a><br><br><br><br>
//                     <strong style="text-transform: capitalize;">Keep Thriiving :)</strong><br>
//                     <strong style="text-transform: capitalize;">Team Thriive Art and Soul</strong>
//                 </p></td></tr><tr><td>
//                 <img src="" style="width: 100%;vertical-align: bottom;" alt=""></td></tr><tr>
//             <td class="footer_section" style=" width: 100%;	height: 100%;margin:0px auto;padding: 10px 40px;background: #19194a;">
//                 <h4 style="font-size: 24px;line-height:30px;margin: 0;color: #ffb813;	text-transform: uppercase; text-align: center;"></h4>
//                 <p style="color: #fff; text-align: center;">&copy; 2020 THRIIVE ART &amp; SOUL LLP. All Rights Reserved.</p>
//             </td>
