...
function new_complete($ocid){
  ...
  global $wpdb;
  $query = "UPDATE online_consultation SET tid_accept=4,chat_end_time=$end_date WHERE id=$ocid";
  $wpdb->query($query);
...
function complete_chat($to,$from){
  ...
  $busy_date = date('Y-m-d H:i:s');
	$query = "UPDATE online_consultation SET remaining_session = 0,busy_date = '$busy_date' WHERE id=$ocid";
  $res = $wpdb->query($query);
  ...
  if($curr_user_role=='therapist'){
    ...
    if(($res[0]->tid_accept==0 OR $res[0]->tid_accept=='-1') && count($res)>0){
      if($res[0]->chat_call_status<1){
          place_call_to_therapist($res[0]->id,'1000058639',$res[0]->session_duration);
      }
      if($res[0]->chat_call_status==1){
      ...
      if(count($ret_menu)<1){$ret_menu = 'NaN';}else{if($ret_menu == 'None'){echo 'u5,'.$res[0]->tid.','.$res[0]->uid.','.$res[0]->tname.','.$res[0]->uname.','.$res[0]->id;} 
      $ret_menu = $ret_menu[0]->menu;}
...
function check_for_response($ocid){
  ...
  if($call_menu == 2 OR $call_menu == 'None'){
    ...
    $curl = curl_init();
      ...
      curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://www.thriive.in/wp-content/themes/thriive/horoscope_new/qiuckcall/test.php?customer_number='.$mobile.'&k_number=8035387035&ivr_id=1000056523&custom_params=123456',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
...
function place_accept_call($ocid){
  global $wpdb;
    ...
    $curl = curl_init();
      ...
      curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://www.thriive.in/wp-content/themes/thriive/horoscope_new/qiuckcall/test.php?customer_number='.$mobile.'&k_number=8035387035&ivr_id=1000057656&custom_params=123456',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
