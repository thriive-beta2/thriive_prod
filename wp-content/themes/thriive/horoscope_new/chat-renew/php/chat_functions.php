...
function endChatDueToInactivity($ocid){
  ...
  $query = "UPDATE online_consultation SET remaining_session = 1, tid_accept='-4',uid_accept='-4',chat_end_time = '$current_time',connected = '-1' WHERE id = $ocid LIMIT 1";
...
function LightboxTherapistInfo($ocid){
  ...
  $tid = $res[0]->tid;
  $therapist_image = get_therapist_image_url($tid);
  $therapist_name = $res[0]->tname;
  $user_name = $res[0]->uname;
  $session_duration = $res[0]->session_duration;
  
  $therapist_data_string = $session_duration.' Mins '.ucfirst($res[0]->action) .' Session with<br> '.$therapist_name;
...
function get_therapist_image_url($tid){
    global $wpdb;
        ...
        endwhile;
          ...
          $idsArray = explode(',', $ids);
            ...
            if(in_array('81924', $idsArray)){
              $category = 'silver-readers';
            }else{
              $category = 'gold-readers';
            }


  $links = array($site_url.'/new-payment-apage?q='.$therapist_slug.'&a=chat&t=tarot-card-reading&c='.$category, $site_url.'/talk-to-best-tarot-card-reader-online', $site_url,$site_url.'/new-payment-apage?q='.$therapist_slug.'&a=call&t=tarot-card-reading&c='.$category);
...
function complete_chat_and_write_to_sql($to, $from, $json_data) {
  ...
  $query = "UPDATE online_consultation SET remaining_session = 0, chat_end_time='$chat_end_time',tid_accept=6 WHERE id=$ocid";
  $res = $wpdb->query($query);
