...
<div class="banner-home">
	<div class="container">	
		<div id="therapyres">	
			...
			<a href="/my-account-page" class="back-btn"> < BACK </a><br/><br/>
				
<?php
	$sessiondata = $wpdb->get_results("SELECT o.id,o.remaining_session,o.therapy_name,o.tid,o.uid,o.package,o.pkgdescription,o.payment_txnid,o.remaining_duration,o.action FROM online_consultation o WHERE o.tid = '". $current_user->ID ."'  AND (tid_accept = 1 OR tid_accept = 6) AND payment_status = 'success' AND month(o.created_date)=month(now())-1 ORDER BY o.id DESC ");
	if(!empty($sessiondata)){
		$currentMonth = date('F');
		$lastmonth =  Date('F', strtotime($currentMonth . " last month"));
		echo "<br/><h3 style='text-align:center;'>Sessions Completed in ".$lastmonth."</h3>";
		$duparr = array();	
		foreach($sessiondata as $sd){
			$payrow = $wpdb->get_var("SELECT COUNT(*) FROM online_consultation WHERE payment_txnid = '". $sd->payment_txnid ."' AND id > '".$sd->id ."'"); 
			if($payrow == 0 || ($sd->remaining_duration == 0 && $sd->action == 'call')){
				$posts = $wpdb->get_row( 
