...
function create_dialog(to, from, mail, msg) {
  ...
  document.querySelector(".main-messages").innerHTML = `
      ...
      <div class="chat_options">
        <table id="chat_options_table">
          <tbody>
            <tr><td data-from='${from}' data-to='${to}' onClick="block_user(this.dataset.from, this.dataset.to)" style="cursor:pointer" class='t-chat-action' id="nUser">Block</td></tr>
  ...
  firestoreDB
    ...
    .then((querySnapshot) => {
      ...
      setTimeout(() => {
          $('.messages').append(` <p id='in-conversation-with' style="font-size:12px;padding: 10px;color: #1565c0; margin-top: 30px; text-align: center"> We have connected you with ${log_name} . This conversation is completely private and confidential.  In case of any issues, please click on Issues/Feedback in Menu.</p>`);
          scrollToBottom();
      }, 2000);
    });
        ...
        if (ret_data[0] == "t0") {
          ...
          if(ret_data[7]==1){
            ...
            setTimeout(function(){
              ...
              op = op+0.1;
              console.log(op);if(op>1){clearInterval(fade_in);}},100);
