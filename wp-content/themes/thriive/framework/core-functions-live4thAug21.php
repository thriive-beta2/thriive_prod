...
function oc_call_seeker_signup(){
	...
	if(!empty($email) && !empty($mobileNumber)  && !empty($firstname)) {
...
function saveOCData($uid,$firstname,$email,$tid,$tfirst_name,$temail,$action,$package,$productinfo,$amount,$status,$txnid,$sendemail,$umobile,$tmobile,$freemins,$coupon,$book_date_time,$tpid,$mihpayid=''){
	...
	if($isexist == 0){
		...
		//	$result = oc_assign_masked_number_to_user($temail,$email);
			...
			$wpdb->query("UPDATE online_consultation SET tid_accept='".$tid_accept."',uid_accept='".$uid_accept."',connected='".$connected."',remaining_session = '0',waiting=1,email_sms=1,call_id = '".$call_id."',busy_date = '".$busy_date."' WHERE id='".$lastid."'");
			//$decrypted_string=openssl_decrypt($encrypted_string,"AES-128-ECB",$password);
			 
			$tempid = '1007507627626666229';
			$tmsg = $uname." has booked an Instant Call session for ".$sessionarr[4]." minutes. Answer the call from +91 8048166487 to start your session.Love & Light - Thriive Wellness.";
			$tempid = '1007909036974742589';
			sendSMS($tidmobile,$tmsg,$tempid);
			//sendSMS('8850630321',$tmsg,$tempid);
			$tsubject = "New Session from ".$uname." -".$lastid;
		    //$tmsg = "Hello ".$tfirst_name.",<br/>Your client ".$uname." has taken ".$sessionarr[2]." and is waiting for a session with you.<br/>Call needs to begin within 2 minutes<br/>Their payment is completed for ".$sessionarr[2]."<br/><br/> Please Note:<br/>There is no auto disconnect for calls. It is your responsibility to end the session within the stipulated time.<br/><br/>Thriive will only make payment for the number of minutes purchased by the client.<br/><br/>You can encourage your client to extend the session, a link for which will be shared with the client after this call ends.";
			$tmsg = "Hi,<br/>".$uname." has booked an Instant Call session with you. Their payment for ".$sessionarr[2]." minutes session is successful.<br/>You will receive a call from +91 8048166487, please answer this call to start your session. For a successful session, ensure that you have a strong network connection on your device.<br/><br/> Note: We now have an auto disconnect feature, wherein the call will disconnect once the purchased session time is over. Please encourage your client to extend the session by purchasing another session from the Thriive website. ";
			$password="password";
			$encrypted_ocid = openssl_encrypt($lastid,"AES-128-ECB",$password);
			$encrypted_uid = openssl_encrypt($uid,"AES-128-ECB",$password);
			...
			$wpdb->query("UPDATE online_consultation SET cron_update=2,waiting=0,remaining_session=1 WHERE id='".$lastid."' AND payment_status = 'success'");	
			
		}
		if(($action == 'chat' || $action == 'book_laterchat') && $status == 'success' && $therapiststatus->availability_status == 1){ 
			//$tmsg = "Your client ".$uname." has taken ".$sessionarr[2]." and is waiting for a response from you. Please login to your account and start online chat with the client on thriive.in";
			//$tmsg = $uname." has booked an Instant Chat Session for ".$sessionarr[4]." minutes. Please start the session within the next 2 minutes.Love & Light - Thriive Wellness";
			
			$tempid = '1007645385754747483';
			
			//sendSMS('8850630321',$tmsg,$tempid);
			$tmsg = $uname." has booked an Instant Chat Session for ".$sessionarr[4]." minutes. Please start the session within the next 2 minutes.Love & Light - Thriive Wellness";
			$tempid = '1007338107017841863';
			...
			$wpdb->query("UPDATE online_consultation SET cron_update=2,waiting=0,remaining_session=0 WHERE id='".$lastid."' AND payment_status = 'success'");	
			
		}
		if(($action == 'book_latercall' || $action == 'book_laterchat'  || $action == 'videocall' || $productinfo == 'Appointment') && $status == 'success'){
			if($action == 'book_latercall'){
				$baction = "call";
			}elseif($action == 'book_laterchat'){ 
				$baction = "chat";	
			...
			//$tmsg = $uname." has booked a Premium session for few minutes. Please start the session 5 minutes in advance to avoid any delay.Love & Light - Thriive Wellness.";

			$tempid = '1007557749947996534';
		//	$tmsg .= site_url()."/accept-reject?tid=".$encrypted_tid.'&uid='.$encrypted_uid.'&ocid='.$lastid;
			//
			...
			sendSMS($tmobile,$tmsg,$tempid);
			//sendSMS('8850630321',$tmsg,$tempid);
			$tsubject = "Client has taken a video call session on ".$split_datetime[0]." at ".$split_datetime[1];
		   // $tmsg = "Hello ".$tfirst_name.",<br/>Your client ".$uname." has taken a video call session  on ".$split_datetime[0]." at ".$split_datetime[1]."";   
		...
		if($sendemail == "yes" && $status == 'success'){
			...
			//	$umsg .= site_url()."/accept-reject?tid=".$encrypted_tid.'&uid='.$encrypted_uid.'&ocid='.$lastid;
				...
				$msg = "Hey,<br/><br/>You have successfully booked an Instant Call session with ".$tfirst_name." for ".$package." minutes. You will receive a call from the number +91 8048166487. For a seamless session, make sure you have a strong network connection on your device.<br/><br/>Note: We now have an auto disconnect feature, wherein the call will disconnect once the purchased session time is over. You can continue the session with your therapist by booking another session from the Thriive website.<br/><br/>All the best!<br/><br/>Love & Light,<br/>Team Thriive";
				sendEmailWithInvoice($email, $subject, $msg, $uid, 'subscriber', 'oc_invoice.php');
				//sendEmailWithInvoice('prathamesh@thriive.in', $subject, $msg, $uid, 'subscriber', 'oc_invoice.php');
			} 
			...
			//sendSMS('9821717700',$alertmsg,$tempid);
			sendSMS('9022107852',$alertmsg,$tempid);
			sendSMS('9137582401',$alertmsg,$tempid);
...
$query = "UPDATE oc_video_call SET page_id=$created_room_id WHERE channel=$room_id";
$wpdb->query($query);
...
function getTherapistConsultStatus($tid,$created_date=''){ 
	...
	//if($consultrow){
		//foreach($consultrow as $cr){
			/* if($cr->call_id != ''){
				$knowrow = $wpdb->get_row("SELECT * FROM knowlarity_after_call WHERE call_uuid = '". $cr->call_id ."'");
				if(!$knowrow){
					$consultrow1->connected = 1;
				} else {
					$consultrow1 = 0;
				}
			} else { */
				/* $session_duration = $cr->session_duration + 5;
				$currenttime = date('H:i:s', strtotime($cr->busy_date ."+". $session_duration . " mins"));
				
				$consultrow1 = $wpdb->get_row("SELECT * FROM online_consultation WHERE tid = '". $tid ."' AND DATE(created_date)  = '".$currentdate."' AND TIME(busy_date) BETWEEN '". date('H:i:s') ."' AND '".$currenttime."' AND payment_status = 'success' AND action='call' ");
				if($consultrow1 > 0){
					return $consultrow1;
				} else {
					 $consultrow1 =0;
...
function getFeedback(){ 
	...
	if($sval != "" ){
		$sql .= " AND (uname LIKE '%".$sval."%' OR uemail LIKE '%".$sval."%'  OR therapy_name LIKE '%".$sval."%'  OR payment_txnid LIKE '%".$sval."%')";
	}  
	...
	$sql = "SELECT f.*,o.tname,o.uname,o.uemail,o.payuid,o.payment_txnid,o.call_id,o.uid,o.tid,o.id,o.action,o.payment_status,o.created_date as payment_date FROM `call_feedback` as f,`online_consultation` as o WHERE 1 ";
	if($mval != ""){
	...
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td>Session Type</td><td>User Name</td><td>User Mobile</td><td> Therapist</td><td>Therapist Mobile</td><td>Feedback</td><td>Payment ID</td><td>Payment Date</td><td>Created Date</td></tr></thead>";
	...
	foreach($result as $r){
			...
			$html .= "<td><a href='#TB_inline?width=3&height=150&inlineId=".$r->cid ."'  class='thickbox' >Feedback</a></td>";
			$html .= "<td>".$r->payment_txnid ."</td>";
			$payrow = $wpdb->get_row("SELECT created_date as payment_date FROM online_consultation WHERE payment_txnid = '".$r->payment_txnid ."' ");
			$html .= "<td>".date("dS M Y",strtotime($payrow->payment_date)) ."</td>";
			$html .= "<td>".date("dS M Y h:i a",strtotime($r->created_date)) ."</td>";
...
function getOnlineConsultation(){ 
	...
	$sql = "SELECT count(*) FROM `online_consultation` WHERE payment_status = 'failed' ";
	if($sval != "" ){
	...
	$sql .= " GROUP BY  payment_txnid ORDER BY created_date  DESC";
	$failed_count = count($wpdb->get_results($sql)); 
	$sql = "SELECT count(*) FROM `online_consultation` WHERE payment_status = 'success' ";
	if($sval != "" ){
		$sql .= " AND (uname LIKE '%".$sval."%' OR uemail LIKE '%".$sval."%' OR package LIKE '%".$sval."%' OR therapy_name LIKE '%".$sval."%' OR amount LIKE '%".$sval."%' OR payment_txnid LIKE '%".$sval."%'  OR payuid LIKE '%".$sval."%')";
	}  
	if($sdval != "" && $edval != ""){
	...
	$html .= "<ul class='subsubsub'><li class='all'>Total Consultation Count: <span class='count'>(".$totalcnt.")</span></li> | <li class='all'>Total Success Count: <span class='count'>(".$success_count."/".$totalcnt.")</span></li> | <li class='all'>Total Failed Count: <span class='count'>(".$failed_count."/".$totalcnt.")</span></li> </ul>";
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td>User Name</td><td>User Email</td><td>User Mobile</td><td>Action</td><td>Chat Menu</td><td>Therapist Response</td><td>User Response</td><td>Remaining Session </td><td>Final Therapist</td><td>Therapist Mobile</td><td>Total Therapist Change Count</td><td>Package</td><td>Therapy Name</td><td>Amount</td><td>Txid</td><td>Payu ID</td><td>Sessoon Date</td><td>Payment Date</td><td>Call / Chat  Time </td><td>Duration </td></tr></thead>";
	...
	foreach($result as $r){
		...
		if(!in_array($r->payment_txnid,$paytxarr) || $r->payment_txnid == ''){
			...
			if($r->action == 'chat'){
				$chat_menu = $wpdb->get_row("SELECT * FROM chat_call_conf WHERE oc_id = '".$second_therapist->id ."' "); 
				$html .= "<td>".$chat_menu->menu."</td>";
				
			} else {
				$html .= "<td>--</td>";
			}		
			if($second_therapist->action == 'chat'){
				if($second_therapist->tid_accept >= 1){
					$html .= "<td>Accepted</td>";
			...
			if($r->payment_txnid != ""){
				$first_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE payment_txnid = '".$r->payment_txnid ."'  LIMIT 1"); 
			} else {
				$first_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE id='". $r->id ."'  LIMIT 1"); 
			}
			$callrow = $wpdb->get_row("SELECT * FROM knowlarity_after_call WHERE call_uuid LIKE '%".$second_therapist->call_id ."%' ");	
			
			if($callrow->agent_status == 'Connected' && $callrow->call_transfer_status == 'Connected'){	
				$connected =  "Connected";	
			} else {
			...
			if($r->payment_txnid != ""){
				$second_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE payment_txnid = '".$r->payment_txnid ."'   ORDER BY id DESC  LIMIT 1");
				//if($second_therapist){
					/* if($second_therapist > 1){  */
						$html .= "<td>". $second_therapist->tname ."</td>";
					/*} else { 
						$html .= "<td>". $r->tname ."</td>";
					}
				...
				/* } else {
					$html .= "<td>".$first_therapist->tname ."</td>";
				} */
			}
			$tidmobile = get_user_meta($second_therapist->tid,'mobile',true);
			$html .= "<td>".$tidmobile."</td>";
			$changetpcnt = $wpdb->get_var("SELECT count(*) FROM online_consultation WHERE  payment_txnid = '".$r->payment_txnid ."'");  
				
			if(($changetpcnt-1) >= 1){
				$html .= "<td><a href='#TB_inline?width=3&height=150&inlineId=".$second_therapist->id ."'  class='thickbox' >".($changetpcnt-1) ."</a></td>"; 
			} else {	
			...
			$html .= "<td>". date("dS M Y",strtotime($first_therapist->created_date)) ."</td>";
			if($second_therapist->action == 'call' || $second_therapist->action == 'chat' || $second_therapist->action == 'videocall'){
				if($second_therapist->action == 'call'){
					if($second_therapist->call_id != "" && $connected  == "Connected"){
						//$callrow = $wpdb->get_row("SELECT call_date,call_duration,call_time FROM knowlarity_after_call WHERE call_uuid = '".$r->call_id ."'");
						if(!empty($callrow)){
							
							
							$secs = strtotime($callrow->call_duration)-strtotime("00:00:00");
							$call_end_time = date("H:i:s",strtotime($callrow->call_time)+$secs);
							//$call_end_time =  date("H:i:s",$call_duration);
							//$html .= "<td>".$call_end_time."</td>";
							$html .= "<td>". $callrow->call_time ." |".$call_end_time."</td>";
							$html .= "<td>". $callrow->call_duration ."</td>";
		...
		$therapistrows = $wpdb->get_results("SELECT * FROM online_consultation WHERE payment_txnid = '". $r->payment_txnid ."' ");
		$manuallyconn = array("0"=>'No',"1"=>'Yes');
		if($therapistrows){
			 $html1 .= '<p><table border="1px">
		<tr>
			...
			<th>Action</th>
			<th>Session Date</th>
			<th>Manually Connected</th>
			<th>Error Code</th>
			<th>knowlarity callid</th>
			<th>Online callid</th>
		</tr>';
			foreach($therapistrows  as $tr){
				...
				$html1 .= '<tr>
					...
					<td>'. $menu .'</td>
					<td>'. $tr->action .'</td>
					<td>'. date("dS M Y H:i:s",strtotime($tr->created_date)) .'</td>
					<td>'. $manuallyconn[$tr->def_warn] .'</td>
					<td>'. $calldata->drop_id .'</td>
					<td>'. $tr->call_id .'</td>
					<td>'. $calldata->call_uuid .'</td>
				</tr>';
			}	
			$html1 .= '</table></p>';
		}
	...
	$html .= "</table><div style='text-align: right;display: inline-flex;'>";
	if($paged > 1){
		$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=onlineconsultationdashboard&paged=1&sd=$sdval&ed=$edval&s=$sval&m=$mval'>First</a></div>";
	}
	if($paged <= 1){
		//$html .= "<div class='disabled' style='margin: 5px 10px;'><a href='#'>Prev</a></div>";
    } else {
	...
	if($paged < $total_pages){
		$html .= "<div style='margin: 5px 10px;'><a href='index.php?page=onlineconsultationdashboard&paged=$total_pages&sd=$sdval&ed=$edval&s=$sval&m=$mval'>Last</a></div></div>";
	}
	$html .= '<script type="text/javascript">function therapistdetails() { $("#changedtherapist").modal(); }</script>';
...
function csv_export_oc_data() {
	...
	$sql = "SELECT * FROM `online_consultation` WHERE ";
	if($mval != ""){
		$sql .= "  payment_status = '". $mval."'";
	}   else {
    ...
    foreach ( $result as $r ) {
		...
		if(!in_array($r->payment_txnid,$paytxarr) || $r->payment_txnid == ''){
			...
			$tmobile = get_user_meta($second_therapist->tid, 'mobile', true);
			if($second_therapist->action == 'call'){
				/* if($r->call_id != "" && $connected  == "Connected"){
					//$callrow = $wpdb->get_row("SELECT call_date,call_duration,call_time FROM knowlarity_after_call WHERE call_uuid = '".$r->call_id ."'");
					if(!empty($callrow)){
						...
						//$call_end_time =  date("H:i:s",$call_duration);
						//$html .= "<td>".$call_end_time."</td>";
						$start_call_time =  $callrow->call_time ." | ".$call_end_time;
						$end_call_time =  $callrow->call_duration;
			...
			/* if($r->action == 'chat'){
				$chat_menu = $wpdb->get_row("SELECT * FROM chat_call_conf WHERE oc_id = '".$r->id ."' "); 
				$chat_menus = $chat_menu->menu;
				
			} else {
				$chat_menus = "--";
			} */
			/* $first_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE payment_txnid = '".$r->payment_txnid ."' AND action='".$r->action ."' LIMIT 1"); 
			
			$ft =  $first_therapist->tname ; */
			if($r->payment_txnid != ""){
				/* $second_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE payment_txnid = '".$r->payment_txnid ."' AND id != '".$r->id ."' AND action='".$r->action ."' ORDER BY id DESC  LIMIT 1");
				if($second_therapist){
					
					if($second_therapist > 1){
						$st = $second_therapist->tname ;
					} else { */
						$st =  $second_therapist->tname ;
					/* }
				} else {
				...
				$second_therapist = $wpdb->get_row("SELECT * FROM online_consultation WHERE uid = '".$r->uid ."' AND id != '".$r->id ."' AND therapy_name = 'Free Covid Session' AND remaining_session > 0 ORDER BY id DESC LIMIT 1");
				if($second_therapist){
					if($second_therapist > 1){
						$st = $second_therapist->tname ;
					} else {
			...
			/*  if($second_therapist->action == 'chat'){
				if($second_therapist->tid_accept >= 1 && $second_therapist->uid_accept >= 1){
					$connected = "Connected";	
				} else {
					$connected = "Not Connected";	
				}
			...
			/* if($r->action == 'videocall'){	
				...
				if($videorow){	
					...
					$connected = "Connected";
					$start_call_time = $videorow->book_time;
					$end_call_time = date("H:i:s",strtotime('+'. $vtime .' mins',strtotime($videorow->book_time)));
				} else {
					$connected = "Not Connected";	
			...
			if($r->payment_txnid != ''){
				$remaining_session = $r->remaining_session;
			} else {
				$remaining_session = "NA";
			}
			$changetpcnt = $wpdb->get_var("SELECT count(DISTINCT(tid)) FROM online_consultation WHERE  payment_txnid = '".$r->payment_txnid ."'"); 
				
			if(($changetpcnt-1) >= 1){
				$changetpcnt = ($changetpcnt-1); 
			} else {	
				$changetpcnt = 0; 
