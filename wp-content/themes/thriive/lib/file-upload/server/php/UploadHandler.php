...
class UploadHandler
{
    ...
    protected function fix_file_extension($file_path, $name, $size, $type, $error,
        ...
        if (strpos($name, '.') === false &&
            preg_match('/^image\/(gif|jpe?g|png|pdf)/', $type, $matches)) {
            $name .= '.'.$matches[1];
        }
    ...
    protected function is_valid_image_file($file_path) {
        if (!preg_match('/\.(gif|jpe?g|png|pdf)$/i', $file_path)) {
            return false;
        }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            $field_name = $this->handle_form_data($file, $index);
            $upload_dir = $this->get_upload_path();
            if (!is_dir($upload_dir)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    //Uploading image as an attachment
                    $current_user = wp_get_current_user();
                    $post_id = $current_user->post_id;

                    $this->get_wp_upload_dependency();
                    $attach_id = media_handle_upload( 'files', $post_id );
                    //var_dump($attach_id);
                    // var_dump($uploaded_file);
                    // var_dump($file_path);
                    // var_dump(get_attached_file($attach_id));
                    //var_dump($field_name);
                    ...
                    else if($field_name == 'select_identity_image')
                    {
                        //set_post_thumbnail( $post_id, $attach_id );
                        update_field($field_name,$attach_id,$post_id);
                    }
                    else if($field_name == 'certificate')
                    {
                        //echo "Reached";
                        $certificate_row = array($field_name => $attach_id);
                        //print_r($certificate_row);
                        $resultData = add_row('certificates', $certificate_row, $post_id);
                        $file->row_id = $resultData;
                        // print_r($resultData);
                        // var_dump($resultData);
                    }
                    ...
                    {
                        //echo "Reached";
                        $gallery_row = array($field_name => $attach_id);
                        //print_r($certificate_row);
                        $resultData = add_row('gallery', $gallery_row, $post_id);
                        $file->row_id = $resultData;
                        //print_r($resultData);
                        // var_dump($resultData);
                    }
                    //$file->image_url = wp_get_attachment_url($attach_id);
                    $file->attach_id = $attach_id;
                    $file->image_url = wp_get_attachment_image_src($attach_id, 'thumbnail');
                    //var_dump($field_name);
                    //End

                    move_uploaded_file($uploaded_file, get_attached_file($attach_id));
                    //$this->set_additional_file_properties($file);
    ...
    public function get_wp_upload_dependency()
    {
    ...
    protected function handle_form_data($file, $index) {
        ...
        return $field_name = $_REQUEST['field_name'];
    }
