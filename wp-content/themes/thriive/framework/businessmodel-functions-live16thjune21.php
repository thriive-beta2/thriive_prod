...
function submit_feedback(){ 
	...
	$call_drop = $_POST['call_drop'];
	$ocid = $_POST['ocid'];
	$callid = $_POST['callid'];
	$query = "INSERT INTO call_feedback (call_opinion,feedback,call_drop,ocid,created_date) VALUES ('".$call_opinion."','".$feedback."','".$call_drop."','".$ocid."',NOW())"; 
	$wpdb->query($query);
	$qy = $wpdb->get_row("SELECT * FROM knowlarity_after_call WHERE call_uuid LIKE '%".$callid."%'");	
	echo 'call duration '.$qy->call_duration .'<br/>';
	echo 'call time '.$qy->call_time .'<br/>';
	echo 'error code '.$qy->drop_id .'<br/>';
	echo 'actual call duration '.$qy->call_timer .'<br/>';
	exit(); 
}	
function response(){
	global $wpdb;
