...
function saveOCData($uid,$firstname,$email,$tid,$tfirst_name,$temail,$action,$package,$productinfo,$amount,$status,$txnid,$sendemail,$umobile,$tmobile,$freemins,$coupon,$book_date_time,$tpid,$mihpayid=''){
	...
	if($isexist == 0){
		...
		if(($action == 'chat' || $action == 'book_laterchat') && $status == 'success' && $therapiststatus->availability_status == 1){ 
			$tmsg = "Your client ".$uname." has taken ".$sessionarr[2]." and is waiting for a response from you. Please login to your account and start online chat with the client on thriive.";
			$tempid = '1007102003722838536';
			sendSMS($tmobile,$tmsg,$tempid);
			sendSMS('8369415594',$tmsg,$tempid);
			$tsubject = "New Session from ".$uname." -".$lastid;
...
function getFeedback(){ 
	...
	$mval = $_GET['m'];
	$start = ($paged-1)*$posts_per_page;
	$sql = "SELECT count(*) FROM `call_feedback` as f,`online_consultation` as o WHERE 1 ";
	if($sval != "" ){
	...
	$sql = "SELECT f.*,o.tname,o.uname,o.uemail,o.payuid,o.payment_txnid,o.call_id,o.uid,o.tid,o.id,o.action FROM `call_feedback` as f,`online_consultation` as o WHERE 1 ";
	if($mval != ""){
	...
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td>User Name</td><td>User Mobile</td><td> Therapist</td><td>Therapist Mobile</td><td>Feedback</td><td>Payu ID</td></tr></thead>";
...
function getOnlineConsultation(){ 
	...
	$success_count = count($wpdb->get_results($sql));  
	//$success_count = $wpdb->get_var("SELECT count(*) FROM `online_consultation` WHERE payment_status = 'success' GROUP BY payment_txnid");
	
	/* $sql = "SELECT DISTINCT(payment_txnid)  as pytx FROM `online_consultation` WHERE 1 ";
	if($mval != ""){
		$sql .= " AND payment_status = '". $mval."'";
	} else {
		$sql .= " AND payment_status = 'success'";
	...
	$sql .= " GROUP BY  payment_txnid ORDER BY payment_txnid DESC"; 
	$oc_count = count($wpdb->get_results($sql));*/
	$oc_count = $success_count ;
	...
	$html .= "<table class='wp-list-table widefat fixed striped pages'>";
	$html .= "<thead><tr><td>User Name</td><td>User Email</td><td>User Mobile</td><td>Action</td><td>Chat Menu</td><td>Therapist Response</td><td>User Response</td><td>Remaining Session </td><td>Final Therapist</td><td>Therapist Mobile</td><td>Total Therapist Change Count</td><td>Package</td><td>Therapy Name</td><td>Amount</td><td>Call/Chat Status</td><td>Payment Status</td><td>Txid</td><td>Payu ID</td><td>Created Date</td><td>Call / Chat  Time </td><td>Duration </td></tr></thead>";
	...
	foreach($result as $r){
		...
		if(!in_array($r->payment_txnid,$paytxarr) || $r->payment_txnid == ''){
			...
			if($r->payment_txnid != ""){
				...
				/* $second_therapist = $wpdb->get_results("SELECT * FROM online_consultation WHERE uid = '".$r->uid ."' AND id != '".$r->id ."' AND therapy_name = 'Free Covid Session' AND remaining_session > 0 ORDER BY id DESC ");
				if(count($second_therapist) > 0){
					if(count($second_therapist) > 1){
						$html .= "<td>". $second_therapist[0]->tname ."</td>";
					} else {
						$html .= "<td>". $r->tname ."</td>";
					}
			...
			if($second_therapist->action == 'chat'){
				$curr_uid = $second_therapist->uid;
				$curr_tid = $second_therapist->tid;
			...
			if($second_therapist->action == 'videocall'){
				$videorow = $wpdb->get_var("SELECT COUNT(*) FROM oc_video_call WHERE  oc_id = '".$second_therapist->id ."'  AND (utime !=0 AND ttime !=0)");	
				if($videorow > 0){
					$html .= "<td>Connected</td>";	
				} else {
					$html .= "<td>Not Connected</td>";	
			...
			$html .= "<td>". $second_therapist->payment_status ."</td>";
