<?php /* Template Name: New Free Pranic Healing online consultation page tee */ 
...
<select id="select_therapy" class="form-control mb-2 mt-2">
            ...
            <?php if(have_rows('free_therapist_data', 'option')):
                while(have_rows('free_therapist_data', 'option')): the_row();
                    if('pranic-healing' == get_sub_field('taxonomy')->slug){
            ...
            // Loop Start
            if($getPost) { ?>
                <h1 class="och1title">Consult Pranic Healing Expert</h1>
                <section class="topreadingList">
                    ...
                    foreach($tempArr as $t){
                        ...
                        if($post->active_for_free_session){
                            setup_postdata( $post );
                            set_query_var('therapy',array('pranic-healing'));
