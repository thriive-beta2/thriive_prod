<?php /* Template Name: New inner child healing appointment page */ 
...
$current_url = add_query_arg( array(), $wp->request ); ?>
 ...
 <style>
    ...
    text-align: center;
		...
		.och1title{color: #333333 !important; text-align:left;margin-top: 18px;margin-bottom: 5px !important;font-weight: 600 !important; font-size:18px !important;text-align: center;}
		h1.hpctitle {
    font-size: 20px;
    width: 100%;
    margin-top: -15px;
...
<div class="container">
	<div class="row">
		<h1 class="hpctitle">Inner Child Healing</h1>
		<p class="hpcdesc">We all have an inner child hidden within the adult. The inner child holds onto all the things that have marked the individual's childhood which has left a scar on the psyche. The conscious adult may have suppressed these memories, but the long buried wounds may unconsciously trigger imbalanced behaviours. Inner child therapy helps the individual access these memories, and release the negative triggers.<p>
		<div class="col-md-6 col-xs-6">
		<p class="maillist">Relationship Issues</p>
		</div>
		<div class="col-md-6 col-xs-6">
		<p class="maillist">Anger Management</p>
		</div>
		<div class="col-md-6 col-xs-6">
			...
			<div id="therapyres">	
				<?php if(have_rows('therapist_data', 'option')):
					while(have_rows('therapist_data', 'option')): the_row();
						if('inner-child-healing'  == get_sub_field('taxonomy')->slug){
				...
				foreach($ids as $id){
					$appointment_cost = get_post_meta($id ,'appointment_cost',true);
				...
				if($getPost) : ?>
					...
					<section class="topreadingList">
						...
						foreach($tempArr as $t){
							...
							set_query_var('therapy',array('inner-child-healing'));
