...
/* echo "SELECT  * FROM appointment_session_details WHERE uid = '". $current_user->ID ."'  AND therapy_name = '".$_GET['t']."' AND payment_status='success' AND sessions_pending > 0"
$sessiondata = $wpdb->get_row("SELECT  * FROM appointment_session_details WHERE uid = '". $current_user->ID ."'  AND therapy_name = '".$_GET['t']."' AND payment_status='success' AND sessions_pending > 0");
