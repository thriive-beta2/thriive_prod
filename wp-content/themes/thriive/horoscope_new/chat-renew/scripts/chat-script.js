...
console.log('live-script');
function book_another_session(dialogue){

    close_emoji();
    
    if(dialogue == 'first'){
    $.post('wp-content/themes/thriive/horoscope_new/chat-renew/php/chat_functions.php', {action:'fetch_link',therapist:to}, function(data){
        ////console.log(data);
...
function feed_star_rating(ocid = 0,therapist_rating,experience_rating){
  $.post('wp-content/themes/thriive/horoscope_new/chat-renew/php/chat_functions.php', {ocid:ocid,action:'feed_star_rating',therapist_rating:therapist_rating, experience_rating:experience_rating},function(data){
    ////console.log(data);
    if(data == 1){
...
function alert_support(mode){
  $.post('wp-content/themes/thriive/horoscope_new/chat-renew/php/GET_CURR_USER.php', {action:'alert_support', ocid:OC_ID, mode:mode}, function(data){
    //console.log(data);
  });
}
...
//the code above only hides previous session messages, fetching is being done anyway.
  ...
  if(message['message'].split(',')[0] == 'hide'){
    generateOnlineNotification(message);
    className = '';
    response = `
      <div id='${id}'>
...
function check_all_chats() {
  ...
  $.post(
    "wp-content/themes/thriive/horoscope_new/chat-renew/php/GET_CURR_USER.php",
...
function generateOnlineNotification(message){
    //console.log(message);
...
if(created_time.length<4 && fetch_time_call == 0){
  if(fetch_time_call == 0){
    fetch_time_call = 1;
  $.post('wp-content/themes/thriive/horoscope_new/chat-renew/php/chat_functions.php',{action:'created_time',ocid:ocid},function(data){
