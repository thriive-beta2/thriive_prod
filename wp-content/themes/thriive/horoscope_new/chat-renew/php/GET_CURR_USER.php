...
function add_session($ocid,$comment){
  ...
  if($ocid){
  $row = $wpdb->get_row("SELECT payuid FROM  online_consultation  WHERE id=$ocid ");
  $row1 = $wpdb->get_row("SELECT session_duration FROM  online_consultation  WHERE payuid= '".$row->payuid."' LIMIT 1");
    $query = "UPDATE online_consultation SET session_duration='".$row1->session_duration."',remaining_duration = '".$row1->session_duration."', remaining_session=1,func_name='add_session1',support_modification='$json_string' WHERE id=$ocid LIMIT 1";
